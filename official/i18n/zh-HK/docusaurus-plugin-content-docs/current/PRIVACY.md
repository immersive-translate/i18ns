...
- 我們僅收集您的電郵，主要用於您的身份標識、登入和找回密碼以及邀請您參與後續產品更新體驗。
- 收集 Google 賬號、Apple 賬號或微信等由第三方提供的 OpenID、電郵，主要是為了提供更好的登入體驗。
- 使用沉浸式翻譯提供的文件/圖片翻譯功能服務時，我們將收集您進行翻譯過程中所輸入的內容（包括圖片、文件），但收集的這些資料不會與您的個人資料相關聯，僅用於改進翻譯服務，以便為您提供便捷高質素的翻譯結果。
- 使用沉浸式翻譯提供的 babeldoc 翻譯功能服務時，我們會收集並儲存您上載的文件內容，主要用於改進翻譯質素，您提交的文本或文件不會被永久儲存，而僅會在提供和傳輸翻譯或改進結果所必需的範圍內暫時保留，且不會與個人資料相關聯。在完全履行合約約定的服務後，所有提交的文本或文件及其翻譯或改進結果都將被刪除。
...
- 供應商：我們為會員提供 pdf 文件翻譯服務時，您上載的 pdf 文件會被加密傳輸到我們的第三方服務供應商 mathpix 進行處理，以提取文件內容並生成翻譯所需的機器可讀文本。在此過程中，我們會嚴格遵守資料最小化原則，僅傳輸必要的文件，且根據合約要求 mathpix 採取行業標準的安全措施來保護您的文件。Mathpix 完成處理後，會將提取的內容傳回給我們，用於為您生成翻譯結果。更多關於 mathpix 的資訊可訪問 [Mathpix 私隱政策](https://mathpix.com/privacy)。另外，我們不會將您的個人資料及個人數據與第三方銷售商或合作夥伴共享，除非是法律規定的必要情況除外。
- 我們使用了第三方翻譯服務，在翻譯時，我們需要將翻譯相關內容發送至這些服務。據我們目前所知，並根據我們與相關服務供應商的合約約定及其公開聲明，這些服務供應商不會將相關內容用於其模型的訓練。

我們在提供翻譯功能時，為實現翻譯目的，相關文本內容可能會被安全地傳輸至這些第三方服務進行處理。用戶直接從客戶端發起請求，如自定義 API 等，不屬於此處描述的我們所提供之翻譯功能。
...
您亦可透過註銷賬戶之方式請求刪除您的個人資料。您可於「個人中心－賬戶管理－註銷賬戶」路徑提交註銷申請。賬戶註銷申請提交後將立即生效，您將無法再登入或使用相關服務。您的用戶資料、翻譯紀錄、訂單資訊亦將立即進行刪除處理，但法律法規另有規定者除外，且 15 日內註冊賬號所使用之電郵將無法再次註冊。
...
6. 資料可獲取與可攜帶權（如適用）  
在技術可行且法律要求的範圍內，您有權請求以結構化、通用且機器可讀的格式取得您提供給我們的個人資料，或在對您而言可行的情況下，將其傳輸至其他服務提供者。
...
我們不會出售您的個人資料。如需行使相關權利，您可以透過本政策第十一條所列聯絡方式與我們聯絡。

您可透過本政策第十一條所列聯絡方式向我們提交上述請求。為保障您的個人資料安全，我們可能在合理範圍內核實您的身份。

## 四、自動化決策與畫像

為提升服務質量和用戶體驗，我們可能基於您對本服務的使用情況，對相關資料進行分析處理，並形成反映一般使用習慣或偏好的標籤資料。該類分析主要基於您在使用本服務過程中的功能使用情況、操作行為及互動方式等資料，並以歸類或統計方式進行處理。為降低對您個人資料的影響，標籤生成和處理會在您的裝置本地完成，我們不會收集或儲存您所瀏覽的具體網站地址或詳細瀏覽內容。

在上述基礎上，我們可能將相關標籤資料與您的帳戶或裝置識別碼進行關聯，用於理解整體用戶使用情況、優化產品功能與服務體驗、改進內容展示及功能設計，並在適用情況下提供更符合用戶需求的產品或服務。

我們不會在未經人工介入的情況下，僅基於自動化處理對您作出對您產生重大法律或類似影響的決定。在不同司法管轄區內，如相關法律對自動化處理或算法決策有額外要求，我們將依法履行相應義務。

因此，本服務目前不適用有關自動化決策的特別規則。上述基於使用情況的分析和標籤處理，不會對您產生重大法律影響或類似重大影響，我們亦不會以可能對您個人權益產生不利影響的方式，僅透過自動化手段處理您的個人資料。

如未來我們在特定場景中引入可能對您產生重大影響的自動化決策或進一步的資料分析處理方式，我們將依法向您提供必要的信息說明，並在適用法律要求的情況下徵得您的同意。
...
「cookie」是網站儲存在用戶電腦上、由 web 伺服器生成的一小段資料。
...
您可透過啟用瀏覽器設定，以拒絕設置全部或部分 cookies。關於如何執行此功能，請參閱特定瀏覽器的說明，了解如何執行此操作。然而，如您使用瀏覽器設定阻擋所有 cookies（包括絕對必要的 cookies），您可能無法存取我們的網站或部分內容。
