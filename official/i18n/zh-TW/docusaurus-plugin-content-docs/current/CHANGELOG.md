...
- 修復：【輸入框翻譯】在 WhatsApp 中膠囊下拉選單底部溢出的問題。
- 新增：自訂 API 服務支援根據 Base URL 自動補全 OpenAI / Anthropic 介面路徑。
- 優化：Popup 文件翻譯入口，提升入口可見性與使用體驗。
...
- 優化：側邊欄文字翻譯體驗。
- 優化：線上字幕翻譯請求改為受翻譯服務段落數控制。
- 優化：403 / Cloudflare 錯誤彈窗提示。
- 新增：Popup 更多功能新增翻譯快取開關。
- 新增：字幕翻譯新增站點獨立開關。
...
- 修復：Atlas 瀏覽器中側邊欄無法開啟的問題。

## 1.29.1 Release (2026-05-07)

- 新增：支援 plamo 翻譯服務。
- 優化：僅譯文模式下 RTL 語言的排版標記。
- 優化：改善英文術語在部分複數形式下的匹配效果，減少術語命中遺漏。
- 修復：Teams 字幕翻譯時字幕抖動的問題。
- 修復：【劃詞翻譯】在 ShadowRoot 場景下無法正常取詞的問題。

## 1.28.5 Release (2026-04-24)

- 修復：滑鼠懸停時直接翻譯譯文消失的問題。
- 修復：Twitter 字幕下載失敗的問題。
- 新增：在新版面板中新增文件翻譯和文字翻譯入口。
- 修復：部分瀏覽器中文件翻譯快取失敗的相容性問題。
- 修復：僅譯文模式下複雜文字結構漏翻的問題。
...
- 優化：提升了免費 AI 翻譯服務的表現與穩定性。
- 優化：加強了韓語及日語翻譯場景中 AI 的上下文理解能力，進一步提升翻譯品質。
- 優化：提升非 Chrome 瀏覽器下輸入框自動語言識別的準確性與穩定性。
- 新增：Google 翻譯服務現已支援塔馬齊格特語（Tamazight）。
- 修復：解決 iOS 全螢幕播放 YouTube 影片時字幕無法翻譯的問題。

## 1.26.6 Release (2026-03-02)

- 新增：支援 YouTube 直播字幕翻譯。
- 修復：【輸入框翻譯】語言代碼前綴被誤翻譯的問題（例如 `hi hello` 被誤翻譯為 `नमस्ते`）。
- 修復：Android 沉浸式翻譯 App 升級會員後，外掛狀態未同步的問題。
...
- 修復：修正部分瀏覽器下側邊欄無法正常顯示的問題，提升相容性與穩定性。
- 優化：iOS Safari 插件與 APP 的地區切換實現同步，體驗更連貫。
- 優化：進一步提升線上 PDF 文件翻譯於多場景下的相容性。
- 優化：網頁譯文顯示將更好地保留原網站的互動與視覺效果，保持一致性。
...
- 優化：優化了設定信息的同步機制，提升了同步效率和穩定性。

## 1.23.10 Release (2025-12-05)

- 優化：提升 AIFW 隱私模式下資訊識別的準確性。
- 優化：新增 babel-lite 翻譯服務。

## 1.23.8 Preview (2025-11-22)

- 優化：支援 App 端的快捷譯文樣式設定，可設定字體加粗、底線、顏色、背景色。
...
- 修復：【輸入框翻譯】在部分 Android 系統的瀏覽器（如：Edge）下無法觸發的問題。
- 修復：【輸入框翻譯】提升對 Atlas 瀏覽器與指紋瀏覽器的相容性。
...
- 優化：兼容 o1，o3 這類不支持 system 角色的模型（System Prompt 設定為空時不再傳 system 角色參數）。
