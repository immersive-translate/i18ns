...
最新支援 [AI 術語庫](https://dash.immersivetranslate.com/#terms) 功能，請注意此術語庫僅支援 AI 翻譯服務。

AI 術語庫預設不支援 Google / 微軟這類機器翻譯的術語。

機器翻譯模型採用的是佔位符替換，使用術語庫可能會導致翻譯品質下降。
...
由於 Gemini 系列模型的特殊性，擴展內置了部分設定。用戶若想覆蓋擴展內置的設定，可以參考以下配置：
...
> 為減少大型語言模型出現「幻想」的問題，擴充功能內建了翻譯品質校驗機制。系統會通過對比回應文本與請求文本的 Token（符號）數量比例，來判斷翻譯結果的合理性。當該比例異常（過高或過低）時，當前結果會被視為無效，並自動切換到備用翻譯方案。
>
> 如果你的自訂提示詞屬於非翻譯類任務（如擴寫、潤飾或其他指令），可能會導致 Token 比例不符合常規標準。為此，你可以透過設定以下參數，強制讓程式嚴格執行你的自訂指令，跳過比例校驗：
