...
建議電腦端透過設定[輸入增強快捷鍵](https://dash.immersivetranslate.com/#shortcuts)觸發翻譯
...
`ja 這裡輸入中文` 即可翻譯為日語，`fr 輸入中文` 即可翻譯為法語。其他語言類似，請在下方檢視所有語言程式碼

## 只翻譯部分內容

“`你好世界`” -> “`你好 //世界`”翻譯 -> “`你好 world`”
...
其他語言程式碼參見[這裡](#language-code)

## 相容性

- 已知明確不支援的瀏覽器：

- 安卓 Kiwi 瀏覽器（由於在該瀏覽器中無法監聽移動端輸入的事件，正在研究最佳化中，從版本 1.4.8+ 起，支援透過手勢快捷觸發翻譯操作）
- 安卓 狐猴瀏覽器（由於在該瀏覽器中無法監聽移動端輸入的事件，正在研究最佳化中，從版本 1.4.8+ 起，支援透過手勢快捷觸發翻譯操作）
- Arc 瀏覽器
- 安卓目前只有 Firefox 測試是支援的
...
- qura 的回覆框空格無法翻譯 [快捷鍵觸發支援]

這是因為 qura 回覆框無法輸入 2 個以上空格導致的，所以需要去輸入框的設定修改預設觸發符號成其他的

## 常見問題

- 輸入框無反應
  - 檢查上述相容性
  - 去 https://www.bing.com/ 嘗試，空格快一點
...
- 阿姆哈拉語，am, Amharic
- 阿拉伯語，ar, Arabic
- 自動偵測語言，auto, Detect Language
- 亞塞拜然語，az, Azerbaijani
- 白俄羅斯語，be, Belarusian
- 保加利亞語，bg, Bulgarian
- 澤納語，tn, Zana
- 孟加拉語，bn, Bengali
