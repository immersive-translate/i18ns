...
在 google.com 等搜尋頁面不生效，外掛顯示`請先刷新當前頁面，再開始翻譯`且刷新頁面仍提示該信息：需要在 Opera 外掛設定中找到【沉浸式翻譯】，開啟【允許訪問搜尋頁面結果】選項。
...
系統設定 -> safari 瀏覽器-> 高級 -> 網站數據 -> 編輯 找到 immersivetranslate.com 刪除
...
- 沉浸式翻譯外掛為瀏覽器外掛，**只能在瀏覽器**中運行，無法在其他 app 中使用，不支持其他 App 內部翻譯。
...
我們提供術語表配置功能，您可以通過術語表來指定某些詞彙不被翻譯，或將其翻譯為特定內容。詳細配置請參閱“進階自訂”中的“自訂專業術語翻譯”部分：[點擊這裡查看](https://immersivetranslate.com/docs/advanced)

### 5. Word 檔案翻譯後無法匯出為 Word 格式

目前 word 檔案匯出存在一定技術問題，建議保持現有格式，正在持續優化中
