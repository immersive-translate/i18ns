...
因為檢測您的 chrome 內核低於 115 版本，不支持插件部分高級特性，所以自動降級為 1.13.5 版本。
...
- 在所有頁面隱藏：打開【設置頁】-【介面設置】，關閉【在頁面上顯示懸浮球】即可
...
擴展本身會在使用的時候定時同步官方最新對網站適配的規則，你也可以手動同步最新的規則，點擊瀏覽器的沉浸式翻譯擴展圖標，打開彈窗後擴展會自動檢測最新適配規則並同步，油猴腳本同理。
...
因為插件調用的谷歌免費 API，是谷歌不會持續維護的舊 API，而谷歌官方網頁的翻譯是在持續維護。理論上質量是不如谷歌官方的，且最近谷歌免費 API 的翻譯質量下降嚴重，建議切換到其他翻譯服務，我們也在積極尋找其他替代方案。相關討論：[#2574](https://github.com/immersive-translate/immersive-translate/issues/2547)

### 23. 滑鼠模式下卻顯示觸摸模式

在[進階設置](https://dash.immersivetranslate.com/#advanced)中，開啟僅滑鼠模式即可。1.14.9 版本將優化這個模式判斷
...
- 當前 Netflix 採用漸進式翻譯方案，字幕樣式自托管，不支持人工字幕。
- 舊方案，需要等到所有字幕翻譯完才會顯示字幕，支持人工字幕。
...
打開【[文件快速查找應用 (Everything)](https://www.voidtools.com/zh-cn/downloads/)】應用，搜索 `amkbmndfnliijdhojkpoglbnaaahippg` 沉浸式的擴展 id 瀏覽器的存儲文件夾，刪除之後，然後再去卸載重裝插件
