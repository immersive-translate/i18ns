...
Este registro de alterações é atualizado de acordo com o progresso do desenvolvimento. A data após a versão é a data de mesclagem do código, não a data de release nas lojas de aplicativos (o tempo de revisão varia após a submissão a cada loja de aplicativos, com algumas levando até uma semana para revisão). Atualmente, estamos avançando com duas versões.
...
- Adicionado: Pré-tradução dinâmica de conteúdo a ser traduzido ao navegar por páginas.
...
- Corrigido: Problemas de compatibilidade com o modo "Somente Tradução" quando usado com plugins de leitura como o Reader View.
...
- Otimizado: Otimizada a formatação de texto misto em Chinês e Inglês.
...
- Otimizado: Compatível com modelos como o1, o3 que não suportam papel de sistema (o parâmetro de papel de sistema não será passado quando a configuração de System Prompt estiver vazia).
...
- Melhorado: Item de menu do clique direito pode ser fechado (operado nas configurações avançadas).
...
- Novo: Tradução de imagem gratuita agora está disponível (atualmente suportada apenas nas versões para PC dos navegadores Chrome e Edge), acessível via menu do clique direito.
...
- Corrigido: O problema onde a **Tradução por Hover do Mouse** não funciona em alguns sites.
...
- O serviço de tradução DeepL agora oficialmente suporta Chinês Tradicional como idioma de destino (anteriormente, traduzir para Chinês Tradicional com DeepL envolvia um processo adicional de conversão de Chinês Simplificado para Chinês Tradicional de terceiros).
...
- Nova funcionalidade: tradução de mangá (Beta), em sites de mangá suportados, um botão de tradução de mangá aparecerá abaixo da bola flutuante de tradução rápida da página. Clicando nele ativará a tradução de mangá. Este recurso está disponível para membros Pro (500 páginas por mês, pacotes adicionais podem ser adquiridos), atualmente suportando os seguintes sites:
...
- Corrigido o problema em que modificar o idioma de destino e outras configurações no painel pop-up redefinia ao clicar na bola flutuante de tradução.
...
- Otimização significativa do desempenho de tradução de grandes páginas da web, criação de ePub e arquivos de legendas.
...
- Corrigido o problema em que legendas bilíngues do YouTube não podem ser ajustadas para cima e para baixo em tela cheia [#1654](https://github.com/immersive-translate/immersive-translate/issues/1654)
...
- Melhorias foram feitas para resolver o problema em que legendas bilíngues imersivas não eram exibidas quando os usuários definiam o idioma das legendas como chinês no site do YouTube.
...
- Otimizada significativamente a velocidade de tradução das legendas do Youtube, e adicionado suporte para alternar entre ordem chinês e inglês, personalizar cor e tamanho da fonte, etc.
...
- Otimizado o controle de frequência de solicitações de serviço de tradução
- Recentemente, as solicitações de serviço de tradução da Microsoft de dentro da China têm sido instáveis; quando ocorrem erros, o sistema detecta automaticamente o serviço de tradução atualmente disponível, permitindo que os usuários mudem rapidamente.
- Otimizada a mensagem de erro para erros de rede
- Corrigido o problema onde configurar a cor do texto não suportava pré-visualização RGBA [#1435](https://github.com/immersive-translate/immersive-translate/issues/1435)
...
- Otimizados os pop-ups de erro, pop-ups de erro de problema de rede detectam automaticamente serviços de tradução gratuitos válidos.
- Corrigido o suporte do APP imersivo iOS para reprodução em tela cheia do YouTube.
...
- Otimização: Página de configurações, interação da lista de URLs (para evitar ambiguidade, caixas de seleção não são exibidas por padrão).
...
- Otimização adicional da função de tradução de PDF embutido no Sci-Hub
- Corrigido um problema com PDFs online não abrindo corretamente
- Corrigido o problema com reprodução contínua de legendas bilíngues na Netflix
...
- Adicionado suporte para uma nova configuração de atalho que permite alternar rapidamente se as legendas de vídeo usam as legendas de tradução automática embutidas
- No site Sci-Hub, clicar na bola flutuante traduzirá PDFs embutidos na página da web

## 1.1.6

- **Suporte Móvel para Tradução de Parágrafos Específicos:** A versão móvel agora suporta a tradução de parágrafos especificados e adicionou uma variedade de operações de atalho, incluindo deslizar para a esquerda, deslizar para a direita, toque duplo, toque triplo e gestos de toque com vários dedos. Estes não são ativados por padrão e exigem que o usuário selecione ativamente o gesto de acionamento na página de configurações em [Mouse Hover].
...
- **Otimização de Tradução Japonesa:** Otimizada ainda mais a tradução japonesa da OpenAI para melhorar a precisão e fluência.
...
- **Novo Guia do Usuário**: Quando novos usuários abrem uma página em um idioma não-alvo pela primeira vez, uma bolha de ajuda de bola flutuante é exibida.
...
- Otimizado: Definido automaticamente o idioma de destino com base no idioma do navegador.
...
- Para versões mais antigas do script Tampermonkey, um aviso de atualização foi adicionado à página de configurações.
...
- Corrigir problema de oscilação da página de tradução da caixa de entrada do Reddit.
...
- O recurso de Aprimoramento de Pesquisa (algumas páginas de Pesquisa do Google exibem resultados de pesquisa bilíngues) agora não está habilitado por padrão e será removido na próxima versão.
...
- Suporte ao passar o mouse para pressionar Ctrl novamente para mostrar o texto original.
- Ignorar idioma nunca traduzido ao passar o mouse.
...
- Corrigir problema de tradução da chave de autenticação personalizada do DeepL
...
- Otimizar a contextualização da tradução do DeepL
...
- Corrigir tradução automática quando o readwise é destacado
...
- Suporta [Habilitar Assinatura Pro](/pricing), que permite aos usuários desfrutar de traduções do Deepl e OpenAI e configurações de sincronização em nuvem.
...
- Suporte a Resultados de Tradução com ctrl+z Desfazer
...
- Adicionar exibição de página sem permissão para PDF
- Reparar a exibição de parágrafos da lista de texto do PDF
- Ampliar a escala de fonte pequena do PDF no chrome e safari
...
- Aprimoramento da caixa de entrada para mostrar status de Carregando na tradução
...
- Otimizar o problema de traduções do OpenAI adicionando pontos desnecessariamente, (se nenhum ponto for detectado no texto original, se o openai retornar um ponto, então removê-lo)
...
- Corrigir o problema de que as imagens em algumas páginas da web: ficam maiores após a cópia
...
- Corrigir: Tampermonkey Toggle mostra apenas traduções
...
- Recurso: Bing Translate para Microsoft Translate para uma qualidade mais estável.
...
- Corrigir: detecção de espaço em branco em tags sup, sub.
...
- Corrigir: postMessage ao iniciar a página quebrou alguns sites, agora faremos isso apenas quando realmente traduzirmos páginas
...
- Recurso: Suporte a Tencent (alpha)
...
- Correção: problema de rede do serviço de tradução no firefox mobile.
...
- Recurso: Suporte a quadro principal personalizado, pois alguns sites não usam body como quadro principal
...
- Correção: Permitir configuração de API do Google personalizada
...
- Correção: configuração do Azure translate, mas ainda mostra (precisa de configuração)
...
- Suporte a requisições limitadas para múltiplos quadros
...
- Correção de configuração de popup em nova aba
...
- Correção de permanência em configurações abertas.
...
- Suporte a tradução de iframe inline de userscript.
- Suporte a tradução de shadowroot. Por exemplo:
...
- Suporte a tradução de iframe, como email do QQ, tweet incorporado.
...
- Correção de globalThis.GM do safari não funcionando

## 0.2.13

- Suporte a Arrastar Popup de Userscript
- Suporte a Três Dedos em dispositivo de toque para acionar alternância de páginas de tradução
- Suporte a Ocultar o ícone de popup do userscript.
...
- Corrigir mudança de elemento de prevenção no telegra.ph
...
- Corrigir tradução de img em bloco.
...
- Suporte a página de pesquisa do YouTube.
...
- Suporte a resultado de tabela do Google
  [#3](https://github.com/immersive-translate/immersive-translate/issues/3)
