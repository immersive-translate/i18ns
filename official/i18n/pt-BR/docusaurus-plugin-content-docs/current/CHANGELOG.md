...
- Adicionado: Suporte para definir o número de telas a serem pré-traduzidas ao navegar na página atual.
...
- Adicionado: Modo Forçar Suporte ao Mouse (Ative este recurso apenas se a função de passar o mouse não estiver disponível em dispositivos tablet) **Configurações** -> **Configurações Avançadas** -> **Forçar Suporte ao Mouse**
...
- Suporte aprimorado para entrada no Baidu, Gmail e outros campos de entrada
...
- Adotado algoritmo de divisão de sentenças inteligente por IA para legendas automáticas em inglês no YouTube [Pro Disponível]
...
- Legendas bilíngues do YouTube agora suportam divisão inteligente de sentenças (Beta) (Apenas quando manualmente ativando tradução imersiva de legendas do YouTube em [Configurações] - [Legendas de Vídeo], e as legendas originais do vídeo são legendas automáticas em inglês)
...
- Novo recurso: suporte para modo de comparação bilíngue com tradução primeiro e texto original em seguida, que pode ser habilitado na página de configurações -> configurações avançadas.
- Lista de modelos AI personalizados suporta a sintaxe `-all`, que pode deletar todos os modelos predefinidos.
...
- Suporte para configurações de ativação independente para legendas bilíngues de vídeo do YouTube (habilitado por padrão para novos usuários)
...
- Serviço de tradução Gemini é oculto por padrão, pode ser habilitado em configurações de desenvolvedor para mostrar beta deste serviço de tradução
...
- Otimização: Página de configurações, interface de interação da lista de URLs (para evitar ambiguidades, caixas de seleção não são exibidas por padrão).
- Suporte para configurar a fonte de tradução no modo de tradução apenas.
...
- **Atualização da Interface do Painel Pop-up:** O novo design visa melhorar a usabilidade e compreensão. Esta atualização inclui:
...
- **Entrada de tradução de documentos integrada:** Agora, você pode traduzir arquivos PDF, ePub e legendas através de uma entrada de upload unificada. Basta clicar no botão [PDF/ePub] no painel pop-up, sem necessidade de selecionar [Mais] mais.
...
- Corrigido o problema onde traduzir campos de entrada contendo URLs traduzia apenas partes do conteúdo.
...
- **Remoção de Recurso**: Removida a função de aprimoramento de informações bilíngues (exibindo resultados de pesquisa em inglês nas páginas de pesquisa do Google).
...
- Adicionado suporte à tradução Gemini.
...
- Aumentar a escala de fonte pequena em PDF no chrome e safari
...
- Recurso: Adicionar `<all>` para sempre traduzir idiomas, agora você pode usá-lo para traduzir todos os idiomas, exceto o idioma alvo, e nunca traduzir idiomas
...
- Suporte a definição da primeira contagem de texto x para traduzir imediatamente.
