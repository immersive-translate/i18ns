...
- Adicionado: Gerenciamento de glossário AI (suporta apenas serviços de tradução AI)
...
- A opção de comparação bilíngue/exibição apenas de tradução foi movida para configurações gerais.
...
- Corrigidos problemas de layout de texto nas traduções de quadrinhos para idiomas no script latino.
...
- Quando especialistas em IA são usados para "Intelligent Selection", diferentes especialistas em IA podem ser personalizados para diferentes sites. Isso pode ser configurado em [Configurações] -> [Especialistas em IA] -> [Insira qualquer especialista].
- Corrigido o problema onde legendas não eram exibidas no YouTube no modo "Translation Only".
...
- Corrigido o problema onde modificar o idioma alvo e outras configurações no painel pop-up resetava ao clicar na bola flutuante de tradução.

## 1.5.8

- Especialistas em IA suportam o modo "Intelligent Selection", onde o sistema selecionará automaticamente o especialista em IA mais adequado com base no site atual (por exemplo, especialistas em IA relacionados à tecnologia serão automaticamente selecionados para The Verge e Hacker News, enquanto o aprimoramento de tradução do Twitter será automaticamente selecionado para o Twitter).
...
- Otimizados prompts múltiplos do OpenAI, suportando formato YAML, o que melhora a flexibilidade e facilidade de uso da configuração
...
- Otimização do problema de exibição de tradução de legendas de vídeo do Twitter CC.
...
- Corrigido [Quando temporariamente ativando a tradução de página da web por um longo tempo, clicar no painel [Sempre traduzir este site] não cancela a tradução sempre #1172](https://github.com/immersive-translate/immersive-translate/issues/1172)
...
- A bola flutuante no celular agora suporta operação de pressionamento longo, pressionamento curto para traduzir, pressionamento longo para abrir o painel.
- Traduzir e-books bilíngues agora também traduzirá o índice.
- O recurso de Aprimoramento de Pesquisa (algumas páginas de Pesquisa do Google exibem resultados de pesquisa bilíngues) agora não está habilitado por padrão e será removido no próximo Release.
...
- Resolvido o problema de que algumas plataformas não conseguiam reconhecer se o mouse era suportado ou não
  - Existem alguns navegadores de desktop que suportam tanto touchscreen quanto mouse, e o Immersive Translates é tecnicamente incapaz de detectar se tais plataformas suportam mouse, então adicionamos a opção [Forçar Habilitar Suporte a Mouse] na configuração [Mouse Hover]
...
- Suporte para configurar botões de tradução rápida da bola flutuante (suporte para PC/celular)
...
- Corrigir conflito de legendas do Youtube com o plugin de legendas duplas (a tradução de legendas do Youtube pelo Immersive Translate não é ativada quando o plugin de legendas duplas do Youtube é detectado para evitar conflitos)
...
- Corrigir problema de tradução com chave de autenticação personalizada do DeppL

## 0.10.3

- Suporte perfeito para vídeos do Youtube com legendas bilíngues 🎉.
- Para páginas de artigos, o texto principal agora será traduzido antes do restante do conteúdo da barra lateral
...
- Tradução de Caixa de Entrada: exibe um lembrete pop-up quando usado pela primeira vez, e o usuário pode optar por desativá-lo desta vez ou permanentemente para evitar toques acidentais.
- Otimização de velocidade de exportação de tradução apenas para PDF, se você escolher exportar apenas a tradução, pode chamar diretamente a visualização de PDF do sistema para exportar, mais rápido.
...
- Serviço de tradução de caixa de entrada pode ser configurado separadamente
...
- Desativar temporariamente a função de exportação de PDF do Safari devido à restrição do Safari em downloads de plug-ins (bug).
...
- Suporte a Resultados de Tradução de Suporte a Entrada Aprimorada ctrl+z Desfazer
...
- Aprimoramento de Caixa de Entrada: permite omitir o prefixo // e acionar a tradução de toda a caixa de entrada com 3 espaços, ou você pode desativar essa opção na página de configurações.
...
- Tradução de caixa de entrada otimizada, agora funciona suavemente no Bard!
...
- Aprimoramento de caixa de entrada para mostrar status de Carregando na tradução
...
- Tradução de Caixa de Entrada suporta tradução direta de toda a caixa de texto sem prefixos.
...
O idioma padrão para tradução de caixa de entrada agora pode omitir o espaço, ou seja, //Hello World pode ser traduzido também.

## 0.6.2

O aprimoramento de caixa de entrada mais emocionante está aqui:
...
- Corrigir: API do OpenAI não funciona.
...
- Correção: Alguns sites usam `span` para estilos, então usamos `font` em vez de span para o wrapper de tradução
- Correção: limite máximo de tokens do OpenAI, mudar caracteres máximos de 1500 para 1300.

## 0.2.81

- Correção: m.youtube.com
- Correção: opções de formulário UI
...
- Recurso: Suporte para verificar serviço de tradução em options.html
- Recurso: Suporte para quadro principal personalizado, pois alguns sites não usam body como quadro principal
...
- Correção: mudar transparência de 0.4 para 0.618 [#147] (https://github.com/immersive-translate/immersive-translate/pull/147)
...
- Correção: estilo global não mudou após elementos dinâmicos gerados
...
- Suporte a três dedos em dispositivos de toque para acionar a alternância de páginas traduzidas
...
- Corrigir telegra.ph para evitar mudança de elemento
...
- Formatar informações de meta de userscript
...
- Suporte a detectar mais contêineres de conteúdo

## 0.0.37

- Corrigir mudança de serviço de tradução de popup não funciona
...
- Suporte a definir área da página que precisa ser traduzida. para que você possa traduzir mais área.
- Suporte a definir a contagem de texto dos primeiros x para traduzir imediatamente.
...
- Suporte a traduzir a página inteira
- Suporte a traduzir página imediatamente
...
- Suporte a UI básica de popup

## 0.0.4.6

- Suporte a verificar nova versão
