...
- Corrigido: Um problema onde a tecla de atalho de tradução ao passar o mouse não funcionava em dispositivos que suportam tanto mouse quanto toque.
...
- Melhorado: Item do menu de clique com o botão direito pode ser fechado (operado nas configurações avançadas).
...
- Corrigido: Falha de sincronização do Google Drive em alguns navegadores.
...
- Melhorado: Quando o idioma da interface é definido para Inglês, o idioma de destino padrão para a **Caixa de Entrada Aprimorada** é alterado para Chinês.
...
- Adicionado: Exibir um aviso quando a língua de origem e a língua alvo forem as mesmas
...
- Melhorado: Funcionalidade de aprimoramento de entrada e de passar o mouse sobre iframes incorporados em páginas da web

## 1.10.2 (2024-10-11)

- Adicionado: Tradução de imagens (versão Beta)
- Adicionado: Modo Forece Enable Mouse Support (Ative este recurso apenas se a função de passar o mouse não estiver disponível em dispositivos tablet) **Configurações** -> **Configurações Avançadas** -> **Forece Enable Mouse Support**
...
- A opção para exibição de comparação bilíngue/tradução apenas foi movida para configurações gerais.
...
- O serviço de tradução DeepL agora suporta oficialmente o Chinês Tradicional como idioma alvo (anteriormente, traduzir para Chinês Tradicional com DeepL envolvia um processo adicional de conversão de Chinês Simplificado para Tradicional de terceiros).
...
- Corrigido o problema em que as legendas não eram exibidas no YouTube no modo "Somente Tradução".
...
- Corrigido o bug de sincronização em vários dispositivos no Pro.
...
- Corrigido tempo de sincronização em vários dispositivos para usuários Pro não sincronizando
...
- Corrigido o problema em que o YouTube Shorts sem legendas sempre exibe legendas do vídeo anterior [#1655](https://github.com/immersive-translate/immersive-translate/issues/1655)
...
- Suporte para legendas bilíngues no [VK Video](https://vk.com/video)
...
- Implementar suporte à internacionalização (i18n) para navegação em documentos de página
...
- Suporte para tradução bilíngue no Google Docs
...
- Otimizada significativamente a velocidade de tradução das legendas do Youtube, e adicionado suporte para alternar entre ordem em chinês e inglês, personalizar cor e tamanho da fonte, etc.
...
- A funcionalidade de Aprimoramento de Busca (algumas páginas de Busca do Google exibem resultados de busca bilíngues) agora não está habilitada por padrão e será removida no próximo release.
...
- Resolvido o problema de que algumas plataformas não conseguiam reconhecer se o mouse era suportado ou não
  - Existem alguns navegadores de desktop que suportam tanto touchscreen quanto mouse, e o Immersive Translates tecnicamente não consegue detectar se tais plataformas suportam mouse, então adicionamos a opção [Forçar Habilitar Suporte ao Mouse] na configuração [Mouse Hover]
...
- Deeplx suporta múltiplas URLs, basta separá-las com .
...
- Suporte a pdf para ajustes de espaçamento de linha e palavra para re-reconhecer parágrafos
- Correção do problema de rolagem automática na leitura online de epub em dispositivos móveis

## 0.8.7

- Suporte a pdf apenas para downloads de tradução
...
- Suporte a Resultados de Tradução com ctrl+z Desfazer no Aprimoramento de Entrada
...
- Funcionalidade é amplamente inalterada (novos recursos estarão disponíveis no próximo lançamento!)
...
- Suporte a não traduzir hashtags como #hash, tags como @xxx, $tag, como $APPL
