...
- Adicionado: Gerenciamento de biblioteca de terminologia AI
...
- Corrigido: Tradução de barra lateral inválida nas configurações avançadas.
...
- ~~Melhorado: Aprimorada a exibição de traduções bilíngues em várias linhas no Twitter.~~
...
- O serviço de tradução DeepL agora oficialmente suporta Chinês Tradicional como idioma alvo (anteriormente, traduzir para Chinês Tradicional com DeepL envolvia um processo adicional de conversão de Chinês Simplificado para Tradicional de terceiros).
...
- Adicionado tradução de grande modelo Zero-One-Thing, que pode ser usado preenchendo a Chave API após registrar-se na plataforma Zero-One-Thing.
...
- Legendas bilíngues do YouTube agora suportam divisão inteligente de sentenças (Beta) (Apenas quando ativar manualmente a tradução imersiva de legendas do YouTube em [Configurações] - [Legendas de Vídeo], e as legendas originais do vídeo são legendas automáticas em inglês)
...
- Corrigido um problema em que alguns usuários Pro não conseguiam modificar as configurações no navegador Chrome.
...
- Resolvido o problema de que algumas plataformas não conseguiam reconhecer se o mouse era suportado ou não
  - Existem alguns navegadores de desktop que suportam tanto touchscreen quanto mouse, e o Immersive Translates tecnicamente não consegue detectar se tais plataformas suportam mouse, então adicionamos a opção [Forçar Habilitar Suporte a Mouse] na configuração [Mouse Hover]
...
- Ampliar escala de fonte pequena do PDF no chrome e safari
...
- Otimização de tradução de PDF, área de tradução agora pode ser editada/arrastada/excluída
  - Arrastar canto superior esquerdo, excluir canto superior direito, mudar tamanho canto inferior direito
...
- Aprimoramento de caixa de entrada para mostrar status de Carregamento na tradução
...
O idioma padrão para tradução de caixa de entrada agora pode omitir o espaço, ou seja, //Hello World também pode ser traduzido.
...
- Otimizar: O número mínimo de caracteres em um parágrafo que aciona a tradução foi modificado para um mínimo de 4 caracteres para reduzir a confusão, enquanto usa outros recursos para evitar traduzir as áreas de navegação e final do site.
- Corrigir: detalhes do Github não traduzindo após expandir.

## 0.5.14

- Corrigir o problema de que imagens em algumas páginas da web: ficam maiores após copiar
...
- Correção: sincronização do google drive no firefox android.
...
- Recurso: Recurso de Tradução Imersiva Deepl
...
- Correção: Elemento de Bloqueio de Tags Desconhecidas
...
- Suporte para tradução de iframe inline no userscript.
...
- Suporte para três dedos em dispositivos touch para acionar a alternância de páginas traduzidas
...
- Suporte para definir a área da página que precisa ser traduzida. Assim, você pode traduzir mais áreas.
