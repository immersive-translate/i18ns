...
- Adicionado: Navegador Android suporta tradução de imagem gratuita. 【Pendente, precisa aguardar o release do navegador】
...
- Otimizado: Compatível com modelos como o1, o3 que não suportam system role (o parâmetro system role não será passado quando a configuração do System Prompt estiver vazia).
...
- Adicionado: Tradução de **Subtítulos em Reuniões em Tempo Real** agora suporta ativação via "bola flutuante", disponível no Zoom, Google Meet e Microsoft Teams.
...
- Suporte para definir a fonte da tradução no modo somente tradução.
...
- Corrigir o problema de oscilação da página de tradução da caixa de entrada do Reddit.
...
- Otimizar contextualização de tradução de arquivos de legendas do OpenAI.
...
- Fix: Popup de Userscript do Firefox ausente.
...
- Fix: popup de política de segurança de conteúdo para extensão, e parcialmente para userscript.
...
- Fix: detecção de espaço em branco de tag sup, sub.
...
- Correção: sincronização do google drive nativa no firefox android.
...
- Correção de mudança de URL do cliente, auto manter o estado de tradução.
