...
- Otimizado: Serviço de tradução Gemini suporta prompt de sistema customizado (System Prompt).
...
- Corrigido: Problema de exibição de tradução customizada deepseek.

## 1.14.11 Release (2025-02-18)

- Corrigido: Erro `401: Authentication Fails` na chave API customizada do DeepSeek.
...
- Corrigido: Problema de correspondência inteligente de especialistas em AI em serviços de tradução customizados.
...
- ~~Melhorado: Adicionada a opção de customizar a "**Habilitar Tradução de Barra Lateral & Navbar**" em "**Configurações Avançadas**".~~
...
- A tradução Claude agora suporta tibetano e permite a configuração do parâmetro de Temperatura
...
- Adicionado tradução de grande modelo Zero-One-Thing, que pode ser usado preenchendo a Chave API após o registro na plataforma Zero-One-Thing.
...
- Otimizado prompts de erro para tradução de legendas bilíngues locais
...
- A página de configurações agora acomoda ajustes para [estilos de legenda do YouTube](https://dash.immersivetranslate.com/#subtitle).
...
- Adicionado opções de 50%, 150% para tamanho de fonte de legenda do Youtube
...
- Otimizado prompts múltiplos do OpenAI, suportando formato YAML, o que melhora a flexibilidade e facilidade de uso da configuração
...
- **Novo Guia do Usuário**: Quando novos usuários abrem uma página pela primeira vez em um idioma não-alvo, uma bolha de ajuda de bola flutuante é exibida.
...
- Correção: Problema de reconhecimento do idioma alvo padrão em algumas máquinas após a primeira instalação.
...
- O recurso de Aprimoramento de Pesquisa (algumas páginas do Google Search exibem resultados de pesquisa bilíngues) agora não está habilitado por padrão e será removido na próxima versão.
...
- Corrigido o problema em que alterar o atalho de passar o mouse exigia uma atualização da página para ter efeito
...
- Feat: Nova página de guia para novos usuários.
...
- Fix: Página da web TXT pura como [esta](https://edoras.sdsu.edu/~vinge/misc/singularity.html)
...
- Fix: Progresso do Construtor de Ebook 99% parado.
...
- Feat: OpenAI suporta apiUrl personalizado para configurações de desenvolvimento.
...
- Feat: Suporte a tradução de página da web txt
...
- Fix: Alguns sites usam `span` para estilos, então usamos `font` em vez de span para o wrapper de destino de tradução
...
- Corrigir: alto uso de memória para userscripts e extensão (removendo opencc zh-CN para zh-TW, em vez de usar Google translate)
...
- Corrigir erro de sincronização do google drive do userscript.
