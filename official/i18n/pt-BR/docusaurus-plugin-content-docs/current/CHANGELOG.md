...
- Corrigido: Problema onde a **Mouse Hover Translation** não conseguia traduzir texto rico.
...
- Melhorado: Adicionada uma entrada de avaliação da loja nas opções **More** no painel.
...
- Corrigido: O problema onde a **Mouse Hover Translation** não funciona em alguns sites.
...
- Adicionado: Tradução de **Subtítulos em Reuniões em Tempo Real** agora suporta ativação via "float ball", disponível no Zoom, Google Meet e Microsoft Teams.
...
- Melhorado: Reduzir o tamanho do pacote de instalação
...
- Melhorado: Funcionalidade de aprimoramento de entrada e de passar o mouse dentro de iframes incorporados em páginas da web
...
- A opção para exibição de comparação bilíngue/apenas tradução foi movida para configurações gerais.
...
- Por padrão, traduzir Chinês Simplificado para Chinês Tradicional e vice-versa usará o modo apenas tradução, em vez do modo de comparação bilíngue.
- Corrigidos alguns bugs.

## 1.9.1 (2024-09-03)

- Suporte para configurar exceções para idiomas e sites no modo de contraste bilíngue ou apenas tradução (configure na página de Configurações -> Configurações Avançadas). Por exemplo: Se o seu modo de tradução padrão é contraste bilíngue, mas você não deseja que o Chinês Tradicional também use contraste bilíngue, então você pode adicionar o Chinês Tradicional aos idiomas de exceção para contraste bilíngue, assim o Chinês Tradicional usará o modo apenas tradução para tradução. Da mesma forma, se o seu modo de tradução padrão é apenas tradução, mas você deseja que um determinado idioma ou site use o modo de contraste bilíngue, você também pode adicionar esse idioma ou site aos idiomas de exceção.
...
- Otimizado o desempenho de tradução de texto rico.
...
- Corrigidos problemas de tradução com o script do Tampermonkey SiliconCloud
...
- Legendas bilíngues do YouTube agora suportam divisão inteligente de sentenças (Beta) (Apenas quando habilitar manualmente a tradução imersiva de legendas do YouTube em [Configurações] - [Legendas de Vídeo], e as legendas originais do vídeo são legendas em inglês geradas automaticamente)
...
- Otimizadas palavras de prompt multi OpenAI, suportando formato YAML, o que melhora a flexibilidade e facilidade de uso da configuração
...
- Recentemente, as solicitações de serviço de tradução da Microsoft na China têm sido instáveis; quando ocorrem erros, o sistema detecta automaticamente o serviço de tradução atualmente disponível, permitindo que os usuários mudem rapidamente.
...
- Suporte para configurar a fonte da tradução no modo de tradução apenas.
...
- Adicionado um modo apenas de tradução na página de configurações.
...
- **Guia para Novos Usuários**: Quando novos usuários abrem uma página em um idioma não-alvo pela primeira vez, uma bolha de ajuda de bola flutuante é exibida.
...
- Otimizado: Ao usar o mouse hover, o problema de perder o foco na página e depois reativar se torna ineficaz.
...
- A funcionalidade de Aprimoramento de Pesquisa (algumas páginas de Pesquisa do Google exibem resultados de pesquisa bilíngues) agora não está habilitada por padrão e será removida no próximo release.
...
- Corrigir problema de tradução da chave de autenticação personalizada do DeppL
...
- correção de problema de rolagem automática em leitura online de epub em dispositivos móveis
...
- Recurso: aplicativo iOS adiciona botão de ativação de extensão e link de comunidades
...
- Corrigir: permissão do userscript do Open AI
...
- Recurso: Suporte a frame principal personalizado, pois alguns sites não usam body como frame principal
...
- Corrigir: configuração do Azure translate, mas ainda mostra (precisa de configuração)
...
- Corrigir idioma de origem indefinido na tradução caiyun
...
- Corrigir configuração de popup em nova aba
- Corrigir substituição de placeholder de tradução

## 0.2.39

- Suporte a atalhos para mostrar tradução de máscara
- Suporte a ativar recurso beta no painel do desenvolvedor
...
- Suporte a opção de mostrar menu de contexto.
...
- Corrigir problema de requisição do macaque
...
- Corrigir solicitação grease monkey
...
- Adicionar arquivo js dist ao repositório público
...
- Suporte a deeplx
- Corrigir múltiplas traduções ao alternar serviço
...
- Suporte a não traduzir hashtags como #hash, at tags como @xxx, $tag, como $APPL
