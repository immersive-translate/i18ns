...
Este registro de alterações é atualizado de acordo com o progresso do desenvolvimento. A data após a versão é a data de mesclagem do código, não a data de lançamento nas lojas de aplicativos (o tempo de revisão varia após a submissão a cada loja de aplicativos, com algumas levando até uma semana para revisão). Atualmente, estamos avançando com duas versões.
...
- Corrigido: Resolvido um problema onde algum conteúdo era perdido durante a tradução em múltiplos segmentos no Gemini.
...
- A opção de comparação bilíngue/exibição apenas de tradução foi movida para as configurações gerais.
...
- Corrigido um problema onde alguns usuários Pro não conseguiam modificar configurações no navegador Chrome.
...
- **Entrada de tradução de documentos integrada:** Agora, você pode traduzir arquivos PDF, ePub e de legendas através de uma entrada de upload unificada. Basta clicar no botão [PDF/ePub] no painel de pop-up, sem necessidade de selecionar [Mais] mais.
...
- Corrigir cliques trêmulos do hoverball na página.
...
- Otimização da velocidade de exportação de tradução apenas para PDF, se você escolher exportar apenas a tradução, pode chamar diretamente a visualização do sistema PDF para exportar, mais rápido.
...
O aprimoramento mais empolgante da caixa de entrada está aqui:
