...
We mainly use the following categories of cookies on our website:

| Context | Description | Expiry |
| --- | --- | --- |
| First-party persistent cookies | Used to remember your language preferences, recognize login status, record your country or region, retain download/onboarding context, and, where necessary, keep experiment assignment or source attribution consistent so we can provide core functionality, continuity of experience, product optimization, and performance measurement. | The retention period is set according to the specific purpose and is typically 3 days, 30 days, or 365 days, and does not exceed 13 months. |
| Third-party analytics cookies | Primarily set by analytics services such as Google Analytics and Microsoft Clarity to aggregate traffic, recognize returning visits, measure page performance, understand usage journeys, and help us improve our website and services. | The retention period is set according to the specific service configuration and is typically for the session duration or between 30 days and 13 months. |
| Third-party functional and payment cookies | Set by payment or security-related services (such as Stripe) to support payment flow integrity, risk control, fraud prevention, and service stability. | The retention period is set according to the specific service configuration and is typically for the session duration or no more than 12 months. |
| Third-party session cookies | Set by third-party services during the current visit to support service loading, security checks, or temporary state retention. | Expire when you close your browser. |

Apart from the situations described above, we do not currently use cookie-based cross-site targeted advertising cookies on the website.
