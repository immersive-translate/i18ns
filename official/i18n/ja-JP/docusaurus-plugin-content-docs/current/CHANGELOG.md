...
- 改善: **Image Translation** が weibo.com と互換性を持つように（Chrome と Edge のみサポート）
...
- 改善: Twitter では、複数行のコンテンツ翻訳がデフォルトで折り返されないように設定。コンテンツが10行または1000文字を超える場合のみ折り返しが発生。設定で有効化可能 **Advanced Settings** -> **Enable automatic line wrapping for long paragraphs**
...
- 追加: **AI Context-Aware Translation** が専門的なコンテンツ翻訳の精度を向上（Pro メンバーのみ利用可能、OpenAI モデルでのみサポート）**Options** -> **Grneral** -> **Enable AI Context-Aware Translation**
...
- 改善: "**Hover - immediately translate this paragraph**" モードで画像が翻訳されなくなる

## 1.11.4 (2024-11-16)

- 修正: バージョン 1.11.2 の「Twitter 翻訳改善」による数式翻訳の問題

## 1.11.2 (2024-11-13)

- Fixed: Facebookの翻訳専用モードで「もっと見る」をクリックした後にコンテンツが消える問題を修正しました。
- ~~Improved: Twitterでの複数行のバイリンガル翻訳の表示を改善しました。~~
...
- Improved: PDF翻訳中に翻訳ボタンが機能しない可能性がある問題に対処しました。
...
- Removed: 「SiliconCloud」モデルqwen1.5-7B-chatを削除しました（公式の終了により）。
- Fixed: macOS 15でのSafariプラグインとのログイン互換性の問題を解決しました。
...
- サードパーティのウェブサイト向けにイマーシブ[JS SDK](https://immersivetranslate.com/docs/js-sdk/)の統合をサポートしました。
- Huluの字幕表示を最適化しました。
...
- 新機能：漫画翻訳（ベータ版）、サポートされている漫画サイトでは、ウェブページのクイック翻訳フローティングボールの下に漫画翻訳ボタンが表示されます。クリックすると漫画翻訳が有効になります。この機能はProメンバー向けで（月500ページ、追加パック購入可能）、現在以下のサイトをサポートしています：
...
- AI翻訳サービスが[AI Expert]を設定して翻訳戦略を指定することをサポートし、現在ベータ機能であり、ベータを有効にした後、[Developer Settings](https://dash.immersivetranslate.com/#developer)で有効にでき、ページを更新すると[AI Expert]メニューが表示されます。
...
- 翻訳サービスが別のタブで明確に整理され、すべての利用可能な翻訳サービスの包括的な概要を提供します。さらに、ユーザーは表示する翻訳サービスをカスタマイズする柔軟性があります。デフォルトでは、限られた翻訳サービスのみが表示されますが、[More Services](https://dash.immersivetranslate.com/#services)セクションで表示設定を調整できます。
...
- Gemini翻訳サービスはデフォルトで非表示になっており、開発者設定でこの翻訳サービスのベータ版を表示するように有効にできます
...
- Safariプラグインバージョンのアップグレード時に常にインストール成功ページが表示される問題を修正
...
- `Ctrl+C`を`Ctrl`と誤認しないようにCtrlホバー検出の感度を最適化
- 動画字幕が組み込みの機械翻訳字幕を使用するかどうかを素早く切り替える新しいショートカット設定を追加
...
- **openAI設定更新**: 秒あたりの設定数を小数で設定できるようになりました。例えば0.5は2秒に1回のリクエストを意味します。

## 0.12.14

- 修正: 初回インストール後、一部のマシンでのデフォルトターゲット言語認識の問題。
...
- 一部のビデオウェブサイトで字幕スイッチが翻訳を繰り返し表示するバグを修正しました。
- 一部のケースでショートカットキーを押さずにマウスをホバーして翻訳を続ける問題を修正しました。
- macOSでパネルショートカットの表示名を最適化しました。

## 0.12.8

- 「現在のサイトを翻訳しない」に設定されている場合、元のビデオ字幕が表示されない問題を修正しました。
- 一部のプラグインとの競合で無限にページが再表示される問題を修正しました。
- 長文の改行をオンにした後、一部の段落が翻訳されない問題を修正しました。
...
- Firefoxページのホバーボールアクションパネルのクリックの揺れ問題を修正しました。
...
- [長時間ウェブページ翻訳を一時的にオンにしたとき、常に翻訳をキャンセルしない #1172](https://github.com/immersive-translate/immersive-translate/issues/1172)
- PDFファイルの初期化問題を最適化
...
- より多くのビデオプラットフォームに対してバイリンガル字幕サポートを提供し、現在サポートされているのは: [Youtube](https://www.youtube.com/)、[Netflix](https://www.netflix.com)、[Udemy](https://www.udemy.com/)、[Khanacademy](https://www.khanacademy.org/)、[Coursera](https://www.coursera.org/)、[Vimeo](https://vimeo.com/)、[Nebula](https://nebula.tv)、[Bloomberg](https://www.bloomberg.com)、[Bilibili](https://www.bilibili.com/)などです。（技術的制限により、一部のウェブサイトではバイリンガル字幕を初めてオンにした後、ページをリフレッシュする必要があるか、翻訳が完了するのを待つ必要があります）
- プラグインのzipサイズを大幅に最適化し、元の半分に削減し、ダウンロードと更新がより速くなりました。
- 拡張PDFダウンロードの問題を修正
...
- プラグインと公式ウェブサイトのために11の新しいインターフェース言語を追加し、現在サポートされているインターフェース言語は14に達しました。これには、中国語簡体字、中国語繁体字、英語、日本語、韓国語、ロシア語、スペイン語、ポルトガル語、ヒンディー語、イタリア語、ドイツ語、フランス語、アラビア語、ペルシャ語が含まれます。
...
- 一部のプラットフォームがマウスをサポートしているかどうかを認識できない問題を解決しました
  - 一部のデスクトップブラウザはタッチスクリーンとマウスの両方をサポートしており、Immersive Translatesは技術的にそのようなプラットフォームがマウスをサポートしているかどうかを検出できないため、[マウスホバー]設定に[マウスサポートを強制的に有効にする]オプションを追加しました
...
- Youtubeの字幕がDual subtitleプラグインと競合する問題を修正（Youtube Dualプラグインが検出された場合、競合を避けるためにImmersive TranslateのYoutube字幕翻訳は有効になりません）
- [ビデオ字幕機能を永久に無効化]を追加。他の競合する問題があり、Immersive Translateでバイリンガル字幕機能を有効にしたくない場合に使用します。
- 字幕の改行を最適化

## 0.10.3

- Custom DeppL Auth Key翻訳の問題を修正
...
- 最適化: 翻訳をトリガーする段落の最小文字数を4文字に変更し、混乱を減らしつつ、サイトのナビゲーションや終了エリアの翻訳を避けるために他の機能を使用。
...
- OpenAIリクエスト頻度制限問題の最適化

- ChatGPTはデフォルトでモバイルモデルを使用し、より速い！
...
- 修正: ebookビルダーの特定の文字問題、例えば一部の章のパスが`xxx' xxxx`のようになる。
...
- 機能: 新しいテーマ「Solid Border」を追加 (新テーマ、実線枠)
...
- 修正: PDF ファイルの翻訳失敗、0.2.60 からの zh-CN から zh-TW への deepl サポートのバグ
...
- PDF の基本翻訳を修正
- 設定されていないサービスを選択したときにオプションページに移動するポップアップを修正。
- 設定を開いたままにする問題を修正。
- 複数言語の検出速度を修正
...
- タッチデバイスで3本指を使用して翻訳ページのトグルをトリガーすることをサポート
...
- sendMessage を使用して connect の代わりに、Chrome が5分後にポートを切断するため
- テキストコンテナの検出を改善
...
- 即時翻訳モードを設定することをサポートし、ウェブページをできるだけ早く翻訳できます。
- 翻訳が必要なページ領域を設定することをサポートし、より多くの領域を翻訳できます。
- 最初の x テキスト数を即時に翻訳することをサポート
...
- #hash や @xxx、$tag、$APPL のようなハッシュタグやアットタグを翻訳しないことをサポート
- ブロック要素をサポート
