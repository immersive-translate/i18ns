...
- AIエキスパートの詳細ページにエキスパートが使用するプロンプトを表示
...
- YouTubeバイリンガル字幕がインテリジェント文分割をサポート（ベータ）（[設定] - [ビデオ字幕]でYouTube字幕のイマーシブ翻訳を手動で有効にした場合のみ、元の動画字幕が自動生成された英語字幕である場合）
...
- AI翻訳サービスはモデルリストをカスタマイズ可能になり、[OpenAI]など、システムには最も一般的に使用されるモデルがいくつか組み込まれています。モデルのドロップダウンリストをクリックすると、最後に表示される項目は[Set More Models]で、設定後は異なるカスタムモデルをテストするために自動的に記憶されます。
- 一部のケースで翻訳のレイアウトの不一致を最適化。
- YouTube字幕スタイルのリセットボタンを追加し、デフォルトスタイルに迅速に戻すことが可能。
...
- YouTubeウェブサイトで字幕言語を中国語に設定した際に没入型バイリンガル字幕が表示されない問題を解決。
...
- 中国からのMicrosoft翻訳サービスリクエストが不安定な場合があり、エラーが発生するとシステムが現在利用可能な翻訳サービスを自動的に検出し、ユーザーが迅速に切り替えられるようにします。
...
- **プラグイン設定ページUI更新**:
  ...
  - 「高度な設定」に、段落を翻訳するために必要な最小文字数を設定する新しいオプションが追加されました。デフォルトは4文字ですが、より高く設定することも可能（例：20文字）で、長い段落のみが翻訳されます。
...
- **openAI設定更新**: 秒あたりの設定数を小数で設定することが可能になり、例えば0.5は2秒ごとに1リクエストを意味します。
...
- 修正: 一部のケースでOpenAIの長い段落翻訳の結合問題。 [#1276](https://github.com/immersive-translate/immersive-translate/issues/1276)
...
- Deeplxは複数のURLをサポートし、で区切るだけで済みます。

## 0.9.2

- PDF翻訳ツールがオンライン版に移行しました: https://app.immersivetranslate.com/pdf/ 、これによりGrease MonkeyやSafariでもPDF翻訳が使用でき、問題を解決するためにバージョンを発行する必要なく、問題をより良く反復できます。
...
- PDFファイルを開くときに、拡張パネルが権限がないと表示される問題を修正
...
- 拡張パネルがサポートされていないページを示すことができるようになりました（例: 権限がないページや非HTMLページ）
...
入力ボックス翻訳のデフォルト言語はスペースを省略できるようになり、//Hello Worldも翻訳可能です。

## 0.6.2

最もエキサイティングな入力ボックス強化が登場:

- 任意のウェブページの入力ボックスに // Hello World と入力し、スペースバーを3回クリックすると、段落を英語に翻訳
- 特定の言語への翻訳を指定することもできます: /ja Hello World と入力し、スペースバーを3回クリックすると、段落を日本語に翻訳
...
- 修正: 純粋なTXTウェブページ [こちら](https://edoras.sdsu.edu/~vinge/misc/singularity.html)
...
- バージョン: 今後、マイナーバージョン番号を毎月変更します。例えば、3月は0.3.0から始まり、4月は0.4.0から始まります。これは、拡張機能がセマンティクスに従う意味がないため、時間の法則に従ってバージョン番号を標準化することが、開発を継続的に更新する動機となり、ユーザーが問題をより簡単に見つけることができるためです。
...
- 修正: オプションフォームUI
- 修正: Open AIプロンプト
- 機能: OpenAIの複数キーをサポート、`,`で分割。

## 0.2.80

- 機能: ポップアップ -> moreの有効/無効メニューを追加
- 修正: DingTalkメッセージの競合
...
- 修正: firefoxモバイルでの翻訳サービスのネットワーク問題を確認。
...
- 機能: options.htmlで翻訳サービスの確認をサポート
...
- 改善: Deepl Freeのサポート
- 修正: ユーザースクリプトと拡張機能の高メモリ使用量 (openccのzh-CNからzh-TWへの変換を削除し、Google翻訳を使用)
- 修正: Relingo [#159] (https://github.com/immersive-translate/immersive-translate/issues/159)
- 修正: Azure翻訳の設定が必要と表示される問題
...
- Chrome/FirefoxでのGoogle Authのログインが必要な問題を修正
- 翻訳サービスリンクを置き換え
- 権限を改善。
- minifyを削除。
...
- selectorMatchesをサポートし、すべてのmanstondonを一致させることができます。
