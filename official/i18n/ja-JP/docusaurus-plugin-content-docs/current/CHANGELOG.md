...
- 新規: 無料画像翻訳が利用可能になりました（現在、Chrome と Edge の PC バージョンのみサポート）、右クリックメニューからアクセス可能
...
- 改善: Twitter では、複数行のコンテンツ翻訳がデフォルトで折り返されないように設定。コンテンツが10行または1000文字を超える場合のみ折り返しが発生。設定で有効にすることが可能 **高度な設定** -> **長い段落の自動改行を有効にする**
...
- Fixed: "Twitter Translation Improvement" による数式翻訳の問題をバージョン 1.11.2 で修正しました。

## 1.11.2 (2024-11-13)

- Fixed: Facebook の翻訳専用モードで「もっと見る」をクリックした後にコンテンツが消える問題を修正しました。
- ~~Improved: Twitter での複数行のバイリンガル翻訳の表示を強化しました。~~
- Improved: パネル内の翻訳サービスのドロップダウンリストの UI を更新しました。

## 1.11.1 (2024-11-05)

- Added: リアルタイム会議の **Subtitles Translation** が「フロートボール」でのアクティベーションをサポートし、Zoom、Google Meet、Microsoft Teams で利用可能になりました。
- Fixed: YouTube で広告を視聴した後の字幕の同期タイミングの問題を修正しました。
- Fixed: MacOS 15 の Safari での右クリック翻訳メニューの表示問題を修正しました。
- Fixed: 特定のウェブサイトでの **Enhanced input** の Ctrl+Z の元に戻す機能の問題を修正しました。
...
- Added: ソース言語とターゲット言語が同じ場合に警告を表示します。
- Fixed: リッチテキストの空白文字翻訳の問題 [#2175](https://github.com/immersive-translate/immersive-translate/issues/2175) を修正しました。
- Improved: ウェブページ内の埋め込み iframe での入力強化とマウスホバー機能を改善しました。

## 1.10.2 (2024-10-11)

- Added: 画像翻訳（ベータ版）
- Added: マウスサポートモードの強制有効化（タブレットデバイスでマウスホバー機能が利用できない場合にのみこの機能を有効にします） **Settings** -> **Advanced Settings** -> **Forece Enable Mouse Support**
- Added: ビデオ字幕翻訳が失敗した場合にエラーメッセージを表示します。
- Fixed: リッチテキスト翻訳の問題 [#2163](https://github.com/immersive-translate/immersive-translate/issues/2163) を修正しました。
...
- Removed: 「SiliconCloud」モデル qwen1.5-7B-chat を公式終了に伴い削除しました。
- Fixed: macOS 15 の Safari プラグインでのログイン互換性の問題を解決しました。

## 1.9.7 (2024-09-20)

- Baidu、Gmail などの入力フィールドでの Enhanced input サポートを強化しました。
- Claude Anthropic API の anthropic-dangerous-direct-browser-access リクエストヘッダーをサポートしました。
...
- 同じ Twitter ページ上の複数のビデオでのバイリンガル字幕表示の問題を修正しました。
- いくつかのバグを修正しました。

## 1.9.3 (2024-09-05)

- バイリンガル比較/翻訳専用表示のオプションが一般設定に移動しました。
- デフォルトでは、バイリンガル比較または翻訳専用表示のためにパネル内のアイコンをクリックして切り替えたモードをシステムが記憶します。一時的に切り替えるには、パネル内の「More」->「Switch to translation-only display」をクリックします。
- デフォルトでは、簡体字中国語から繁体字中国語への翻訳とその逆は、バイリンガル比較モードではなく翻訳専用モードを使用します。
- いくつかのバグを修正しました。

## 1.9.1 (2024-09-03)

- バイリンガルコントラストまたは翻訳専用モードでの言語とウェブサイトの例外を設定するサポート（設定ページ -> 高度な設定で設定）。例えば、デフォルトの翻訳モードがバイリンガルコントラストである場合でも、繁体字中国語がバイリンガルコントラストを使用しないようにしたい場合は、繁体字中国語をバイリンガルコントラストの例外言語に追加することで、繁体字中国語は翻訳専用モードを使用します。同様に、デフォルトの翻訳モードが翻訳専用である場合でも、特定の言語やウェブサイトがバイリンガルコントラストモードを使用するようにしたい場合は、その言語やウェブサイトを例外言語に追加することができます。
...
- DeepL 翻訳サービスが正式に繁体字中国語をターゲット言語としてサポートしました（以前は、DeepL で繁体字中国語に翻訳する際に追加のサードパーティの簡体字から繁体字への変換プロセスが必要でした）。
- リッチテキスト翻訳のパフォーマンスを最適化しました。

## 1.8.3

- Google Meet がライブ会議でのバイリンガル字幕をサポートしました：Google Meet 会議でバイリンガル字幕機能を楽しむことができます。会議リンクを開き、イマーシブ翻訳パネルでバイリンガル字幕をアクティブにし、ページをリフレッシュしてください。
...
- イマーシブ翻訳のフロートボールでウェブページの翻訳問題を素早く報告できるようになりました。ウェブページを翻訳した後、問題がある場合は、フロートボールの右側にある [Feedback] ボタンをクリックし、問題の説明を記入すると、できるだけ早く対応します。
- Epub ファイルがリッチテキスト翻訳をサポートしました（各段落の元のテキストの形式を保持します。リンク、太字など）。
...
- YouTube の自動生成された英語字幕に AI インテリジェント文分割アルゴリズムを採用しました [Pro Available]
...
- YouTube のバイリンガル字幕がインテリジェント文分割（ベータ）をサポートしました（[Settings] - [Video Subtitles] で YouTube 字幕のイマーシブ翻訳を手動で有効にした場合のみ、元のビデオ字幕が自動生成された英語字幕である場合）。
...
- 新機能：漫画翻訳（ベータ版）、サポートされている漫画サイトでは、ウェブページのクイック翻訳フローティングボールの下に漫画翻訳ボタンが表示されます。クリックすると漫画翻訳が有効になります。この機能はProメンバー向け（毎月500ページ、追加パック購入可能）で、現在以下のサイトをサポートしています：
...
- YouTubeがフルスクリーンのときにフォントサイズが比例して拡大縮小されない問題を修正しました[#1681](https://github.com/immersive-translate/immersive-translate/issues/1681)
...
- AI翻訳サービスはモデルリストをカスタマイズできるようになり、例えば[OpenAI]では、システムには最も一般的に使用されるモデルがいくつか組み込まれています。モデルのドロップダウンリストをクリックすると、最後に表示される項目は[Set More Models]で、設定後、自動的に記憶され、ユーザーが異なるカスタムモデルをテストするのに便利です。
...
- 翻訳サービスは別のタブで明確に整理され、すべての利用可能な翻訳サービスの包括的な概要を提供します。さらに、ユーザーは表示する翻訳サービスをカスタマイズする柔軟性があります。デフォルトでは、限られた翻訳サービスのみが表示されますが、[More Services](https://dash.immersivetranslate.com/#services)セクションで表示設定を調整できます。
...
- OpenAI翻訳結果における中国語と英語の間のスペースのサポート
- パネルスイッチが現在のページにのみ影響を与えるように翻訳ボタンの表示を修正
...
- YouTubeのフルスクリーン再生のための没入型iOSアプリサポートを修正
...
- 設定ページ -> [基本設定] -> [翻訳スタイル]で翻訳のフォントを指定できるように
- モバイルデバイスのフローティングボールパネルに[指定段落の翻訳]のショートカット設定を追加
- `Ctrl+C`を`Ctrl`と誤認しないように、Ctrl上のマウスホバー検出の感度を最適化
...
- **ポップアップパネルUIアップグレード:** 新しいデザインは使いやすさと理解しやすさを向上させることを目的としています。このアップデートには以下が含まれます:
...
- **YouTubeバイリンガル字幕**: YouTube動画再生ウィンドウにバイリンガル字幕を有効または非表示にするためのクイックアクセスを追加（この機能はオフにできます）。

- **Deepl言語サポート**: ポルトガル語（ブラジル）のサポートを追加。

- **新規ユーザーガイド**: 新規ユーザーがターゲット言語以外のページを初めて開くと、浮遊するボールのヘルプガイドバブルが表示されます。
...
- 拡張PDFダウンロード問題を修正。
- [Arxiv](https://arxiv.org/abs/1910.06709)論文サイトの右側にクイック翻訳PDFポータルを追加し、クリーンなHTMLページに移動（いくつかの論文のみサポート、元の著者がソースコードを提出する必要があるため、約50%の論文がこのポータルを表示）。
...
- 一部のプラットフォームでマウスがサポートされているかどうかを認識できない問題を解決。
  - 一部のデスクトップブラウザはタッチスクリーンとマウスの両方をサポートしており、Immersive Translateは技術的にそのようなプラットフォームがマウスをサポートしているかどうかを検出できないため、[マウスホバー]設定に[マウスサポートを強制的に有効にする]オプションを追加。
...
- Youtube字幕がDual subtitleプラグインと競合する問題を修正（Youtube Dualプラグインが検出された場合、競合を避けるためにYoutube字幕翻訳のImmersive Translateは有効になりません）
...
- 新しいユーザー向けに頻繁に使用される英語のウェブサイトの翻訳を自動的に有効化
...
- 検索強化をサポートし、有効化すると、Google/Google Newsで中国語で検索すると、右側の列に対応する英語キーワードの検索結果が自動的に表示されます。デフォルトで有効です。
...
- 特定の言語に翻訳を指定することもできます：/ja Hello World、その後スペースバーを3回クリックすると、段落が日本語に翻訳されます
...
- 修正: openaiの翻訳問題を繁体字中国語で

## 0.5.15

- 最適化: 翻訳をトリガーする段落の最小文字数を4文字に変更し、混乱を減らし、サイトのナビゲーションや終了エリアの翻訳を避けるために他の機能を使用。
- 修正: Githubの詳細が展開後に翻訳されない問題。

## 0.5.14

- 修正: 一部のウェブページで画像がコピー後に大きくなる問題
- 修正: mediumのコメントセクションが翻訳されない問題
- 修正: 一部のページで画像が誤ってコピーされる問題
...
- 修正: 一部のケースで段落の改行が発生する問題
- 修正: Tampermonkeyのトグルが翻訳のみを表示する
...
- バージョン: 今後、マイナーバージョン番号を毎月変更します。例えば、3月にはバージョンが0.3.0から始まり、4月にはバージョン番号が0.4.0から始まり、次の4月にはバージョン番号が1.4.0になります。これは、拡張機能がセマンティクスに従う意味がないため、時間の法則に従ってバージョン番号を標準化することが開発の更新を続ける動機となり、ユーザーが問題をより簡単に見つけることができるためです。
- 機能: Firefox用のダークアイコンをサポート

## 0.2.86

- Open AI に対するリクエストごとに最大テキスト長オプションを追加
- 修正: ebook 識別子の重複
- 機能: txt ウェブページ翻訳のサポート

## 0.2.85

- 修正: 一部の epub ファイルが見つからない問題。
...
- 機能: 新しいテーマ「Solid Border」を追加 (新しいテーマ、実線の境界線)
...
- 修正: Firefox モバイルの翻訳サービスネットワーク問題の検証。
...
- 機能: 翻訳サービスとしての Open AI のサポート。
- 機能: options.html での翻訳サービスの検証サポート
- 機能: 一部のサイトがメインフレームとして body を使用していないため、カスタムメインフレームのサポート
...
- Chrome ストアの拒否のためのタブ権限を削除
- ページ全体を翻訳する際にフッターが無視される問題を修正
- about ページにメモを追加
- ビルドイン設定からのカスタム URL のサポート
...
- タッチデバイスで三本指を使って翻訳ページのトグルをトリガーする機能をサポート
- Userscript のポップアップアイコンを非表示にする機能をサポート
...
- すべてのサイトで段落言語を検出する機能を有効化

## 0.0.35

- YouTube の次のページに移動する機能を修正
- YouTube 検索ページをサポート
- オプションの高度なスイッチを修正
- img タグ、hidden タグを修正
...
- 翻訳が必要なページ領域を設定するサポートを追加し、より多くの領域を翻訳できるようにします。
