...
- تمت الإضافة: إدارة المصطلحات AI (تدعم فقط خدمات الترجمة AI)
...
- تم الإصلاح: مشكلة عدم عمل تمكين الترجمة الثنائية المؤقتة على YouTube.
...
- تم التحسين: يتم إيقاف الترجمة النصية الغنية افتراضيًا في وضع الترجمة الثنائية لتحسين جودة الترجمة.
...
- تم الإضافة: دعم **ترجمة العناوين الفرعية** في الاجتماعات الفورية عبر "الكرة العائمة"، متاح على Zoom وGoogle Meet وMicrosoft Teams.
...
- تم الإصلاح: مشكلة ترجمة المسافات في النصوص الغنية [#2175](https://github.com/immersive-translate/immersive-translate/issues/2175)
...
- خدمة الترجمة DeepL تدعم الآن رسميًا الصينية التقليدية كلغة هدف (سابقًا، كانت الترجمة إلى الصينية التقليدية مع DeepL تتطلب عملية تحويل إضافية من الصينية المبسطة إلى الصينية التقليدية بواسطة طرف ثالث).
...
- تم تمكين نسخ العناوين الفرعية على YouTube
...
- تدعم الآن العناوين الفرعية ثنائية اللغة على YouTube تقسيم الجمل الذكي (نسخة تجريبية) (فقط عند تفعيل الترجمة الغامرة للعناوين الفرعية على YouTube يدويًا في [الإعدادات] - [العناوين الفرعية للفيديو]، والعناوين الفرعية الأصلية للفيديو هي عناوين فرعية إنجليزية تم إنشاؤها تلقائيًا)
- تم إضافة خدمة الترجمة Tencent [【نموذج Hunyuan الكبير】](https://immersivetranslate.com/docs/services/tencent-hunyuan/)

- إصلاح مشكلات تنسيق النص في ترجمات القصص المصورة للغات ذات النصوص اللاتينية.
...
- الترجمة الثنائية لـ Udemy متوافقة مع العرض على الجوال
...
- تحسين كلمات موجهة Gemini
- دعم إعداد عرض الترجمة للترجمات الفيديو بلغتين/الأصلية والترجمة
...
- تم إصلاح المشكلة التي كانت فيها ترقية إصدار ملحق Safari دائمًا تعرض صفحة نجاح التثبيت
- أضافت Microsoft دعمًا للفيتنامية
- تم إصلاح المشكلة التي لم تعرض فيها الترجمات المترجمة على موقع edx
...
- تم إصلاح المشكلة التي كانت فيها ترجمة حقول الإدخال التي تحتوي على عناوين URL تترجم أجزاء فقط من المحتوى.

## 1.1.2

- إصلاح: المشكلة التي لم يكن فيها تبديل خدمات الترجمة يؤثر عندما لم يتم ترجمة الصفحة بعد.
...
- تم إصلاح مشكلة اختصار الترجمات الثنائية لـ YouTube التي كانت تحتوي على نص إنجليزي طويل جدًا.
...
- **ترجمات YouTube الثنائية**: تمت إضافة وصول سريع في نافذة تشغيل فيديو YouTube لتمكين أو إخفاء الترجمات الثنائية (يمكن إيقاف تشغيل هذه الميزة).
...
- **تحسين واجهة المستخدم**: تم إعادة تصميم واجهة المستخدم لمطالبات خطأ ترجمة الصفحة لتكون أسهل في الفهم. عندما تكون هناك العديد من الأخطاء، ستظهر نافذة منبثقة لتنبيه المستخدم بنشاط.
...
- تم تحسين: عند استخدام تمرير الماوس، المشكلة التي تصبح فيها فقدان التركيز على الصفحة ثم إعادة التحفيز غير فعالة.
- تم إصلاح: المشكلة التي لا يزال فيها التخزين المؤقت موجودًا بعد تعديل النموذج/النموذج في OpenAI.

## 0.12.12

- تم تحديث: تحسين اللوحة المنبثقة، وإزالة بعض الخيارات للموقع الحالي.
- تم تحسين: تحسين عملية دمج الترجمة اليدوية.
- تم تحسين: ضبط اللغة المستهدفة تلقائيًا بناءً على لغة المتصفح.
- تم إضافة: دعم الترجمة الثنائية اللغة لمنصة التعلم [ArtStation](https://www.artstation.com/learning) و [ZDF](https://www.zdf.de/).
...
- على macOS، تم تحسين اسم عرض اختصار اللوحة.
...
- يتم الآن إخفاء الكرة العائمة افتراضيًا عندما يكون الفيديو في وضع ملء الشاشة.
...
- إصلاح نقرات الكرة العائمة المتذبذبة
...
- إصلاح: أفضل لتقسيم الفقرة الطويلة، 1.5 مليار، 25.5%، Mr. لن يعتبر كحد
...
- ميزة: دعم OpenAI لنقطة نهاية apiUrl مخصصة لإعدادات المطور.
...
- دعم تحميل الموضوع
- إصلاح getpocket.com
- إصلاح تذييل الجانب لمنطقة الجسم
...
- إصلاح الترجمة الأساسية لملف PDF
...
- إصلاح stay.app API المتصفح للبرنامج النصي للمستخدم
...
- إصلاح النسخة المدمجة من Reddit
