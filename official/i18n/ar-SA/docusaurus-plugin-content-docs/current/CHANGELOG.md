...
- تم الإصلاح: مشكلة عدم عمل مفتاح الاختصار لترجمة التحويم على الأجهزة التي تدعم كل من الفأرة واللمس.
...
- تم الإصلاح: مشكلة خدمة الترجمة AI التي تخرج بشكل غير صحيح الصينية المبسطة عند ترجمة الصينية التقليدية.
- تم الإصلاح: مشكلة عدم عمل الترجمة التحويم في وضع "الترجمة أولاً، يتبعها النص الأصلي".
- تم الإصلاح: مشاكل التوافق مع وضع "الترجمة فقط" عند استخدامه مع الإضافات القرائية مثل Reader View.
...
- تم التحسين: إصدار Tampermonkey يدعم الآن ترجمة العناوين الفرعية على YouTube.
...
- تم الإصلاح: مشكلة تحميل وظيفة التحويم للفأرة لخدمة الترجمة الخاطئة في ظل ظروف معينة.
...
- تم الإصلاح: مشكلة في مفاتيح الاختصار لـ **Enhanced input** التي لا تعمل
...
- بعد تعديل موضع العناوين الفرعية الثنائية لـ YouTube، سيتذكر النظام الموضع الجديد تلقائيًا.
...
- تحسين العناوين الفرعية الثنائية للنسخة الإنجليزية من iQIYI (iq.com)
- توفير المزيد من الأوراق العلمية من arXiv مع تحسين تخطيط الترجمة الثنائية
- بسبب قيود موقع Youtube، لم يعد يدعم سكربت Chrome Tampermonkey العناوين الفرعية الثنائية لـ Youtube. يرجى استخدام [نسخة الإضافة](https://immersivetranslate.com/).
...
- تحسين الكشف عن ترجمات الأوراق العلمية من arXiv
...
- اعتماد خوارزمية تقسيم الجمل الذكية بالذكاء الاصطناعي للعناوين الفرعية الإنجليزية المولدة تلقائيًا على YouTube [متاح للمحترفين]
...
- تدعم العناوين الفرعية الثنائية لـ YouTube الآن تقسيم الجمل الذكي (نسخة تجريبية) (فقط عند تفعيل الترجمة الغامرة للعناوين الفرعية لـ YouTube يدويًا في [الإعدادات] - [العناوين الفرعية للفيديو]، والعناوين الفرعية للفيديو الأصلي هي عناوين فرعية إنجليزية مولدة تلقائيًا)
...
- تم إصلاح مشكلة عدم القدرة على اختيار خبراء الذكاء الاصطناعي في الخدمات المخصصة.
...
- قائمة نماذج الذكاء الاصطناعي المخصصة تدعم صيغة `-all`، والتي يمكن أن تحذف جميع النماذج المحددة مسبقًا.
...
- تم إصلاح المشكلة التي كانت تمنع كرة الترجمة العائمة من الترجمة تحت iOS 18.
...
- يدعم أعضاء Pro خدمات الترجمة Claude و Gemini (Beta)
...
- يمكن لخدمات الترجمة بالذكاء الاصطناعي الآن تخصيص قائمة النماذج، مثل [OpenAI]، النظام يحتوي فقط على بعض النماذج الأكثر استخدامًا مدمجة. عند النقر على قائمة النماذج المنسدلة، العنصر الأخير الذي تراه هو [Set More Models]، بعد الإعداد، سيتم تذكره تلقائيًا لراحة المستخدمين في اختبار النماذج المخصصة المختلفة.
- تحسين التناقض في تخطيط الترجمات في بعض الحالات.
- إضافة زر إعادة تعيين لنمط الترجمة في YouTube، والذي يمكنه بسرعة استعادة النمط الافتراضي.
...
- تم إصلاح المشكلة التي كانت تجعل الترجمة الثنائية في YouTube لا يمكن تعديلها لأعلى ولأسفل في وضع الشاشة الكاملة [#1654](https://github.com/immersive-translate/immersive-translate/issues/1654)
- دعم الترجمة الثنائية لـ [VK Video](https://vk.com/video)
- دعم إعدادات التفعيل المستقلة للترجمة الثنائية في فيديوهات YouTube (ممكّن افتراضيًا للمستخدمين الجدد)
...
- تم إصلاح اختفاء الترجمة في وضع الشاشة الكاملة على iOS Safari
- تحسين سرعة الترجمة للترجمة في YouTube
...
- منصة الترجمة للفيديو تدعم [جامعة ساوثهامبتون](https://southampton.cloud.panopto.eu)
- الترجمة الثنائية لـ Udemy متوافقة مع العرض على الأجهزة المحمولة
...
- دعم إعداد الترجمة الثنائية/الأصلية والعرض للترجمة في الترجمة للفيديو
...
- تم تحسين حساسية اكتشاف تحريك الماوس فوق Ctrl لتجنب الخلط بين `Ctrl+C` و `Ctrl`
...
- تم إصلاح مشكلة ترجمة حقول الإدخال التي تحتوي على عناوين URL حيث تم ترجمة أجزاء فقط من المحتوى.
...
- تم إصلاح مشكلة اختصار الترجمات الثنائية ليوتيوب حيث كان النص الإنجليزي طويلًا بشكل مفرط.
...
- **دليل المستخدم الجديد**: عندما يفتح المستخدمون الجدد صفحة بلغة غير مستهدفة لأول مرة، يتم عرض فقاعة دليل مساعدة الكرة العائمة.

### التحسينات والإصلاحات

- **تحسين واجهة المستخدم**: تم إعادة تصميم واجهة المستخدم لتنبيهات خطأ ترجمة الصفحة لجعلها أسهل في الفهم. عندما تكون هناك العديد من الأخطاء، ستظهر نافذة منبثقة تنبه المستخدم بنشاط.

- **إصلاحات الأخطاء**:

- تم إصلاح المشكلة التي كانت تحدث عند فشل ترجمة mouseover عندما تفقد الصفحة التركيز.
- تم إصلاح المشكلة التي كانت تحدث عندما لم يتم ترجمة أقل من 3 أحرف في ميزة تحسين صندوق الإدخال.
- تم إصلاح المشكلة التي كانت تحدث عندما لم يتم ترجمة بعض الدلائل أثناء إنتاج Epubs ثنائية اللغة.

- **إزالة الميزة**: تم إزالة ميزة تحسين المعلومات ثنائية اللغة (عرض نتائج البحث باللغة الإنجليزية على صفحات بحث Google في نفس الوقت).

### تحديثات أخرى

- **تحديث تكوين openAI**: الآن يدعم ضبط عدد التكوينات في الثانية بالأرقام العشرية، مثل 0.5، مما يعني طلب واحد كل ثانيتين.
...
- تحسين: عند استخدام mouse hover، تم حل المشكلة التي كانت تحدث عندما تفقد الصفحة التركيز ثم تعيد التشغيل تصبح غير فعالة.
- تم إصلاح: المشكلة التي كانت تحدث عندما لا يزال الكاش موجودًا بعد تعديل prompt/model في OpenAI.
...
- إضافة: دعم الترجمة الثنائية للمنصة التعليمية [ArtStation](https://www.artstation.com/learning) ومنصة [ZDF](https://www.zdf.de/).
- تم إصلاح: حل المشكلة التي كانت تحدث عندما لم يتم ترجمة العناوين في صفحة قائمة jstor [#1268](https://github.com/immersive-translate/immersive-translate/issues/1268).
- تم إصلاح: تم إصلاح المشكلة التي كانت تحدث عندما يختفي جزء من المحتوى على Hacknews [#1264](https://github.com/immersive-translate/immersive-translate/issues/1264).

## 0.12.11

- تم إضافة دعم الترجمة الثنائية للمنصات [HBO Max](https://play.max.com/)، [BBC](https://www.bbc.com/)، [Disney+](https://www.disneyplus.com)، [ARD Mediathek](https://www.ardmediathek.de/)، [ITV](https://www.itv.com/)، و [Domestika](https://www.domestika.org).

## 0.12.10

- تم إصلاح مشكلة التفويض لنطاق Gemini تحت سكربت Tampermonkey.
- دعم الترجمة الفورية للترجمات في Twitter Space.
- بالنسبة للإصدارات القديمة من سكربت Tampermonkey، تم إضافة تنبيه تحديث الآن إلى صفحة الإعدادات.

## 0.12.9

- تم إضافة دعم لترجمة Gemini.
- تم إصلاح المشكلة التي كانت تحدث عندما لم تستجب الترجمات في الوقت المناسب عند التمرير إلى منتصف الصفحة في بعض الحالات.
- تم إصلاح الخطأ الذي كان يحدث عندما تسبب تبديل الترجمات على بعض مواقع الفيديو في عرض الترجمة بشكل متكرر.
- تم إصلاح المشكلة التي كانت تحدث عندما كان الماوس يحوم لمواصلة الترجمة دون الضغط على مفتاح الاختصار في بعض الحالات.
- على macOS، تم تحسين اسم العرض لاختصار اللوحة.

## 0.12.8

- إصلاح عدم عرض ترجمات الفيديو الأصلية عندما يتم تعيين "الموقع الحالي لا يترجم أبدًا".
...
- تم إضافة ترجمات ثنائية لدعم منصات [TED](https://www.ted.com)، [Frontend Masters](https://frontendmasters.com/)، [edx](https://www.edx.org/)، [CodeWithChris](https://www.edx.org/)، [Skillshare](https://www.ted.com) https://learn.codewithchris.com/enrollments)، [Skillshare](https://www.skillshare.com/) Platforms
- الآن يتم إخفاء hoverball افتراضيًا عندما يكون الفيديو في وضع الشاشة الكاملة.
- إصلاح مشكلة النقر المتذبذب في لوحة عمل hoverball في Firefox.
- دعم التعاون تحت موقع pubmed.ncbi.nlm.nih.gov وإضافة scholarscope
- إصلاح مشكلة اهتزاز صفحة الترجمة في صندوق إدخال reddit

## 0.12.6

- إصلاح المشكلة التي كانت تحدث عندما لا تكون ترجمة YouTube/Web of Science حساسة عند تبديل التبويبات.
- الآن يدعم hoverball على الجوال عملية الضغط الطويل، والضغط القصير للترجمة، والضغط الطويل لفتح اللوحة.
- الآن سيتم ترجمة جدول المحتويات عند ترجمة الكتب الإلكترونية ثنائية اللغة.
...
- إصلاح مشكلة عدم عمل ترجمة Epub عند النقر على اللوحة
...
- يوفر الوصول السريع إلى النسخ الثنائية لجميع الأوراق على موقع [Arxiv](https://arxiv.org/abs/1910.06709).
- [دعم hoverball ليتم تثبيته على اليسار #1168](https://github.com/immersive-translate/immersive-translate/issues/1168)
...
- الآن يمكن للصفحات PDF عبر الإنترنت بدون امتداد .pdf الانتقال مباشرة إلى صفحة ترجمة PDF بالنقر على hoverball في الصفحة.
- إصلاح بعض مشاكل تحسين صندوق الإدخال تحت Safari
- تحسين كشف اللغة في Grease Monkey وSafari

## 0.11.6

- تم إضافة 11 لغة جديدة لواجهة الإضافة والموقع الرسمي، الآن اللغات المدعومة للواجهة تصل إلى 14، بما في ذلك الصينية المبسطة، الصينية التقليدية، الإنجليزية، اليابانية، الكورية، الروسية، الإسبانية، البرتغالية، الهندية، الإيطالية، الألمانية، الفرنسية، العربية، والفارسية.
...
- إصلاح عدم استجابة النقر على hoverball في PDF عبر الإنترنت
...
- تم حل المشكلة التي كانت تحدث عندما لم تتمكن بعض المنصات من التعرف على ما إذا كان الماوس مدعومًا أم لا
  - هناك بعض المتصفحات المكتبية التي تدعم كل من الشاشة اللمسية والماوس، وImmersive Translates غير قادر تقنيًا على اكتشاف ما إذا كانت هذه المنصات تدعم الماوس، لذا قمنا بإضافة الخيار [فرض تمكين دعم الماوس] في إعداد [Mouse Hover]
...
- تحسين Prompt الصينية لـ OpenAI، الآن أقل عرضة للهلوسة!
- تقليل طول Prompts OpenAI من 90 إلى 40، مما يوفر المزيد من حركة المرور.

## 0.11.0

- إصلاح نقرات hoverball المتذبذبة على الصفحة
...
- دعم إعداد أزرار الترجمة السريعة لـ hoverball (دعم لكل من PC/الجوال)
- تحسين حكم اللغة في Grease Monkey
- إصلاح ترجمة ملفات txt

## 0.10.7

- دعم mouse hover للضغط على Ctrl مرة أخرى لعرض النص الأصلي
- تجاهل mouse hover للغة التي لا تترجم أبدًا
...
- إصلاح تعارض ترجمات Youtube مع إضافة Dual subtitle (ترجمة ترجمات Youtube في Immersive Translate غير مفعلة عند اكتشاف إضافة Youtube Dual لتجنب التعارض)
...
- دعم نتائج الترجمة في تعزيز الإدخال ctrl+z تراجع
...
- يمكن للوحة الامتداد الآن الإشارة إلى الصفحات غير المدعومة (مثل الصفحات التي لا تحتوي على أذونات والصفحات غير HTML)
...
يمكن الآن للغة الافتراضية لترجمة صندوق الإدخال تجاهل المسافة، أي //Hello World يمكن ترجمتها أيضًا.
...
- إصلاح مشكلة: الكتب الإلكترونية ثنائية اللغة لا تحتوي على صور بعد التصدير
...
- تحسين: تم تعديل الحد الأدنى لعدد الأحرف في الفقرة التي تثير الترجمة إلى حد أدنى 4 أحرف لتقليل الالتباس، مع استخدام ميزات أخرى لتجنب ترجمة مناطق التنقل والنهاية في الموقع.
...
- إعادة بناء تحليل النواة للويب، مما يعني:
  ...
  - ترجمة صفحات الويب الكبيرة في ثوانٍ
    - على سبيل المثال،: https://pve.proxmox.com/pve-docs/pve-admin-guide.html، التي كانت تستغرق 30 ثانية من قبل، الآن تُترجم في ثوانٍ.
...
- تم إصلاح مشكلة الشاشة البيضاء بعد ترجمة المواقع التي تحتوي على الترطيب، مثل Next.js
...
- إصلاح: اختفاء النافذة المنبثقة لمستخدمي Firefox Userscript.
...
- واجهة المستخدم: استخدام نموذج حقل إعدادات OpenAI اختيار بدلاً من مربع إدخال النص.

## 0.4.5

- إصلاح: مشكلة امتداد Safari في iOS 15.0.
- إصلاح: اختصارات امتداد Safari في macOS
- إصلاح: سياسة أمان المحتوى للنافذة المنبثقة للامتداد، وجزئيًا لـ userscript.
...
- إصلاح: اكتشاف المسافات البيضاء لعلامات sup, sub.
...
- Fix: m.youtube.com
- Fix: واجهة نموذج الخيارات
- Fix: Open AI prompt
- Feat: دعم OpenAI لمفاتيح متعددة، استخدم `,` لفصلها.

## 0.2.80

- Feat: إضافة قائمة تمكين/تعطيل للنوافذ المنبثقة -> المزيد
- Fix: تعارض رسالة DingTalk

## 0.2.79

- Fix: Open AI لفقرة المسافة

## 0.2.78

- Feat: دعم OpenAI CHATGPT 3.5 (يدعم واجهة OpenAI ChatGPT 3.5)
- Feat: إضافة موضوع جديد Solid Border (إضافة موضوع جديد، إطار خطي)

## 0.2.77

- Fix: خطأ في علامات الكود المتعددة.[#178](https://github.com/immersive-translate/immersive-translate/issues/178)
...
- Feat: دعم عدد النصوص الفورية المخصصة لخدمات الترجمة المختلفة.

## 0.2.74

- Feat: دعم Tencent (alpha)
- Fix: ترجمة openai
- Fix: التحقق من العلامات غير المعروفة داخل السطر

## 0.2.73

- Feat: دعم موضوع الترجمة الرمادي
- Fix: صفحة Github Trending

## 0.2.72

- Fix: مشكلة التحقق من خدمة الترجمة على شبكة فايرفوكس المحمول.
...
- Feat: دعم الإطار الرئيسي المخصص لأن بعض المواقع لا تستخدم الجسم كإطار رئيسي

## 0.2.68

- دعم Caiyun (Alpha)
- Fix: علامات الكتل غير المعروفة

## 0.2.67

- Feat: إضافة `<all>` للغات الترجمة الدائمة، بحيث يمكنك الآن استخدامها لترجمة جميع اللغات باستثناء اللغة المستهدفة، وعدم ترجمة اللغات
- Fix: السماح بتكوين API مخصص لجوجل
- Better: دعم Deepl Free
- Fix: استخدام ذاكرة عالية للبرامج النصية والإضافات (عن طريق إزالة opencc zh-CN إلى zh-TW، بدلاً من ذلك مع ترجمة جوجل)
- Fix: Relingo [#159] (https://github.com/immersive-translate/immersive-translate/issues/159)
- Fix: إعداد ترجمة Azure ولكن لا يزال يظهر (يحتاج إلى إعداد)
...
- Feat: دعم خدمة ترجمة Azure
- Feat: دعم خدمة ترجمة Papago
- Fix: مزامنة جوجل درايف الأصلية على فايرفوكس أندرويد.
- Fix: تغيير الشفافية من 0.4 إلى 0.618 [#147] (https://github.com/immersive-translate/immersive-translate/pull/147)
- Fix: نصائح اختصارات النوافذ المنبثقة
...
- Fix: البرنامج النصي للمستخدم كبير جدًا بالنسبة لـ greasy fork
...
- Fix: عنصر كتلة علامات غير معروفة
- Fix: عنصر translate=no يتجاوز
- Fix: تطابق URL مع متعددة \*
...
- Fix: تكرار الترجمة للعناصر المضمنة مع عنصر الكود.
- Fix: التحقق من العلامات غير المعروفة داخل السطر/الكتلة
...
- Fix: خطأ مزامنة جوجل درايف للبرنامج النصي للمستخدم.
...
- CI: تقديم الشيفرة المصدرية إلى فايرفوكس
...
- Fix: مشكلة تحميل جوجل درايف (حقًا) [#81](https://github.com/immersive-translate/immersive-translate/issues/81)

## 0.2.49

- إزالة اختصارات alt+d، alt+s لأنها قد تتعارض مع الاختصارات الأصلية.
- Fix: مشكلة تحميل جوجل درايف [#81](https://github.com/immersive-translate/immersive-translate/issues/81)

## 0.2.48

- Better: للسرعة، بإضافة minLength إلى 50 لاكتشاف اللغة.
- Fix: التحقق من صحة رمز جوجل درايف.
...
- Fix: ترجمة caiyun لغة المصدر غير معروفة

## 0.2.44

- Fix: تبديل قناع البرنامج النصي للمستخدم
- Fix: منطق تبديل القناع

## 0.2.43

- Fix: تبديل قناع البرنامج النصي للمستخدم مع حدث اللمس.
...
- Fix: مشكلة مزامنة السحابة للبرنامج النصي للمستخدم
- نقل صفحة الخيارات المتقدمة إلى القائمة اليسرى.
- إضافة منطق إعادة المحاولة لخدمة الترجمة

## 0.2.41

- Fix: ترجمة niu للبرنامج النصي للمستخدم
- Fix: ترجمة xhtml

## 0.2.40

- Fix: عرض ميزة بيتا
- Fix: إعداد الصفحة المنبثقة في صفحة علامة تبويب جديدة
...
- دعم المزامنة إلى جوجل درايف

## 0.2.35

- Fix: تجاهل علامة rb، rt اليابانية.
- Better: لواجهة المستخدم المنبثقة المزيد
- Better: لنصائح البرنامج النصي للمستخدم السيئة
- إضافة مساهمة إلى صفحة حول
- Fix: ترجمة volc لاكتشاف اللغة التلقائي

## 0.2.34

- Fix: سرعة اللغة المتعددة
...
- Fix: اختيار نافذة منبثقة لخدمة غير مكونة، انتقل إلى صفحة الخيارات.
- Fix: البقاء على فتح الإعدادات.
- Fix: سرعة اكتشاف اللغة المتعددة

## 0.2.31

- Fix: حقن CSS للإطار الديناميكي

## 0.2.30

- دعم ترجمة iframe المضمنة للبرنامج النصي للمستخدم.
...
- إزالة المطابقة غير القياسية للبرنامج النصي للمستخدم

## 0.2.27

- دعم ترجمة iframe المضمنة. (فقط للإضافة، غير متاح للبرنامج النصي للمستخدم)
- Fix: ترجمة اللغة المتعددة
...
- Fix: برنامج stay.app API للمتصفح
- Fix: صفحة خيارات stay.app

## 0.2.23

- Fix: ترجمة صفحة اللغة المتعددة

## 0.2.22

- Fix: بناء البرنامج النصي للمستخدم
...
- دعم سحب النافذة المنبثقة للبرنامج النصي للمستخدم
- دعم ثلاث أصابع على جهاز اللمس لتفعيل تبديل صفحات الترجمة
- دعم إخفاء أيقونة النافذة المنبثقة للبرنامج النصي للمستخدم.
...
- إصلاح بعض علامات span هي عنصر block
