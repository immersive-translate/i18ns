...
- تمت الإضافة: دعم Deepseek,Gemini,Cluade لتبديل السياق.
...
- إصلاح: ترجمة الشريط الجانبي غير الصالحة في الإعدادات المتقدمة.
- إصلاح: مشكلة عرض الترجمة المخصصة deepseek.

## 1.14.11 Release (2025-02-18)

- إصلاح: خطأ DeepSeek custom API Key `401: Authentication Fails`.
...
- تحسين: الوصول إلى واجهة اختبار صفحة الإعدادات عبر الإنترنت.
- تحسين: تحسين تجربة المستخدم لميزة معاينة مقارنة السياق.
- تحسين: منطق الحكم على وضع اللمس والفأرة.

## 1.14.8 Release (2025-02-10)

- إصلاح: مشكلة عدم ترجمة الملفات الطويلة في PDF-Pro تلقائيًا.
...
- تحسين: إصدار Tampermonkey يدعم الآن ترجمة ترجمات YouTube.
...
- تحسين: بالنسبة لتويتر، يتم تعيين ترجمات المحتوى متعدد الأسطر لعدم الالتفاف افتراضيًا. سيتم الالتفاف فقط عندما يتجاوز المحتوى 10 أسطر أو 1000 حرف. يمكن تمكين الالتفاف من خلال الإعدادات **Advanced Settings** -> **Enable automatic line wrapping for long paragraphs**.

## 1.12.8 (2025-01-03)

- إصلاح: مشكلة عدم عرض صفحة إعدادات iOS 18.3 بشكل صحيح.
...
- تحسين: الفيديوهات بدون ترجمات ستعرض أيقونة الترجمة وتوفر سبب عدم إمكانية الترجمة.
...
- تمت الإضافة: دعم **Subtitles Translation** للاجتماعات في الوقت الحقيقي عبر "الكرة العائمة"، متاح على Zoom و Google Meet و Microsoft Teams.
...
- تمت الإضافة: وضع دعم الفأرة الإجباري (قم بتمكين هذه الميزة فقط إذا كانت وظيفة التحويم بالماوس غير متاحة على أجهزة التابلت) **Settings** -> **Advanced Settings** -> **Forece Enable Mouse Support**
...
- تحسين: معالجة المشاكل التي قد لا يعمل فيها زر الترجمة أثناء ترجمة PDF
...
- يدعم Google Meet الآن الترجمة الثنائية للاجتماعات الحية: الآن، يمكنك الاستمتاع بميزة الترجمة الثنائية في اجتماعات Google Meet. ببساطة افتح رابط الاجتماع، فعّل الترجمة الثنائية في لوحة الترجمة الغامرة، ثم قم بتحديث الصفحة لتجربتها.
...
- يمكنك الآن الإبلاغ بسرعة عن مشاكل الترجمة في الكرة العائمة للترجمة الغامرة. بعد ترجمة صفحة ويب، إذا كانت هناك أي مشاكل، يمكنك النقر على زر [التغذية الراجعة] على الجانب الأيمن من الكرة العائمة، وملء وصف المشكلة، وسنتعامل معها في أقرب وقت ممكن.
...
- تدعم الترجمة الثنائية في YouTube الآن تقسيم الجمل الذكي (بيتا) (فقط عند تمكين الترجمة الغامرة يدويًا لترجمات YouTube في [الإعدادات] - [ترجمات الفيديو]، والترجمات الأصلية للفيديو هي ترجمات إنجليزية تلقائية)
...
- إضافة زر إعادة تعيين لنمط ترجمات يوتيوب، والذي يمكنه استعادة النمط الافتراضي بسرعة.
...
- تم تحسين المشكلة التي كانت تجعل الترجمات الثنائية اللغة الغامرة لا تظهر عندما يضبط المستخدمون لغة الترجمة إلى الصينية على موقع يوتيوب.
...
- دعم ترجمة ترجمات الفيديو لـ [Unreal Sensei](https://www.unrealsenseiacademy.com/)
...
- **دعم الجوال لترجمة الفقرات المحددة:** يدعم الإصدار المحمول الآن ترجمة الفقرات المحددة وأضيفت مجموعة متنوعة من العمليات المختصرة، بما في ذلك السحب لليسار، السحب لليمين، النقر المزدوج، النقر الثلاثي، وإيماءات اللمس المتعددة الأصابع. هذه ليست مفعلة افتراضيًا وتتطلب من المستخدم اختيار الإيماءة المحفزة في صفحة الإعدادات تحت [تحريك الماوس].
...
- تم إصلاح المشكلة التي لم يكن للقائمة المنسدلة في الوضع الداكن على Windows لون.
...
- **تحسين واجهة المستخدم**: إعادة تصميم واجهة المستخدم لتنبيهات خطأ ترجمة الصفحة لجعلها أسهل في الفهم. عندما يكون هناك العديد من الأخطاء، ستظهر نافذة منبثقة تنبه المستخدم بنشاط.

- **إصلاحات الأخطاء**:

  - تم إصلاح المشكلة التي كانت تفشل فيها ترجمة تحريك الماوس عندما تفقد الصفحة التركيز.
  - تم إصلاح المشكلة التي لم تكن تترجم فيها أقل من 3 أحرف في ميزة تحسين مربع الإدخال.
...
- تم إصلاح: مشكلة خياطة ترجمة الفقرات الطويلة لـ OpenAI في بعض الحالات. [#1276](https://github.com/immersive-translate/immersive-translate/issues/1276)
- تحسين: عند استخدام تحريك الماوس، المشكلة التي كانت تفقد فيها الصفحة التركيز ثم تصبح غير فعالة عند إعادة التحريك.
- تم إصلاح: المشكلة التي كانت لا تزال موجودة بعد تعديل المطالبة/النموذج في OpenAI.
...
- تم إضافة تنبيه تحديث لصفحة الإعدادات للإصدارات القديمة من سكريبت Tampermonkey.
...
- تحسين حجم ملف الإضافة بشكل كبير، حيث تم تقليله إلى النصف مقارنة بالأصل، مما يجعل التنزيل والتحديث أسرع.
...
- إصلاح عدم استجابة النقر على الكرة العائمة للملفات الإلكترونية عبر الإنترنت
...
- تم حل المشكلة التي كانت تمنع بعض المنصات من التعرف على ما إذا كان الماوس مدعومًا أم لا
...
- تجاهل تحويم الماوس للغة التي لا تترجم أبدًا
...
- إصلاح مشكلة عدم ترجمة النقر على المزيد من النصوص في ins
...
- يسمح لك بتعيين الفاصل الزمني بين تفعيل مجموعة تحسين الإدخال
- إصلاح بعض الأخطاء
...
- يدعم مترجم Arigato إعداد الحقول ومعرف المصطلحات
...
- إصلاح عدم ترجمة epub لبعض الكتب
...
- تحسين: تم تعديل الحد الأدنى لعدد الأحرف في الفقرة التي تفعّل الترجمة إلى حد أدنى 4 أحرف لتقليل الارتباك، مع استخدام ميزات أخرى لتجنب ترجمة مناطق التنقل والمناطق النهائية للموقع.
...
- إصلاح مشكلة عدم التعرف على بعض علامات الكود بشكل صحيح.
...
- إصلاح المشكلة التي تتطلب تحديث الصفحة لتفعيل تعديل اختصار تمرير الماوس
...
- إصلاح مشكلة الشاشة البيضاء بعد ترجمة المواقع مع الترطيب، مثل Next.js

- إصلاح المشكلة التي تتطلب تحديث الصفحة لتفعيل تغيير اختصار تمرير الماوس
...
- إصلاح: مشكلة منطقة تمرير الماوس.
...
- إصلاح: مستمع اختصار تمرير الماوس.
- إصلاح: ترجمة مستند PDF
- ميزة: إضافة صفحة دليل عميل جديدة

## 0.5.2

- إصلاح: إعدادات اختصارات تمرير الماوس في userscript
...
- ميزة: ترجمة الفقرة الحالية عند تمرير الماوس.
...
- واجهة المستخدم: استخدام نموذج تحديد بدلاً من مربع إدخال النص في إعدادات OpenAI.
...
- إصلاح: postMessage عند كسر الصفحة في بعض المواقع، الآن سنفعل ذلك فقط عندما نترجم الصفحات حقًا
...
- إصلاح: حالة إدخال خدمات الترجمة في واجهة المستخدم للخيارات لم تتغير.
...
- إصلاح: تحريك المؤشر في نموذج الخيارات.
...
- إصلاح: خطأ متعدد في وسم الكود.[#178](https://github.com/immersive-translate/immersive-translate/issues/178)
...
- إصلاح: ترجمة openai
- إصلاح: التحقق من الوسوم غير المعروفة داخل النص
...
- إصلاح: مشكلة التحقق من خدمة الترجمة على فايرفوكس موبايل.
...
- دعم Caiyun (Alpha)
- إصلاح وسوم الكتل غير المعروفة

## 0.2.67

- ميزة: إضافة `<all>` للغات التي تُترجم دائمًا، بحيث يمكنك الآن استخدامها لترجمة جميع اللغات باستثناء اللغة المستهدفة، وعدم ترجمة اللغات
- إصلاح: السماح بتكوين Google API مخصص
- تحسين: دعم Deepl Free
- إصلاح: استخدام ذاكرة عالية للسكريبتات والمكونات الإضافية (عن طريق إزالة opencc من zh-CN إلى zh-TW، بدلاً من ذلك باستخدام Google translate)
...
- إصلاح: السكريبت كبير جدًا لـ greasy fork
...
- إصلاح: عنصر كتلة وسوم غير معروفة
...
- إصلاح التحقق من الوسوم غير المعروفة داخل/خارج النص
...
- إصلاح خطأ مزامنة Google Drive للسكريبت.
...
- إصلاح تبديل قناع السكريبت
- إصلاح منطق تبديل القناع

## 0.2.43

- إصلاح تبديل قناع السكريبت مع حدث اللمس.
...
- إصلاح مشكلة مزامنة السحابة للسكريبت
- نقل صفحة الخيارات المتقدمة إلى القائمة اليسرى.
- إضافة منطق إعادة المحاولة لخدمة الترجمة

## 0.2.41

- إصلاح ترجمة niu للسكريبت
- إصلاح ترجمة xhtml

## 0.2.40

- إصلاح عرض ميزة بيتا
- إصلاح إعداد النافذة المنبثقة في صفحة علامة تبويب جديدة
...
- إصلاح تجاهل وسوم rb، rt اليابانية.
- تحسين واجهة المستخدم المنبثقة أكثر
- تحسين نصائح السكريبت السيئة
...
- إصلاح اختيار النافذة المنبثقة لخدمة غير مكونة، الانتقال إلى صفحة الخيارات.
...
- إصلاح API متصفح السكريبت stay.app
...
- إصلاح الترجمة الذكية اليابانية لـ tencent
...
- إصلاح نسخة Reddit المحمولة المدمجة
...
- إصلاح علامة BR
- التعامل مع علامات block وفقًا للقواعد
...
- إصلاح الأحرف الخاصة في Wikipedia
