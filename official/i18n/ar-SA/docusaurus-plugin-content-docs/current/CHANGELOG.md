...
- تم الإصلاح: مشكلة في ترجمة التمرير التي لا تعمل في وضع "الترجمة أولاً، النص الأصلي يتبع".
...
- تم الإصلاح: مشكلة تحميل وظيفة التمرير للماوس خدمة الترجمة الخاطئة في ظل ظروف معينة.
...
- تمت الإضافة: دعم **ترجمة العناوين الفرعية** في الاجتماعات الفورية الآن عبر "الكرة العائمة"، متاح على Zoom وGoogle Meet وMicrosoft Teams.
...
- تمت الإضافة: عرض تحذير عندما تكون لغة المصدر واللغة الهدف هي نفسها
...
- تمت الإضافة: وضع Forece Enable Mouse Support (قم بتمكين هذه الميزة فقط إذا كانت وظيفة التحويم بالماوس غير متاحة على أجهزة التابلت) **الإعدادات** -> **الإعدادات المتقدمة** -> **Forece Enable Mouse Support**
...
- تم التحسين: معالجة المشكلات التي قد لا تعمل فيها زر الترجمة أثناء ترجمة PDF
...
- تمت إضافة خيار "الإبلاغ عن مشاكل الترجمة للصفحة الحالية" وخيار "تشغيل/إيقاف الكرة العائمة بسرعة" في خيارات المزيد في اللوحة.
...
- يمكنك الآن الإبلاغ بسرعة عن مشاكل الترجمة للصفحات في الكرة العائمة للترجمة الغامرة. بعد ترجمة صفحة ويب، إذا كانت هناك أي مشاكل، يمكنك النقر على زر [التغذية الراجعة] على الجانب الأيمن من الكرة العائمة، وملء وصف المشكلة، وسنتعامل معها في أقرب وقت ممكن.
...
- توفير المزيد من أوراق arXiv بتخطيط ترجمة ثنائي محسن
...
- تدعم ترجمات YouTube الثنائية الآن تقسيم الجمل الذكي (Beta) (فقط عند تمكين الترجمة الغامرة لترجمات YouTube يدويًا في [الإعدادات] - [ترجمات الفيديو]، والترجمات الأصلية للفيديو هي ترجمات إنجليزية تلقائية)
...
- يدعم الخبراء AI وضع "الاختيار الذكي"، حيث سيقوم النظام تلقائيًا باختيار الخبير AI الأنسب بناءً على الموقع الحالي (على سبيل المثال، سيتم اختيار خبراء AI المتعلقين بالتكنولوجيا تلقائيًا لـ The Verge و Hacker News، بينما سيتم اختيار تحسين الترجمة لـ Twitter تلقائيًا لـ Twitter).

## 1.5.7

- دعم إضافة خدمات ترجمة AI مخصصة متوافقة مع OpenAI، يمكن الوصول إليها في أسفل صفحة [الإعدادات]->[خدمات الترجمة].
...
- يدعم اسم نموذج AI المخصص صيغة متقدمة، استخدم + لإضافة نموذج، استخدم - لإخفاء نموذج، واستخدم model_name=display_name لتخصيص اسم العرض للنموذج، على سبيل المثال: +gpt-3.5-turbo,-gpt-4,gpt-4-turbo=gpt-4-super
...
- يمكن الآن لخدمات الترجمة AI تخصيص قائمة النماذج، مثل [OpenAI]، النظام يحتوي فقط على بعض النماذج الأكثر استخدامًا مدمجة. عند النقر على قائمة النماذج المنسدلة، العنصر الأخير الذي تراه هو [Set More Models]، بعد الإعداد، سيتم تذكره تلقائيًا لراحة المستخدمين في اختبار نماذج مخصصة مختلفة.
...
- إضافة ميزة مؤقتة لتمكين تحرير الترجمة في "المزيد" في اللوحة
...
- متوافق مع ترجمة مربع الإدخال في صفحة Bing Copilot
...
- تم إصلاح عرض الترجمات غير الطبيعي لبعض مقاطع فيديو Netflix [#1630](https://github.com/immersive-translate/immersive-translate/issues/1630)
...
- يدعم منصة ترجمات الفيديو [جامعة ساوثهامبتون](https://southampton.cloud.panopto.eu)
...
- دعم إعداد عرض الترجمات الثنائية اللغة/الأصلية والفورية
...
- تم إصلاح مشكلة محاذاة خيار "المزيد" حيث لم يكن محاذيًا لليسار على Windows.
...
- **إصلاحات الأخطاء**:
  ...
  - تم إصلاح المشكلة حيث كانت ترجمة الماوس تفشل عند فقدان الصفحة للتركيز.
...
- دعم [Enable Pro Membership](/pricing)، مما يسمح للمستخدمين بالاستمتاع بترجمات Deepl و OpenAI وإعدادات المزامنة السحابية.
...
- إصلاح مشكلة تجاوز عرض Popup في Safari.
- تحسين عملية البناء
...
- أكثر ذكاءً في تحديد ما إذا كانت منطقة تنقل، وإذا كان الأمر كذلك، لا يتم إجراء الترجمة
...
- إصلاح: مشكلة النافذة المنبثقة في علامة التبويب الجديدة في macOS.
...
- إصلاح: صفحة ويب TXT النقية مثل [هذه](https://edoras.sdsu.edu/~vinge/misc/singularity.html)
...
- إصلاح: مشكلة حرف محدد في منشئ الكتب الإلكترونية، مثل بعض مسار الفصول هو `xxx' xxxx`.
...
- ميزة: دعم عدد النصوص الفورية المخصصة للترجمة لخدمات الترجمة المختلفة.
...
- CI: تقديم الكود المصدري إلى فايرفوكس
...
- إصلاح اختيار الخدمة المنبثقة غير المكونة، الانتقال إلى صفحة الخيارات.
