...
- تمت الإضافة: دعم إعداد عدد الشاشات التي سيتم ترجمتها مسبقًا عند تصفح الصفحة الحالية.
...
- تم الإصلاح: مشكلة في ترجمة الصيغ بسبب "تحسين الترجمة على تويتر" في الإصدار 1.11.2.
...
- تم الإصلاح: مشكلة عدم تأثير [الإعدادات المتقدمة -> الحد الأدنى لعدد الأحرف المطلوبة لترجمة فقرة] في بعض الحالات
...
- يمكنك الآن الإبلاغ بسرعة عن مشاكل الترجمة لصفحات الويب في الكرة العائمة للترجمة الغامرة. بعد ترجمة صفحة ويب، إذا كانت هناك أي مشاكل، يمكنك النقر على زر [التغذية الراجعة] على الجانب الأيمن من الكرة العائمة، وملء وصف المشكلة، وسنتعامل معها في أقرب وقت ممكن.
...
- تم إصلاح المشكلة التي كانت تجعل YouTube Shorts بدون ترجمات تعرض دائمًا ترجمات من الفيديو السابق [#1655](https://github.com/immersive-translate/immersive-translate/issues/1655)
...
- تحسين كلمات Prompt متعددة لـ OpenAI، دعم صيغة YAML، مما يحسن من مرونة وسهولة استخدام التكوين
...
- تحسين النوافذ المنبثقة للأخطاء، تكتشف النوافذ المنبثقة لمشاكل الشبكة تلقائيًا خدمات الترجمة المجانية الصالحة.
...
- إضافة وصول سريع لتمكين ترجمات الفيديو على Netflix و Ted و Bloomberg و Udemy و Coursera.
...
- **إعدادات مفتاح الاختصار**: إضافة قائمة جديدة من المستوى الأعلى "الاختصارات" والوظائف التالية لمفاتيح الاختصار القابلة للتخصيص:
  ...
  - إضافة مفاتيح اختصار مخصصة لـ 6 خدمات ترجمة (مثل DeepL و OpenAI و Google و Microsoft و Gemini و Tencent Interactive Translation) لتسهيل التبديل المؤقت بين خدمات الترجمة.
...
- تم إصلاح: المشكلة التي لا يزال فيها التخزين المؤقت موجودًا بعد تعديل الـ prompt/النموذج في OpenAI.
...
- إصلاح عدم عرض ترجمات الفيديو الأصلية عند ضبط "الموقع الحالي على عدم الترجمة أبدًا".
...
- عند تشغيل الترجمة الثنائية في اللوحة، سيتم تحديث الصفحة تلقائيًا أولاً (لعرض الترجمات الثنائية بدقة أكبر)، وبعض المواقع لا تزال تتطلب من المستخدمين النقر يدويًا على زر "CC" في الموقع لتشغيل الترجمات.
...
- تحسين الـ Prompt الصيني لـ OpenAI، الآن أقل عرضة للهلوسة!
- تقليل طول الـ Prompts لـ OpenAI من 90 إلى 40، مما يوفر المزيد من حركة المرور.
...
- تحسين تكييف الترجمة لـ DeepL
- تحسين تكييف الترجمة لـ OpenAI لملفات الترجمة
...
- تعطيل مؤقتًا وظيفة تصدير PDF في Safari بسبب قيود Safari على تنزيل المكونات الإضافية (خطأ).
...
- إضافة عرض صفحة عدم الإذن في PDF
...
- تحسين Google Bard
- دعم ترجمة صندوق الإدخال للترجمة المباشرة لصندوق النص بالكامل بدون بادئات.
...
- تحسين: تم تعديل الحد الأدنى لعدد الأحرف في الفقرة التي تثير الترجمة إلى حد أدنى 4 أحرف لتقليل الارتباك، مع استخدام ميزات أخرى لتجنب ترجمة مناطق التنقل والمناطق النهائية للموقع.
...
- إصلاح: ترجمة openai
- إصلاح: التحقق من الوسوم غير المعروفة
...
- إصلاح: استخدام ذاكرة عالية للبرامج النصية والمكونات الإضافية (عن طريق إزالة opencc zh-CN إلى zh-TW، بدلاً من Google translate)
...
- إصلاح: البرنامج النصي كبير جدًا لـ greasy fork
- أفضل: تقليل حجم الملف

## 0.2.60

- ميزة: دعم zh-CN إلى zh-TW لـ Deepl
- ميزة: ميزة الترجمة الغامرة لـ Deepl
...
- إصلاح: عنصر كتلة الوسوم غير المعروفة
- إصلاح: عنصر translate=no يتجاوز
- إصلاح: مطابقة URL مع متعددة \*

## 0.2.58

- ميزة: دعم لون نص الترجمة المخصص، لون الإطار.
...
- إصلاح التحقق من الوسوم غير المعروفة المضمنة/الكتلة
...
- إصلاح خطأ مزامنة Google Drive للبرنامج النصي.
...
- إصلاح أن النص الداخلي للعناصر غير معرف
- إصلاح ترجمة caiyun لغة المصدر غير معروفة

## 0.2.44

- إصلاح تبديل قناع البرنامج النصي
- إصلاح منطق تبديل القناع

## 0.2.43

- إصلاح تبديل قناع البرنامج النصي مع حدث اللمس.
...
- إصلاح مشكلة مزامنة السحابة للبرنامج النصي
- نقل صفحة الخيارات المتقدمة إلى القائمة اليسرى.
- إضافة منطق إعادة المحاولة لخدمة الترجمة

## 0.2.41

- إصلاح ترجمة niu للبرنامج النصي
- إصلاح ترجمة xhtml

## 0.2.40

- إصلاح عرض ميزة بيتا
- إصلاح صفحة إعداد النوافذ المنبثقة في صفحة علامة تبويب جديدة
...
- أفضل لنصائح البرنامج النصي السيئة
...
- إصلاح اختيار النوافذ المنبثقة لخدمة غير مكونة، انتقل إلى صفحة الخيارات.
...
- دعم ترجمة iframe المضمنة للبرنامج النصي.
- دعم ترجمة shadowroot. على سبيل المثال:
  ...
  منطقة المحادثة.
- أيضًا تحقق من قاعدة المزامنة على النوافذ المنبثقة
...
- إزالة المطابقة غير القياسية للبرنامج النصي

## 0.2.27

- دعم ترجمة iframe المضمنة. (فقط للملحق، غير متاح للبرنامج النصي)
...
- إصلاح stay.app API للمتصفح للبرنامج النصي
...
- دعم سحب النوافذ المنبثقة للبرنامج النصي
- دعم ثلاث أصابع على جهاز اللمس.
- دعم إخفاء أيقونة النوافذ المنبثقة للبرنامج النصي.
