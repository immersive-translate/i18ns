...
- Adicionado: A versão para PC suporta a funcionalidade de tradução de seleção de texto (não-userscript)
- Adicionado: Suporte para ativar legendas AI no Youtube (membros Pro)
...
- Melhorado: Atualizada a interface do utilizador da lista suspensa do serviço de tradução no painel.
...
- Corrigido um problema onde o [Configurações Avançadas -> Número mínimo de caracteres necessários para traduzir um parágrafo] não entrava em vigor em alguns casos
...
- Corrigidos os problemas de layout de texto das traduções de banda desenhada para idiomas no alfabeto latino.
...
- Quando especialistas em IA são usados para "Seleção Inteligente", diferentes especialistas em IA podem ser personalizados para diferentes sites. Isto pode ser configurado em [Configurações] -> [Especialistas em IA] -> [Insira qualquer especialista].
...
- Nova funcionalidade: Tradução por IA suporta [Doubao large model](https://www.volcengine.com/product/doubao)
...
- A lista de modelos de IA personalizados suporta a sintaxe `-all`, que pode eliminar todos os modelos predefinidos.
...
- Corrigido o problema onde alterações no prompt geral do especialista sobrepunham o prompt do especialista em IA especificado [#1692](https://github.com/immersive-translate/immersive-translate/issues/1692)
...
- Corrigido o problema onde alterações nos estilos de legendas do Youtube não eram salvas
...
- Corrigido o suporte do APP imersivo para iOS para reprodução em tela cheia no YouTube.
...
- **Otimização da UI**: Redesenhada a UI para os avisos de erro de tradução de página para torná-los mais fáceis de entender. Quando há muitos erros, um pop-up irá avisar ativamente o utilizador.
...
- Corrigido: O problema em que a cache ainda existe após modificar o prompt/modelo no OpenAI.
...
- Corrigido o bug em que a troca de legendas em alguns sites de vídeo causava a exibição repetida da tradução.
- Corrigido o problema em que o rato pairava para continuar a traduzir sem pressionar a tecla de atalho em alguns casos.
...
- Reparar a não tradução de alguns parágrafos após ativar quebras de linha em parágrafos longos.
- Corrigido [Quando se ativa temporariamente a tradução de página web por um longo tempo, clicar no painel [Sempre traduzir este site] não cancela a tradução sempre #1172](https://github.com/immersive-translate/immersive-translate/issues/1172)
...
- Corrigir o problema de clique instável do painel de ação da bola flutuante da página do Firefox.
...
- A bola flutuante no móvel agora suporta operação de longa pressão, pressão curta para traduzir, longa pressão para abrir o painel.
- Traduzir e-books bilíngues agora também traduzirá o índice.
- A funcionalidade de Melhoria de Pesquisa (algumas páginas de Pesquisa do Google exibem resultados de pesquisa bilíngues) agora não está ativada por padrão e será removida no próximo Release.
...
- Quando se ativa legendas bilíngues no painel, ele primeiro atualizará a página automaticamente (para exibir legendas bilíngues com mais precisão), e alguns sites ainda exigem que os utilizadores cliquem manualmente no botão "CC" no site para ativar as legendas.
...
- Agora pode gerar um link público para a página bilíngue traduzida para o Immersive Translate.
  - [Clique](/docs/share/) no Ícone de Compartilhamento do Immersive Translate para gerá-lo com um clique!
- Resolvido o problema de que algumas plataformas não conseguiam reconhecer se o rato era suportado ou não
  - Existem alguns navegadores de desktop que suportam tanto touchscreen quanto rato, e o Immersive Translate é tecnicamente incapaz de detectar se tais plataformas suportam rato, então adicionamos a opção [Forçar Ativar Suporte a Rato] na configuração [Mouse Hover]
...
- Corrigir cliques instáveis na bola flutuante da página
...
- Suporte ao passar o rato para pressionar Ctrl novamente para mostrar o texto original
- Ignorar Língua Nunca Traduzir ao Passar o Rato
...
- Adicionado [Desativar Permanentemente a Função de Legendas de Vídeo], caso existam outros problemas de conflito e não deseje ativar a função de legendas bilíngues com o Immersive Translate.
...
- Otimização da velocidade de exportação de tradução apenas para PDF, se escolher exportar apenas a tradução, pode chamar diretamente a pré-visualização do sistema PDF para exportar, mais rápido.
- Deeplx suporta múltiplos URLs, basta separá-los com .

## 0.9.2

- A ferramenta de tradução PDF é migrada para a versão online: https://app.immersivetranslate.com/pdf/ , para que Grease Monkey e Safari possam usar a tradução PDF, e os problemas podem ser melhor iterados sem a necessidade de emitir uma versão para resolver o problema.
...
- Otimização de Tradução PDF
- Corrigido alguns bugs

## 0.7.16

- Tradução PDF com novas opções para controle rápido do estilo de tradução (arquivos PDF são muito estranhos e ativar essas opções pode ser útil para alguns arquivos PDF com formatação confusa)
...
- Aprimoramento da Caixa de Entrada: permite omitir o prefixo // e acionar a tradução de toda a caixa de entrada com 3 espaços, ou você pode desativar esta opção na página de configurações.

## 0.7.1

- Suporte a Aprimoramento de Pesquisa, quando ativado, ao pesquisar no Google/Google News em chinês, a coluna da direita mostrará automaticamente os resultados da pesquisa de palavras-chave correspondentes em inglês, que está ativado por padrão.
...
- Reestruturação da análise do núcleo da web, o que significa:
  ...
  - Tradução em larga escala de páginas da web em segundos
    - Por exemplo,: https://pve.proxmox.com/pve-docs/pve-admin-guide.html, que levava 30 segundos antes, agora é virada em segundos.
  - Uso de memória ultra-baixo para páginas da web complexas
...
- Correção: alguns ficheiros epub não podem ser encontrados.

## 0.2.84

- Funcionalidade: Suporte para Leitor e Criador de Ebooks
...
- Correção: Alguns sites usam `span` para estilos, por isso usamos `font` em vez de span para o wrapper de tradução alvo
...
- Correção: Conflito de Mensagem DingTalk
...
- Funcionalidade: Adicionar novo tema Solid Border (novo tema, borda sólida)
...
- Correção: configuração de tradução Azure, mas ainda mostra (necessário configurar)
...
- Correção: alterar transparência de 0.4 para 0.618 [#147] (https://github.com/immersive-translate/immersive-translate/pull/147)
- Correção: dicas de atalhos no popup
...
- Funcionalidade: Recurso Deepl do Immersive Translate
...
- Correção de innerText de elemento indefinido
- Correção de idioma de origem indefinido na tradução caiyun

## 0.2.44

- Correção de alternância de máscara no userscript
- Correção de lógica de alternância de máscara

## 0.2.43

- Correção de alternância de máscara no userscript com evento de toque.
...
- Suporte a ativação de recurso beta no painel de desenvolvedor
- Correção de atalhos na extensão mobile
...
- Correção de marca de exclusão de frame
...
- Correção de ignorar tags rb, rt em japonês.
...
- Correção de permanecer aberto configurações.
...
- Suporte a tradução de iframe inline no userscript.
...
- Correção de problema de requisição de macaco
...
- Correção de alteração de URL do cliente, permanecer automaticamente no estado de tradução.
...
- Corrigir safari globalThis.GM não funcionando

## 0.2.13

- Suporte para arrastar Popup de Userscript
- Suporte para três dedos em dispositivos de toque para acionar a alternância de páginas de tradução
- Suporte para ocultar o ícone do popup de userscript.
...
- Corrigir solicitação de grease monkey
...
- Corrigir permissão de conexão de userscript volc alpha.
