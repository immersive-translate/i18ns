...
A **versão Release** é a versão estável oficial, disponível em lojas de aplicativos mainstream como [Chrome](https://chromewebstore.google.com/detail/bpoadfkcbjbfhfodiogcnhhhpibjhbnh), [Edge](https://microsoftedge.microsoft.com/addons/detail/amkbmndfnliijdhojkpoglbnaaahippg), [Firefox](https://addons.mozilla.org/firefox/addon/immersive-translate/), [Apple](https://apps.apple.com/app/id6447957425), etc.
...
- Adicionado: Grok, Ollama, Groq, serviço de tradução Azure-OpenAI.
...
- Corrigido: Problema com a deteção incorreta de idioma na **Caixa de Entrada Aprimorada**.
...
- Corrigido: Problema com a tradução de fórmulas causado pela "Melhoria de Tradução do Twitter" na versão 1.11.2.
...
- Corrigido: Problemas de sincronização de tempo de legendas no YouTube após assistir a anúncios.
...
- Melhorado: Solução para exibição de legendas da Netflix
...
- Melhorado: Funcionalidade de aprimoramento de entrada e de passar o mouse sobre iframes incorporados em páginas web

## 1.10.2 (2024-10-11)

- Adicionado: Tradução de imagens (versão Beta)
- Adicionado: Modo Forece Enable Mouse Support (Ative esta funcionalidade apenas se a função de passar o mouse não estiver disponível em dispositivos tablet) **Settings** -> **Advanced Settings** -> **Forece Enable Mouse Support**
...
- Corrigido o problema com a sincronização de especialistas em IA personalizados
...
- Por padrão, o sistema lembrará o modo alternado ao clicar no ícone no painel para exibição de comparação bilíngue ou tradução apenas. Para alternar temporariamente, clique em "More" -> "Switch to translation-only display" no painel.
...
- Suporte para configurar exceções para idiomas e sites no modo de contraste bilíngue ou apenas tradução (configure na página de Settings -> Advanced Settings). Por exemplo: Se o seu modo de tradução padrão for contraste bilíngue, mas você não deseja que o Chinês Tradicional também use contraste bilíngue, então você pode adicionar o Chinês Tradicional aos idiomas de exceção para contraste bilíngue, assim o Chinês Tradicional usará o modo de tradução apenas. Da mesma forma, se o seu modo de tradução padrão for apenas tradução, mas você deseja que um determinado idioma ou site use o modo de contraste bilíngue, você também pode adicionar esse idioma ou site aos idiomas de exceção.
...
- Corrigido um problema onde o [Advanced Settings -> Minimum number of characters required for translating a paragraph] não surtia efeito em alguns casos
...
- Adicionada a opção de "Reportar problemas de tradução da página atual" e a opção de "Ligar/desligar rapidamente a bola flutuante" nas mais opções do painel.
...
- Após selecionar o texto, o menu de clique direito mudará para [Translate the text], clicando nele você pode pular automaticamente para a página de Tradução de Texto Imersiva
- Novo serviço de tradução gratuito para grandes modelos: SiliconCloud, disponível para todos os usuários.
- Adicionada tradução de grande modelo Zero-One-Thing, que pode ser usada preenchendo a API Key após o registro na plataforma Zero-One-Thing.
...
- Adotado algoritmo de divisão de frases inteligente por IA para legendas em inglês geradas automaticamente no YouTube [Pro Available]
- Otimizar a tradução de clique direito para "Translate to xx target language"
...
- Suporte para personalizar especialistas em IA, a entrada está na parte inferior da página de [Settings]->[AI Expert].
...
- As legendas bilíngues do YouTube agora suportam divisão inteligente de frases (Beta) (Apenas quando ativar manualmente a tradução imersiva de legendas do YouTube em [Settings] - [Video Subtitles], e as legendas originais do vídeo são legendas em inglês geradas automaticamente)
...
- Corrigido o problema em que modificar o idioma de destino e outras configurações no painel pop-up seria redefinido ao clicar na bola flutuante de tradução.
...
- Otimizado a inconsistência no layout das traduções em alguns casos.
...
- Otimizado mensagens de erro para tradução de legendas bilíngues locais
...
- Otimizado ainda mais a velocidade de tradução de legendas do Youtube
...
- Otimizado significativamente a velocidade de tradução de legendas do Youtube, e adicionado suporte para alternar entre ordem em chinês e inglês, personalizar cor e tamanho da fonte, etc.
...
- Otimizado palavras de prompt do Gemini
...
- Otimizada a questão de exibição de legendas de vídeo CC do Twitter.
...
- Corrigido o problema em que os placeholders na criação de ePub não eram substituídos corretamente
...
- **Otimização de Tradução Japonesa:** Otimizada ainda mais a tradução japonesa do OpenAI para melhorar a precisão e fluência.
...
- A funcionalidade de Aprimoramento de Pesquisa (algumas páginas de Pesquisa do Google exibem resultados de pesquisa bilíngues) agora não está ativada por padrão e será removida na próxima release.
...
- Resolvido o problema de que algumas plataformas não conseguiam reconhecer se o mouse era suportado ou não
  - Existem alguns navegadores de desktop que suportam tanto touchscreen quanto mouse, e o Immersive Translate tecnicamente não consegue detectar se essas plataformas suportam mouse, então adicionamos a opção [Forçar Ativar Suporte a Mouse] na configuração [Mouse Hover]
...
- Otimizar a tradução de ficheiros de legendas do OpenAI para contextualização
...
- Corrigir tradução automática quando o readwise está destacado

## 0.9.6

- Corrigido o problema de limpar a cache ao terminar a sessão do utilizador.
...
- Otimizar a deteção de melhorias de entrada para reduzir toques falsos
- Tradução de e-books e legendas online

## 0.9.3

- Tradução da Caixa de Entrada: exibe um lembrete pop-up quando é usada pela primeira vez, e o utilizador pode optar por desativá-la desta vez ou permanentemente para evitar toques acidentais.
- Otimização da velocidade de exportação de tradução apenas para PDF, se optar por exportar apenas a tradução, pode chamar diretamente a pré-visualização do sistema PDF para exportar, mais rápido.
- Deeplx suporta múltiplos URLs, basta separá-los com .

## 0.9.2

- A ferramenta de tradução PDF foi migrada para a versão online: https://app.immersivetranslate.com/pdf/ , para que o Grease Monkey e o Safari possam usar a tradução PDF, e os problemas podem ser melhor iterados sem a necessidade de emitir uma versão para resolver o problema.
...
- correção do problema de rolagem automática de leitura móvel online de epub
...
- Funcionalidades Beta: API Personalizada (Beta ativada nas definições do desenvolvedor)
...
- Suporta [Ativar Membro Pro](/pricing), que permite aos utilizadores desfrutar de traduções Deepl e OpenAI e definições de sincronização na nuvem.
...
- Otimização da Tradução PDF
- Corrigidos alguns bugs

## 0.7.16

- Tradução PDF com novas opções para controlo rápido do estilo de tradução (os ficheiros PDF são muito estranhos e ativar estas opções pode ser útil para alguns ficheiros PDF com formatação desordenada)
- As versões iOS e macOS estão de volta na Apple Store Country Store

## 0.7.15

- Fomos notificados pela Apple que as Traduções OpenAI foram temporariamente removidas do iOS e macOS, e estamos a tentar relançá-las na China.

## 0.7.11- 0.7.14

- Corrigir: problema de tradução de todas as regiões do Gmail
- Desativar temporariamente a função de exportação PDF do Safari devido à restrição do Safari sobre downloads de plug-ins (bug).
...
- Suporte a Resultados de Tradução de Suporte Melhorado de Entrada ctrl+z Desfazer
...
- Suporte a Melhoria de Pesquisa, quando ativado, quando pesquisa no Google/Google News em chinês, a coluna da direita mostrará automaticamente os resultados de pesquisa das palavras-chave correspondentes em inglês, que está ativado por padrão.
...
- Reparar a exibição de parágrafos da lista de texto PDF
- Ampliar a escala de fontes pequenas em PDF no chrome e safari
...
- Otimização da tradução PDF, a área de tradução pode agora ser editada/arrastada/eliminada
  - Arrastar no canto superior esquerdo, eliminar no canto superior direito, mudar o tamanho no canto inferior direito
...
- Suporte a tradução PDF para arrastar para mudar o tamanho e editar o conteúdo da tradução

## 0.6.12

- Corrige imagens de tradução Epub ficando menores em alguns navegadores
...
- O painel de extensão pode agora indicar páginas não suportadas (por exemplo, páginas sem permissões e páginas não-HTML)
...
- Otimizar o problema de traduções do OpenAI adicionando pontos desnecessariamente, (se não for detectado ponto no texto original, se o openai retornar um ponto, então removê-lo)
- Problemas com ficheiros de legendas do safari não sendo reconhecidos

## 0.6.3

O idioma padrão para tradução da caixa de entrada pode agora omitir o espaço, ou seja, //Hello World pode ser traduzido também.
...
- Otimizar: O número mínimo de caracteres num parágrafo que aciona a tradução foi modificado para um mínimo de 4 caracteres para reduzir a confusão, enquanto usa outras funcionalidades para evitar traduzir as áreas de navegação e fim do site.
...
- Corrigido o problema de que modificar o atalho de passar o mouse precisava de atualizar a página para ter efeito
...
- Corrigir: estado de entrada de serviços de tradução de alternância de UI de Opções não alterado.
...
- Funcionalidade: suporte OpenAI CHATGPT 3.5 (suporta interface OpenAI ChatGPT 3.5)
...
- Corrigir: falha na tradução de arquivo PDF, Bug da versão 0.2.60 para suportar deepl de zh-CN para zh-TW
...
- Corrigir seleção de serviço não configurado no popup, ir para a página de opções.
...
- Suporte a Ocultar ícone de popup do userscript.
...
- Corrigir exibição de elemento "display none".
...
- Corrigir alguns tags span que são elementos de bloco

## 0.0.20

- Suporte para não traduzir hashtags como #hash, tags de menção como @xxx, $tag, como $APPL
