...
- Melhorado: Quando o idioma da interface está definido para inglês, o idioma de destino padrão para a **Caixa de Entrada Aprimorada** é alterado para chinês.
...
- Adicionado: Modo Forçar Ativação do Suporte ao Rato (Ative esta funcionalidade apenas se a função de passar o rato não estiver disponível em dispositivos tablet) **Configurações** -> **Configurações Avançadas** -> **Forçar Ativação do Suporte ao Rato**
...
- Corrigido um problema onde as [Configurações Avançadas -> Número mínimo de caracteres necessários para traduzir um parágrafo] não surtia efeito em alguns casos
...
- Adotar algoritmo de divisão de frases inteligente por IA para legendas automáticas em inglês no YouTube [Pro Disponível]
...
- Corrigido o problema onde mudanças no prompt de especialista geral sobrepunham o prompt de especialista AI especificado [#1692](https://github.com/immersive-translate/immersive-translate/issues/1692)
...
- Suporte para traduzir nós de tags de entrada.
...
- Otimizada a sensibilidade de detecção de passar o mouse sobre Ctrl para evitar confundir `Ctrl+C` com `Ctrl`
...
- **Atualização da Interface do Painel de Pop-up:** O novo design visa melhorar a usabilidade e compreensão. Esta atualização inclui:
  ...
  - **Novas funcionalidades no menu principal:**
    - **Alternância de modo bilíngue/apenas tradução:** Agora pode alternar entre "Modo de Tradução Bilíngue" e "Modo Apenas Tradução" diretamente no menu principal, localizado à esquerda do botão de tradução.
...
- Correção: O problema onde alternar serviços de tradução não surtia efeito quando a página ainda não tinha sido traduzida.
...
- **Atualização da Interface da Página de Configurações do Plugin**:
  ...
  - Em "Configurações Avançadas", foi adicionada uma nova opção para permitir que os utilizadores especifiquem certas palavras (por exemplo, "LLM") a serem excluídas da tradução.
  - Em "Configurações Avançadas", foi adicionada uma nova opção para configurar o número mínimo de caracteres necessários para traduzir um parágrafo. O padrão é 4 caracteres, mas pode ser definido mais alto (por exemplo, 20), para que apenas parágrafos mais longos sejam traduzidos.
...
- **Novo Guia do Utilizador**: Quando novos utilizadores abrem pela primeira vez uma página em um idioma não-alvo, uma bolha de ajuda de bola flutuante é exibida.
...
- **Atualização de Configuração openAI**: Agora suporta a definição do número de configurações por segundo em decimais, como 0.5, significando 1 pedido a cada 2 segundos.
...
- [Suporte de configuração do Hoverball para ser fixado à esquerda #1168](https://github.com/immersive-translate/immersive-translate/issues/1168)
...
- Suporte a ajustes de espaçamento de linha e palavra em pdf para re-reconhecer parágrafos
- Correção do problema de rolagem automática em leitura online de epub em dispositivos móveis

## 0.8.7

- Suporte a downloads de tradução apenas em pdf
...
- Tradução PDF com novas opções para controlo rápido do estilo de tradução (ficheiros PDF são muito estranhos e ativar estas opções pode ser útil para alguns ficheiros PDF com formatação confusa)
...
- Suporte a Resultados de Tradução de Suporte a Entrada Melhorada ctrl+z Desfazer
...
- Suporte a tradução PDF arrastar para mudar o tamanho e editar o conteúdo da tradução
...
- Otimizar: O número mínimo de caracteres em um parágrafo que aciona a tradução foi modificado para um mínimo de 4 caracteres para reduzir a confusão, enquanto usa outras funcionalidades para evitar traduzir as áreas de navegação e fim do site.
...
- Funcionalidade: O estilo de tradução com linha dividida adiciona uma linha vertical para traduções de linha única
...
- Funcionalidade: Suporte à tradução de arquivo de legenda local (.srt,.ass,etc.)

## 0.3.17

- Funcionalidade: Suporte à tradução de arquivo .txt local.
...
- Funcionalidade: Suporte à tradução de html local para extensão
...
- Funcionalidade: Suporte à tradução de página web txt
...
- Correção: Alguns sites usam `span` para estilos, então usamos `font` em vez de span para o wrapper de alvo de tradução
...
- Corrigir: tradução de arquivo PDF falhou, Bug da 0.2.60 para suportar deepl de zh-CN para zh-TW

## 0.2.65

- Suporte a solicitação de limitação para múltiplos frames
...
- Funcionalidade: Suporte zh-CN para zh-TW para Deepl
...
- Funcionalidade: suporte a css injetado no painel do desenvolvedor
...
- Corrigir ficheiro local pdf do Firefox
...
- Corrigir erro do userscript do Firefox.
