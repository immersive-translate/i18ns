...
A **Preview version** é publicada com mais frequência e inclui algumas funcionalidades experimentais. Comparada com a versão Release, pode conter mais bugs. É lançada principalmente em
...
- Adicionado: Suporte para definir o número de ecrãs a serem pré-traduzidos ao navegar na página atual.
...
- Adicionado: Modo Forçar Ativação do Suporte ao Rato (Ative esta funcionalidade apenas se a função de passar o mouse não estiver disponível em dispositivos tablet) **Configurações** -> **Configurações Avançadas** -> **Forçar Ativação do Suporte ao Rato**
...
- Suporte aprimorado para entrada em Baidu, Gmail e outros campos de entrada
...
- Corrigido o tamanho da fonte não escalando proporcionalmente quando o YouTube está em tela cheia [#1681](https://github.com/immersive-translate/immersive-translate/issues/1681)
...
- Agora pode especificar uma fonte para a tradução na página de configurações -> [Configurações Básicas] -> [Estilo de Tradução]
...
- **Atualização da Interface do Painel de Pop-up:** O novo design visa melhorar a usabilidade e compreensão. Esta atualização inclui:
  ...
  - **Novas funcionalidades no menu principal:**
    - **Alternância de modo bilíngue/somente tradução:** Agora pode alternar entre "Modo de Tradução Bilíngue" e "Modo Somente Tradução" diretamente no menu principal, localizado à esquerda do botão de tradução.
...
- **Entrada de tradução de documentos integrada:** Agora, pode traduzir arquivos PDF, ePub e legendas através de uma entrada de upload unificada. Basta clicar no botão [PDF/ePub] no painel de pop-up, sem necessidade de selecionar [Mais] mais.
...
- **Novo Guia do Utilizador**: Quando novos utilizadores abrem uma página pela primeira vez em um idioma não-alvo, uma bolha de guia de ajuda de bola flutuante é exibida.
...
- O recurso de Melhoria de Pesquisa (algumas páginas do Google Search exibem resultados de pesquisa bilíngues) agora não está habilitado por padrão e será removido no próximo Release.
...
- Resolvido o problema de que algumas plataformas não conseguiam reconhecer se o mouse era suportado ou não.
  - Existem alguns navegadores de desktop que suportam tanto touchscreen quanto mouse, e o Immersive Translate tecnicamente não consegue detectar se tais plataformas suportam mouse, então adicionamos a opção [Forçar Habilitar Suporte ao Mouse] na configuração [Mouse Hover].
...
- Otimização da velocidade de exportação apenas de tradução em PDF, se optar por exportar apenas a tradução, pode chamar diretamente a pré-visualização do sistema PDF para exportar, mais rápido.
...
- Correção do problema de auto-rolagem em leitura online de epub em dispositivos móveis
...
- Tradução PDF com novas opções para controlo rápido do estilo de tradução (ficheiros PDF são muito estranhos e ativar estas opções pode ser útil para alguns ficheiros PDF com formatação desordenada)
...
- PDF finalmente suporta a exportação de versões bilíngues! Pode clicar no botão [Guardar] para exportar o ficheiro PDF bilíngue traduzido.
...
- Adaptado ao estilo do sistema Safari, o ícone da extensão do Safari mudou para contorno transparente.
...
- Corrigir vários problemas
- Página de configuração suporta eliminação em massa de URLs selecionados.
...
- Otimização da Tradução PDF
- Adaptado para o site Good Reads, site da Amazon e South China Morning Post
...
- Otimização da tradução PDF, a área de tradução agora pode ser editada/arrastada/excluída
...
- Otimizar o problema de traduções OpenAI adicionando pontos sem sentido, (se nenhum ponto for detetado no texto original, se openai retornar um ponto, então removê-lo)
- Problemas com ficheiros de legendas no safari não sendo reconhecidos
...
- Corrigir o problema de que imagens em algumas páginas web não são traduzidas

- Funcionalidade: O estilo de tradução com linha de divisão adiciona uma linha de divisão vertical para traduções de linha única
...
- Correção: Estilo de autoria de eBooks não funcionando
...
- Correção: Problema de estilo de leitura online de eBooks não funcionando
...
- A funcionalidade "Mostrar Apenas Traduções" chegou! Clique em [Mais] -> [Alternar para mostrar apenas traduções].
  ...
  - Suporte a atalhos personalizados, configurados nas configurações de interface -> Configurações de Atalho
...
- Funcionalidade: Refatorar a tradução de PDF, agora você pode traduzir a maioria dos PDFs com Immersive Translate
...
- UI: O modelo de campo de configurações do OpenAI usa seleção em vez de caixa de entrada de texto.
...
- Correção: problema de caractere específico do construtor de eBooks, como alguns caminhos de capítulo são `xxx' xxxx`.
...
- Correção: problema de progresso de eBook.
- Correção: melhor para dividir parágrafo longo, 1.5 bilhões, 25.5%, Sr. não será considerado como um limite
...
- Correção: Progresso do Construtor de eBooks parou em 99%.

## 0.3.9

- Correção: Estado de entrada de serviços de tradução de alternância de UI de Opções não alterado.

## 0.3.8

- UI: cor de carregamento mais transparente
- Correção: Detectar Idioma de eBook.
- Funcionalidade: Adicionar progresso de tradução para o construtor de eBooks, e um belo confete após o sucesso.
...
- Funcionalidade: Suporte a Leitor e Criador de eBooks
...
- Correção: limite máximo de tokens do OpenAI, alterar caracteres máximos de 1500
...
- Feat: Suporte a múltiplas chaves OpenAI, use `,` para separá-las.

## 0.2.80

- Feat: Adicionar Menu Ativar/Desativar para popup -> mais
...
- Feat: Suporte a contagem de texto de tradução imediata personalizada para diferentes serviços de tradução.
...
- Feat: Suporte ao Tema de Tradução Grey
...
- Feat: Suporte para verificar o serviço de tradução em options.html
- Feat: Suporte a quadro principal personalizado, pois alguns sites não usam o corpo como quadro principal
...
- Feat: Suporte à opção de tradução de título

## 0.2.63

- Feat: Suporte ao Serviço de Tradução Azure
- Feat: Suporte ao Serviço de Tradução Papago
...
- Desempenho: solicitações de serial para cocorrência
...
- Feat: Suporte de zh-CN para zh-TW para Deepl
- Feat: Recurso Deepl de Tradução Imersiva
- Feat: Suporte a zoom de tamanho de fonte personalizado
...
- Corrigir: Elemento de Bloqueio de Tags Desconhecidas
- Corrigir: elemento translate=no sobreposto
- Corrigir: correspondência de URL com múltiplos \*

## 0.2.58

- Feat: Suporte a cor de texto de tradução personalizada, cor da borda.
...
- Feat: suporte a css injetado no painel do desenvolvedor
...
- Remover atalhos alt+d, alt+s, pois podem entrar em conflito com atalhos nativos.
...
- Corrigir máscara ao passar o mouse quando alternar máscara novamente.
