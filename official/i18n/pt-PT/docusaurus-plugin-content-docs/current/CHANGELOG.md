...
- Melhorado: Acesso à interface de teste do serviço na página de configurações online.
...
- Melhorado: Melhorada a exibição de tradução bilíngue multilinha no Twitter e YouTube.
- Melhorado: Tradução de texto rico é desativada por padrão no modo bilíngue para melhorar a qualidade da tradução.
- ~~Melhorado: Adicionada a opção de personalizar a "**Ativar Tradução da Barra Lateral & Navbar**" em "**Configurações Avançadas**".~~
...
- ~~Melhorado: Melhorada a exibição de traduções bilíngues multilinha no Twitter.~~
...
- Corrigido o problema de não sincronização de especialistas em IA personalizados
...
- Adicionada tradução de grande modelo Zero-One-Thing, que pode ser usada preenchendo a Chave API após registrar-se na plataforma Zero-One-Thing.
...
- Suporte para definir a fonte da tradução no modo apenas tradução.
...
- **Novo Guia do Utilizador**: Quando novos utilizadores abrem uma página pela primeira vez em um idioma não-alvo, uma bolha de ajuda de bola flutuante é exibida.
...
- **Correções de Bugs**:
  ...
  - Corrigido o problema onde menos de 3 caracteres na funcionalidade de aprimoramento de campos de entrada não eram traduzidos.
  - Corrigido o problema onde alguns diretórios não eram traduzidos durante a produção de Epubs bilíngues.

- **Remoção de Funcionalidade**: Removida a funcionalidade de aprimoramento de informações bilíngues (exibindo resultados de pesquisa em inglês nas páginas de pesquisa do Google).

### Outras Atualizações

- **Guia de Novo Utilizador**: Quando novos utilizadores abrem uma página pela primeira vez em um idioma não-alvo, uma bolha de ajuda de bola flutuante é exibida.
...
- Corrigido: O problema de reconhecimento do idioma alvo padrão em algumas máquinas após a primeira instalação.
...
- O recurso de Melhoria de Pesquisa (algumas páginas do Google Search exibem resultados de pesquisa bilíngues) agora não está habilitado por padrão e será removido na próxima release.
...
- Reparar clique não responsivo do hoverball online em pdf
...
- Suporte de mouse hover pressione Ctrl novamente para mostrar o texto original
...
- Permite definir o intervalo entre os gatilhos de combinação de melhorias de entrada
...
- Corrigir imagens de tradução Epub ficando menores em alguns navegadores
...
- Correção: Pop-up do Userscript do Firefox ausente.
...
- Alteração: Cor do ícone de carregamento, de preto para azul, para adaptar a página web no modo escuro.
...
- Funcionalidade: Suporte a quadro principal personalizado, pois alguns sites não usam o corpo como quadro principal
...
- Funcionalidade: Adicionar `<all>` para sempre traduzir idiomas, agora você pode usá-lo para traduzir todos os idiomas, exceto o idioma alvo, e nunca traduzir idiomas
...
- usar sendMessage em vez de connect, porque o chrome desconectará a porta após
  5 minutos
...
- A tecla de atalho padrão para alternar tradução foi alterada para `alt+A`, porque
