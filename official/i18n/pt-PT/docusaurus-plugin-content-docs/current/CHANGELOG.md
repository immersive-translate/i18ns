...
Este registo de alterações é atualizado de acordo com o progresso do desenvolvimento. A data após a versão é a data de fusão do código, não a data de lançamento nas lojas de aplicativos (o tempo de revisão varia após a submissão a cada loja de aplicativos, podendo algumas demorar até uma semana para revisão). Atualmente, estamos avançando com duas versões.
...
A **versão Preview** é publicada com mais frequência e inclui algumas funcionalidades experimentais. Comparada à versão Release, pode conter mais bugs. É lançada principalmente em
...
- Melhorado o tratamento de "palavras não traduzíveis" em configurações avançadas.
...
- Corrigido o problema onde legendas não são exibidas no YouTube em modo "Apenas Tradução".
...
- Otimização da velocidade de exportação apenas de tradução de PDF, se optar por exportar apenas a tradução, pode chamar diretamente a pré-visualização de PDF do sistema para exportar, mais rápido.
...
- Funcionalidades Beta: API Personalizada (Beta ativado nas definições de desenvolvedor)
...
- Suporte a requisição de limitação para múltiplos frames
