...
- Otimizada a cobertura das configurações de sincronização na nuvem
...
- Suporte a Resultados de Tradução com ctrl+z Desfazer na Melhoria de Entrada
...
- Quando a caixa de entrada é configurada sem prefixo, suporta `ja Hello` tradução para japonês e `English Hello` tradução para inglês.
...
- A funcionalidade é em grande parte inalterada (novas funcionalidades estarão disponíveis no próximo Release!)
...
- Funcionalidade: Suporte à opção de tradução de título
