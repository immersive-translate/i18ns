...
- 추가: 현재 페이지를 탐색할 때 사전 번역할 화면 수 설정 지원.
...
- 추가: 불완전한 ePub 파일을 재수입 시 번역을 중단한 지점부터 계속 번역할 수 있음.
...
- 기본적으로, 간체 중국어에서 번체 중국어로의 번역과 그 반대의 경우 이중 언어 비교 모드가 아닌 번역 전용 모드를 사용
- 몇 가지 버그 수정
...
- 단축키 설정은 이제 모든 번역 서비스에 대해 고유한 단축키 할당을 허용
...
- 라틴 문자로 된 언어에 대한 만화 번역의 텍스트 레이아웃 문제 수정.
...
- 모든 사용자가 몰입형 번역 인터페이스의 다국어 번역에 [온라인으로 기여](https://weblate.immersivetranslate.com/projects/immersive-translate/extension/)할 수 있음.
...
- **통합 문서 번역 항목:** 이제 통합 업로드 항목을 통해 PDF, ePub, 자막 파일을 번역할 수 있습니다. 팝업 패널에서 [PDF/ePub] 버튼을 클릭하기만 하면 됩니다. 더 이상 [More]를 선택할 필요가 없습니다.
...
- 최적화: Epub 및 PDF 번역 과정에서 일부 콘텐츠가 번역되지 않을 경우, 패널에서 전체 번역 프로세스를 다시 시작하지 않고 다른 번역 서비스로 전환할 수 있습니다 (이전 로직은 즉시 새로운 번역 서비스를 사용하여 전체 책을 다시 번역하는 것이었습니다). 번역 중간에 다른 번역 서비스로 전환하고 [모든 실패한 단락 다시 시도]를 클릭하면 시스템이 새로운 서비스로 번역을 계속합니다.
...
- **기능 제거**: 이중 언어 정보 강화 기능 제거 (Google 검색 페이지에서 영어 검색 결과를 동시에 표시).
...
- .pdf 확장자가 없는 온라인 PDF 페이지에서 페이지의 호버볼을 클릭해 직접 PDF 번역 페이지로 이동할 수 있음.
- Safari에서 일부 입력 상자 향상 문제 수정.
- Grease Monkey 및 Safari에서 언어 감지 최적화.

## 0.11.6

- 플러그인 및 공식 웹사이트에 11개의 새로운 인터페이스 언어 추가, 이제 지원되는 인터페이스 언어는 14개로, 중국어 간체, 중국어 번체, 영어, 일본어, 한국어, 러시아어, 스페인어, 포르투갈어, 힌디어, 이탈리아어, 독일어, 프랑스어, 아랍어, 페르시아어를 포함.
...
- 기사 페이지의 경우 본문이 먼저 번역되고 나머지 사이드바 콘텐츠가 번역됩니다.
- DeepL 번역 문맥화 최적화.
...
- 기본 설정 수정: 많은 사용자가 설치 후 번역 도구를 사용하지 않을 것이라는 피드백을 받았으며, 번역 도구에 대한 기대는 설치 후 영어 웹 페이지를 자동으로 번역하는 것이므로, 이번 버전부터 중국 사용자에 대해 기본적으로 영어 페이지 번역 옵션이 켜져 있음 (사용자가 항상 번역할 언어를 이전에 설정한 경우 이를 존중하며, 이 변경은 확장의 초기 설정만 수정함), 이를 취소하려면 [팝업 패널 또는 설정 페이지](/docs/faq/#%E5%A6%82%E4%BD%95%E5%85%B3%E9%97%AD%E8%87%AA%E5%8A%A8%E7%BF%BB%E8%AF%91)에서 쉽게 취소할 수 있음
...
- OpenAI 번역이 무의미하게 마침표를 추가하는 문제 최적화 (원본 텍스트에 마침표가 감지되지 않으면, openai가 마침표를 반환할 경우 이를 제거함)
...
- 6월 첫 번째 릴리스, 이전 개인 서브도메인 https://immersive-translate.owenyoung.com에서 새로운 도메인 https://immersivetranslate.com으로 마이그레이션됨
...
- 일부 웹 페이지의 이미지가 복사된 후 커지는 문제 수정
...
- Fix: eBook 온라인 읽기 스타일이 작동하지 않는 문제
...
- 마우스 오버 단축키를 수정한 후 페이지 새로 고침이 필요했던 문제 수정
...
- 마우스 오버 단축키를 변경한 후 페이지 새로 고침이 필요했던 문제 수정
...
- Feat: `toggleTranslatePage` 호출을 위한 문서 이벤트를 제3자가 보낼 수 있도록 허용
...
- 버전: 이제부터 매달 마이너 버전 번호를 변경합니다. 예를 들어, 3월에는 버전이 0.3.0에서 시작하고, 4월에는 버전 번호가 0.4.0에서 시작합니다. 이는 확장이 의미론을 따르는 것이 의미가 없기 때문이며, 시간의 법칙에 따라 버전 번호를 표준화하는 것이 개발을 계속 업데이트하고 사용자가 문제를 더 쉽게 찾을 수 있도록 하는 동기입니다.
...
- Fix: 일부 사이트에서 스타일을 위해 `span`을 사용하므로 번역 대상 래퍼에 대해 `font`를 사용
