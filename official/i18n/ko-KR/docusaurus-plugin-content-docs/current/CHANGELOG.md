...
이 변경 로그는 개발 진행에 따라 업데이트됩니다. 버전 뒤의 날짜는 코드 병합 날짜이며, 앱 스토어에서의 출시 날짜가 아닙니다 (각 앱 스토어에 제출 후 검토 시간이 다르며, 최대 일주일이 걸릴 수 있습니다). 현재 우리는 두 가지 버전을 진행하고 있습니다.
...
**Preview 버전**은 더 자주 발행되며 일부 실험적 기능을 포함합니다. Release 버전에 비해 더 많은 버그가 있을 수 있습니다. 주로 다음에서 발행됩니다:
...
- 개선: Greasemonkey 및 Safari의 소수 언어가 포함된 페이지에서 언어 인식 기능 개선.
...
- 개선: 이중 언어 모드에서 리치 텍스트 번역이 기본적으로 꺼져 번역 품질을 향상.
- ~~개선: "**고급 설정**"에서 "**사이드바 및 네비게이션 바 번역 활성화**"를 사용자 정의할 수 있는 옵션 추가.~~
...
- 추가됨: 이미지 번역 (베타 버전)
- 추가됨: 마우스 지원 강제 활성화 모드 (태블릿 장치에서 마우스 오버 기능이 사용 불가능할 때만 이 기능 활성화) **설정** -> **고급 설정** -> **마우스 지원 강제 활성화**
...
- 이중 언어 대비 또는 번역 전용 모드에서 언어 및 웹사이트에 대한 예외 구성 지원 (설정 페이지 -> 고급 설정에서 구성). 예를 들어, 기본 번역 모드가 이중 언어 대비인 경우, 번체 중국어가 이중 언어 대비를 사용하지 않도록 하려면 번체 중국어를 이중 언어 대비 예외 언어에 추가하여 번체 중국어가 번역 전용 모드를 사용하도록 할 수 있습니다. 마찬가지로, 기본 번역 모드가 번역 전용인 경우 특정 언어나 웹사이트가 이중 언어 대비 모드를 사용하도록 하려면 해당 언어나 웹사이트를 예외 언어에 추가할 수 있습니다.
...
- Google Meet는 이제 실시간 회의에 대한 이중 언어 자막을 지원: 이제 Google Meet 회의에서 이중 언어 자막 기능을 즐길 수 있습니다. 회의 링크를 열고, 몰입형 번역 패널에서 이중 언어 자막을 활성화한 후 페이지를 새로 고치면 됩니다.
...
- AI 전문가 세부 페이지에 전문가가 사용한 프롬프트 표시
...
- 비디오 이중 언어 자막에서 대상 언어가 간체 중국어이고 원문이 번체 중국어일 때, 원문이 자동으로 간체 중국어로 변환되며 그 반대도 마찬가지입니다.
...
- 최적화: Epub 및 PDF 번역 과정에서 일부 콘텐츠가 번역되지 않을 경우, 패널에서 번역 서비스를 전환하여 전체 번역 과정을 다시 시작하지 않고도 다른 번역 서비스로 전환할 수 있습니다 (이전 로직은 새로운 번역 서비스를 즉시 사용하여 전체 책을 다시 번역하는 것이었습니다). 즉, 번역 중간에 다른 번역 서비스로 전환하고 [모든 실패한 단락 다시 시도]를 클릭하면 시스템이 새로운 서비스로 번역을 계속 진행합니다.
...
- **플러그인 설정 페이지 UI 업데이트**:
  ...
  - "고급 설정"에서 단락을 번역하기 위해 필요한 최소 문자 수를 구성할 수 있는 새로운 옵션 추가. 기본값은 4자이지만, 더 높게 설정할 수 있습니다 (예: 20자), 따라서 더 긴 단락만 번역됩니다.
...
- [일시적으로 웹 페이지 번역을 켠 후, 패널에서 [항상 이 웹사이트 번역]을 클릭해도 항상 번역이 취소되지 않는 문제 #1172](https://github.com/immersive-translate/immersive-translate/issues/1172) 수정.
...
- PDF 번역 도구가 온라인 버전으로 마이그레이션됨: https://app.immersivetranslate.com/pdf/ , Grease Monkey와 Safari가 PDF 번역을 사용할 수 있으며, 문제를 해결하기 위해 버전을 발행할 필요 없이 더 나은 반복이 가능함
...
- PDF가 마침내 이중 언어 버전의 내보내기를 지원! [저장] 버튼을 클릭하여 번역된 이중 언어 PDF 파일을 내보낼 수 있음
...
- 기본 설정 수정: 많은 사용자가 설치 후 번역 도구를 사용하지 않을 것이라는 피드백을 받았으며, 번역 도구에 대한 기대는 설치 후 영어 웹 페이지를 자동으로 번역하는 것이므로, 이 버전부터 중국 사용자에게는 기본적으로 영어 페이지 번역 옵션이 켜져 있음 (사용자가 항상 번역할 언어를 이전에 설정한 경우, 해당 설정이 존중되며, 변경은 확장의 초기 설정만 수정함), 취소하려면 [팝업 패널 또는 설정 페이지](/docs/faq/#%E5%A6%82%E4%BD%95%E5%85%B3%E9%97%AD%E8%87%AA%E5%8A%A8%E7%BF%BB%E8%AF%91)에서 쉽게 취소할 수 있음
...
- 문제 수정: 번역 후 이중 언어 eBook에 이미지가 나타나지 않는 문제 수정
- 수정: 번체 중국어에서 openai 번역 문제 수정

## 0.5.15

- 최적화: 번역을 트리거하는 단락의 최소 문자 수를 최소 4자로 수정하여 혼란을 줄이고, 사이트의 탐색 및 끝 영역 번역을 피하기 위해 다른 기능 사용
- 수정: Github 세부 정보가 확장 후 번역되지 않는 문제 수정

## 0.5.14

- 문제 수정: 일부 웹 페이지에서 이미지를 복사한 후 커지는 문제 수정
- 수정: Medium 댓글 섹션 번역되지 않는 문제 수정
- 문제 수정: 일부 페이지에서 이미지가 잘못 복사되는 문제 수정

## 0.5.12

- 기능: 단일 줄 번역에 수직 분할선을 추가하는 Split Line Translation Style
- 수정: 매우 드문 단락 분할 문제
...
- 기능: `toggleTranslatePage` 호출을 위해 타사 문서 이벤트 전송 허용
...
- 수정: macOS 사파리 확장 단축키
- 수정: 확장에 대한 콘텐츠 보안 정책 팝업, 부분적으로 사용자 스크립트에 대해
...
- 수정: sup, sub 태그 공백 감지
- 기능: 번역 서비스로 ChatGPT Plus 웹사이트 지원, 이는 베타 기능으로 개발자 설정에서 베타 기능을 활성화하여 액세스 가능. 매우 느리며, ChatGPT는 한 번에 하나의 요청만 보낼 수 있음. ChatGPT Plus 계정이 필요하며, 무료 계정에는 더 많은 제한이 있으므로 계정에 위험이 있을 수 있음. 주의해서 사용하세요.
...
- 기능: 다양한 번역 서비스에 대한 즉시 번역 텍스트 수 사용자 정의 지원.
