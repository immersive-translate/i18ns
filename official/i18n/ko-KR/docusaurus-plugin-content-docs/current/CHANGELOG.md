...
- 수정: "번역 우선, 원문 후" 모드에서 호버 번역이 작동하지 않는 문제 수정.
...
- 개선: Greasemonkey 및 Safari의 소수 언어가 포함된 페이지에서 언어 인식 기능 향상.
- 개선: 온라인 설정 페이지 테스트 서비스 인터페이스 접근성 향상.
...
- 개선: Safari 브라우저에서 shonenjumpplus와 같은 사이트의 만화 번역 실패 문제.
...
- 개선: 원본 언어 인식 향상. 이전에는 여러 언어가 포함된 페이지가 번역되지 않았지만, 이제는 올바르게 번역됨.
- 개선: Twitter의 경우, 다중 라인 콘텐츠 번역이 기본적으로 줄 바꿈되지 않도록 설정. 콘텐츠가 10줄 또는 1000자를 초과할 때만 줄 바꿈이 발생. 줄 바꿈은 설정 **고급 설정** -> **긴 단락에 대한 자동 줄 바꿈 활성화**를 통해 활성화 가능.
...
- 수정: YouTube에서 일시적으로 이중 자막을 활성화할 수 없는 문제.
- 수정: 번역 서비스를 전환한 후, "**향상된 입력 상자**"의 번역 서비스가 업데이트되지 않는 문제.
- 수정: 설정 페이지의 "**YouTube 이중 자막 활성화**" 스위치가 작동하지 않는 문제.
...
- 개선: **x.com**에서 번역할 때, 자막이 있는 비디오는 자동으로 이중 자막 번역.
...
- 개선: Twitter 및 YouTube에서 다중 라인 이중 번역의 표시 향상.
- 개선: 이중 모드에서 번역 품질을 향상시키기 위해 리치 텍스트 번역이 기본적으로 꺼짐.
...
- ~~개선: Twitter에서 다중 라인 이중 번역의 표시 향상.~~
...
- 기본적으로, 이중 언어 비교 또는 번역 전용 표시를 위해 패널의 아이콘을 클릭하여 전환한 모드를 시스템이 기억함. 임시로 전환하려면 패널에서 "More" -> "Switch to translation-only display"를 클릭
...
- Claude 번역은 이제 티베트어를 지원하고 Temperature 매개변수 구성을 허용
...
- AI 전문가 사용자 정의 지원, 입구는 [Settings]->[AI Expert] 페이지 하단에 있음.
- TED 사이트에서 자막 로딩 최적화
...
- YouTube 이중 언어 자막은 이제 지능형 문장 분할을 지원 (Beta) (설정 - 비디오 자막에서 YouTube 자막의 몰입형 번역을 수동으로 활성화할 때만, 원본 비디오 자막이 자동 생성된 영어 자막일 때만)
...
- "지능형 선택"에 AI 전문가가 사용될 때, 다른 웹사이트에 대해 다른 AI 전문가를 사용자 정의할 수 있음. 이는 [Settings] -> [AI Experts] -> [Enter any expert]에서 설정할 수 있음.
...
- iOS 18에서 플로팅 볼 바로 가기가 번역되지 않는 문제를 수정했습니다.
...
- AI 전문가가 "Intelligent Selection" 모드를 지원하여, 시스템이 현재 웹사이트에 가장 적합한 AI 전문가를 자동으로 선택합니다 (예: The Verge 및 Hacker News에서는 기술 관련 AI 전문가가 자동으로 선택되며, Twitter에서는 Twitter 번역 향상이 자동으로 선택됩니다).
...
- Claude라는 임시 번역을 위한 새로운 바로 가기가 도입되었으며, [Shortcut Settings page](https://dash.immersivetranslate.com/#shortcuts)에서 구성할 수 있습니다.
...
- [Kanopy](https://www.kanopy.com/), [RachelsEnglishAcademy](https://www.rachelsenglishacademy.com/), [Hulu](https://www.hulu.com/), [Three.js Journey](https://threejs-journey.com/)의 비디오 자막 번역 지원.
...
- **특정 단락 번역에 대한 모바일 지원:** 모바일 버전은 이제 지정된 단락 번역을 지원하며, 왼쪽으로 스와이프, 오른쪽으로 스와이프, 더블 탭, 트리플 탭, 다중 손가락 터치 제스처 등 다양한 바로 가기 작업을 추가했습니다. 기본적으로 활성화되어 있지 않으며, 사용자가 [마우스 호버] 아래의 설정 페이지에서 트리거 제스처를 적극적으로 선택해야 합니다.
...
- **몰입형 번역 경험:** 사용자 습관에 더 잘 적응하기 위해 YouTube 플랫폼의 전체 화면 모드에서 이중 언어 자막에 대한 바로 가기를 왼쪽으로 이동.
...
- 최적화: Epub 및 PDF 번역 과정에서 일부 콘텐츠가 번역되지 않을 경우, 패널에서 전체 번역 프로세스를 다시 시작하지 않고 다른 번역 서비스로 전환할 수 있습니다(이전 로직은 즉시 새로운 번역 서비스를 사용하여 전체 책을 다시 번역하는 것이었습니다). 번역 중간에 다른 번역 서비스로 전환하고 [모든 실패한 단락 다시 시도]를 클릭하면 시스템이 새로운 서비스를 사용하여 번역을 계속합니다.
- 최적화: 번역 오류 프롬프트의 글꼴 크기를 조정하여 가독성 향상.
...
- [일시적으로 웹 페이지 번역을 켜고 오랜 시간 동안 패널에서 [항상 이 웹사이트 번역]을 클릭해도 항상 번역이 취소되지 않는 문제 #1172](https://github.com/immersive-translate/immersive-translate/issues/1172) 수정.
...
- [일시적으로 웹 번역을 켜고 오랜 시간 동안 항상 번역이 취소되지 않는 문제 #1172](https://github.com/immersive-translate/immersive-translate/issues/1172)
- PDF 파일 초기화 문제 최적화.
...
- 기본 설정 수정: 많은 사용자가 설치 후 번역 도구를 사용하지 않을 것이라는 피드백을 받았으며, 번역 도구에 대한 기대는 설치 후 영어 웹 페이지를 자동으로 번역하는 것이므로, 이번 버전부터 중국 사용자에게는 기본적으로 영어 페이지 번역 옵션이 켜져 있음 (사용자가 항상 번역할 언어를 이전에 설정한 경우, 그 설정이 존중되며, 이 변경은 확장의 초기 설정만 수정함), 취소하려면 [팝업 패널 또는 설정 페이지](/docs/faq/#%E5%A6%82%E4%BD%95%E5%85%B3%E9%97%AD%E8%87%AA%E5%8A%A8%E7%BF%BB%E8%AF%91)에서 쉽게 취소할 수 있음.
...
- 일부 입력 상자 스타일 수정
- 내비게이션 영역인지 더 스마트하게 판단하며, 그렇다면 번역이 수행되지 않음
- 가능한 XSS 인젝션 공격 수정
...
- 번역의 기본 로딩 색상 업데이트
- 입력 상자가 접두사 없이 구성된 경우, `ja Hello`는 일본어로 번역되고 `English Hello`는 영어로 번역을 지원함.
...
- 입력: // Hello World를 웹 페이지의 입력 상자에 입력한 다음, 스페이스 바를 세 번 클릭하여 문단을 영어로 번역
...
- 6월 첫 릴리스, 이전 개인 서브도메인 https://immersive-translate.owenyoung.com에서 새로운 도메인 https://immersivetranslate.com으로 마이그레이션
...
- 최적화: 번역을 트리거하는 단락의 최소 문자 수를 최소 4자로 수정하여 혼란을 줄이고, 사이트의 내비게이션 및 종료 영역을 번역하지 않도록 다른 기능을 사용함.
...
- Feat: ChatGPT Plus 웹사이트를 번역 서비스로 지원, 이는 베타 기능으로 개발자 설정에서 베타 기능을 활성화하여 액세스할 수 있습니다. 매우 느리며, chatgpt는 한 번에 하나의 요청만 보낼 수 있습니다. ChatGPT Plus 계정이 있는지 확인하세요. 무료 계정에는 더 많은 제한이 있으며, 계정에 위험을 초래할 수 있는지 확실하지 않으므로 주의해서 사용하세요.
...
- Fix: ebook 진행 문제.
- Fix: 긴 단락 분할을 더 잘 처리, 1.5 billion, 25.5%, Mr.는 경계로 간주되지 않음
...
- 변경: 로딩 아이콘 색상, 어두운 모드 웹 페이지에 적응하기 위해 검정에서 파랑으로 변경.
...
- 버전: 이제부터 매월 마이너 버전 번호를 변경합니다. 예를 들어, 3월에는 버전이 0.3.0에서 시작하고, 4월에는 버전 번호가 0.4.0에서 시작합니다. 이는 확장이 의미론을 따르는 것이 의미가 없기 때문이며, 시간의 법칙에 따라 버전 번호를 표준화하는 것이 개발을 계속 업데이트하고 사용자가 문제를 더 쉽게 찾을 수 있도록 하는 동기입니다.
...
- Fix: 일부 사이트는 스타일에 `span`을 사용하므로 번역 대상 래퍼에 span 대신 `font` 사용
...
- 기능: 다양한 번역 서비스에 대한 즉시 번역 텍스트 수 맞춤 지원.
...
- 5분 후 포트가 끊어지기 때문에 connect 대신 sendMessage 사용
- 텍스트 컨테이너 감지 개선
