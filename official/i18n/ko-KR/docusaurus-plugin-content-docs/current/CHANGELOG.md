...
이 변경 로그는 개발 진행 상황에 따라 업데이트됩니다. 버전 뒤의 날짜는 코드 병합 날짜이며, 앱 스토어에서의 출시 날짜가 아닙니다(각 앱 스토어에 제출 후 검토 시간이 다르며, 일부는 검토에 최대 일주일이 걸릴 수 있습니다). 현재 두 가지 버전을 진행 중입니다.
...
- 추가됨: 실시간 회의 **자막 번역**이 이제 "플로팅 볼"을 통해 활성화할 수 있으며, Zoom, Google Meet, Microsoft Teams에서 사용할 수 있습니다.
...
- 이중 언어 대비 또는 번역 전용 모드에서 언어 및 웹사이트에 대한 예외를 구성하는 지원 (설정 페이지 -> 고급 설정에서 구성). 예를 들어, 기본 번역 모드가 이중 언어 대비인 경우, 번체 중국어가 이중 언어 대비를 사용하지 않도록 하려면 번체 중국어를 이중 언어 대비 예외 언어에 추가하여 번체 중국어가 번역 전용 모드를 사용하도록 할 수 있습니다. 마찬가지로, 기본 번역 모드가 번역 전용인 경우, 특정 언어나 웹사이트가 이중 언어 대비 모드를 사용하도록 하려면 해당 언어나 웹사이트를 예외 언어에 추가할 수 있습니다.
...
- AI 전문가가 "Intelligent Selection" 모드를 지원하여 시스템이 현재 웹사이트에 따라 가장 적합한 AI 전문가를 자동으로 선택합니다 (예: The Verge 및 Hacker News에는 기술 관련 AI 전문가가 자동으로 선택되며, Twitter에는 Twitter 번역 향상이 자동으로 선택됩니다).

## 1.5.7

- OpenAI와 호환되는 사용자 정의 AI 번역 서비스를 추가로 지원하며, [Settings]->[Translation Services] 페이지 하단에서 접근할 수 있습니다.
...
- 긴 단락을 줄 바꿀 때 단어 경계 문제를 수정했습니다 [#86](https://github.com/immersive-translate/immersive-translate/issues/86).
- 일본어 및 한국어 인식 문제를 수정했습니다.
- 모바일 장치에서 Reddit 페이지가 스크롤될 때 번역되지 않는 문제를 수정했습니다.
- DeepL로 번역이 누락된 일부 페이지 문제를 수정했습니다.
...
- 수정: Safari에서 일부 번역 스타일(예: 밑줄)이 효과적이지 않은 문제.
...
- 모바일 기기에서 [지정된 단락 번역]에 대한 단축키 구성 추가
- `Ctrl+C`를 `Ctrl`로 오인하지 않도록 Ctrl 마우스 호버 감도 최적화
- 비디오 자막이 내장된 기계 번역 자막을 사용할지 여부를 빠르게 전환할 수 있는 새로운 단축키 설정 추가
- Sci-Hub 웹사이트에서 플로팅 볼을 클릭하면 웹페이지에 내장된 PDF를 번역

## 1.1.6

- **모바일에서 특정 단락 번역 지원:** 모바일 버전은 이제 지정된 단락 번역을 지원하며, 왼쪽으로 스와이프, 오른쪽으로 스와이프, 더블 탭, 트리플 탭, 다중 손가락 터치 제스처 등 다양한 단축 작업을 추가. 기본적으로 활성화되어 있지 않으며, 사용자가 [마우스 호버] 설정 페이지에서 트리거 제스처를 선택해야 함.
...
- Windows에서 "더 보기" 옵션이 왼쪽 정렬되지 않는 문제 수정.
...
- **통합 문서 번역 항목:** 이제 통합 업로드 항목을 통해 PDF, ePub, 자막 파일을 번역 가능. 팝업 패널에서 [PDF/ePub] 버튼을 클릭하면 [더 보기]를 선택할 필요 없음.
...
- 최적화: Epub 및 PDF 번역 과정에서 일부 콘텐츠가 번역되지 않을 경우, 패널에서 전체 번역 과정을 다시 시작하지 않고 다른 번역 서비스로 전환 가능 (이전 로직은 즉시 새로운 번역 서비스를 사용하여 전체 책을 다시 번역하는 것이었음). 이는 번역 중간에 다른 번역 서비스로 전환하고 [모든 실패한 단락 다시 시도]를 클릭하면 시스템이 새로운 서비스로 번역을 계속 진행함을 의미.
- 최적화: 번역 오류 메시지의 글꼴 크기 조정하여 가독성 향상.
...
- **플러그인 설정 페이지 UI 업데이트**:
  ...
  - "고급 설정"에 특정 단어(예: "LLM")를 번역에서 제외하도록 사용자 지정할 수 있는 새로운 옵션 추가.
  - "고급 설정"에 단락 번역에 필요한 최소 문자 수를 구성할 수 있는 새로운 옵션 추가. 기본값은 4자이지만, 더 높게 설정 가능(예: 20)하여 더 긴 단락만 번역되도록 함.
...
- Gemini 번역 지원 추가.
- 일부 경우 웹페이지가 중간 위치로 스크롤될 때 번역이 즉시 응답하지 않는 문제 수정.
...
- [웹 번역을 일시적으로 켜고 일정 시간 후 항상 번역이 취소되지 않는 문제 #1172](https://github.com/immersive-translate/immersive-translate/issues/1172)
- PDF 파일 초기화 문제 최적화.
...
- Youtube 자막이 Dual subtitle 플러그인과 충돌하는 문제 수정(충돌을 피하기 위해 Youtube Dual 플러그인이 감지되면 Immersive Translate의 Youtube 자막 번역이 활성화되지 않음).
...
- 기사 페이지의 경우, 이제 본문 텍스트가 사이드바 콘텐츠보다 먼저 번역됩니다.
- DeepL 번역 문맥 최적화
...
- 입력 상자 번역: 처음 사용 시 팝업 알림을 표시하며, 사용자는 이번에 비활성화하거나 영구적으로 비활성화하여 실수로 터치하는 것을 방지할 수 있습니다.
...
- 사용자 시스템 지원
- [Enable Pro Membership](/pricing) 지원, 이를 통해 사용자는 Deepl 및 OpenAI 번역과 클라우드 동기화 설정을 즐길 수 있습니다.
...
- 기본 설정 수정: 많은 사용자가 설치 후 번역 도구를 사용하지 않을 것이라는 피드백을 받았으며, 번역 도구에 대한 기대는 설치 후 영어 웹 페이지를 자동으로 번역하는 것이므로, 이번 버전부터 중국 사용자에 대해 기본적으로 영어 페이지 번역 옵션이 켜져 있습니다 (사용자가 항상 번역할 언어를 이전에 설정한 경우, 해당 설정이 유지되며, 변경은 확장의 초기 설정만 수정합니다). [팝업 패널 또는 설정 페이지](/docs/faq/#%E5%A6%82%E4%BD%95%E5%85%B3%E9%97%AD%E8%87%AA%E5%8A%A8%E7%BF%BB%E8%AF%91)에서 쉽게 취소할 수 있습니다.
...
- 모든 웹 페이지의 입력 상자에 // Hello World를 입력한 다음 스페이스 바를 세 번 클릭하여 단락을 영어로 번역합니다.
...
- 기능: Split Line Translation Style이 단일 행 번역에 수직 분할선을 추가
- 수정: 매우 드문 단락 분할 문제
...
- 수정: macOS 사파리 확장 단축키
- 수정: 확장 및 일부 userscript의 콘텐츠 보안 정책 팝업
...
- 수정: &nbsp;를 공백으로 유지
- 제거: Papago 서비스 중단으로 인한 제거 [#310](https://github.com/immersive-translate/immersive-translate/issues/310)
...
- UI: 로딩 색상 더 투명하게
- 수정: eBook 언어 감지
- 기능: eBook 빌더 번역 진행 추가, 성공 후 아름다운 종이 조각
...
- 버전: 이제부터 매월 마이너 버전 번호를 변경합니다. 예를 들어, 3월에는 버전이 0.3.0에서 시작하고, 4월에는 0.4.0에서 시작합니다. 이는 확장이 의미론을 따르는 것이 의미가 없기 때문이며, 시간의 법칙에 따라 버전 번호를 표준화하는 것이 개발을 계속 업데이트하고 사용자가 문제를 더 쉽게 찾을 수 있도록 하는 동기부여가 됩니다.
