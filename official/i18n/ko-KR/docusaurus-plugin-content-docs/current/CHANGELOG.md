...
이 변경 로그는 개발 진행 상황에 따라 업데이트됩니다. 버전 뒤의 날짜는 코드 병합 날짜이며, 앱 스토어에서의 출시 날짜가 아닙니다(각 앱 스토어에 제출 후 검토 시간이 다르며, 일부는 검토에 최대 일주일이 걸릴 수 있습니다). 현재 우리는 두 가지 버전을 진행하고 있습니다.

**Release 버전**은 공식 안정 버전으로, [Chrome](https://chromewebstore.google.com/detail/bpoadfkcbjbfhfodiogcnhhhpibjhbnh), [Edge](https://microsoftedge.microsoft.com/addons/detail/amkbmndfnliijdhojkpoglbnaaahippg), [Firefox](https://addons.mozilla.org/firefox/addon/immersive-translate/), [Apple](https://apps.apple.com/app/id6447957425) 등과 같은 주류 앱 스토어에서 사용할 수 있습니다.
...
- 최적화: 시스템 역할을 지원하지 않는 o1, o3과 같은 모델과의 호환성을 개선했습니다(시스템 프롬프트 구성이 비어 있을 때 시스템 역할 매개변수가 전달되지 않음).

## 1.14.16 Release (2025-02-21)

- 추가: Deepseek, Gemini, Claude가 컨텍스트 전환을 지원합니다.
- 수정: 업데이트 용어가 새로운 번역 요청을 보내지 않습니다.
...
- 새로운 기능: Pro 멤버십이 DeepSeek (v3) 번역 서비스를 지원합니다.
- 수정: 사용자 구성 파일이 크기 제한을 초과하는 문제를 해결했습니다.
- 개선: 오른쪽 클릭 메뉴 항목을 닫을 수 있습니다(고급 설정에서 조작).
- 개선: Greasemonkey 및 Safari의 소수 언어 페이지에서 언어 인식 기능을 개선했습니다.
...
- 새로운 기능: 무료 이미지 번역이 이제 PC 버전의 Chrome 및 Edge 브라우저에서 사용할 수 있으며, 오른쪽 클릭 메뉴를 통해 접근할 수 있습니다.
- 수정: Gemini에서 다중 세그먼트 번역 중 일부 콘텐츠가 누락되는 문제를 해결했습니다.
- 최적화: YouTube 자막 로딩을 개선했습니다.
- 새로운 기능: AI 번역 서비스가 이제 노르웨이어를 지원합니다.
...
- 추가: Pro 회원은 이제 **Zhipu 4 Plus** 모델을 직접 사용할 수 있습니다.
- 개선: Gemini 번역을 개선했습니다.

## 1.13.1 (2025-01-10)

- 추가: 번역된 텍스트와 원본 텍스트가 동일한 문자 체계에 속할 때, 번역을 전문 스타일로 표시합니다.
...
- 개선: Twitter의 경우, 다중 라인 콘텐츠 번역이 기본적으로 줄 바꿈되지 않도록 설정됩니다. 콘텐츠가 10줄 또는 1000자를 초과할 때만 줄 바꿈이 발생합니다. 줄 바꿈은 설정 **고급 설정** -> **긴 단락에 대한 자동 줄 바꿈 활성화**를 통해 활성화할 수 있습니다.
...
- 수정: YouTube에서 일시적으로 이중 언어 자막을 활성화할 수 없는 문제를 수정했습니다.
...
- 추가: 불완전한 ePub 파일을 다시 가져올 때 번역을 중단된 지점부터 계속할 수 있습니다.
...
- ~~개선: "**사이드바 및 네비게이션 바 번역 활성화**"를 사용자 지정할 수 있는 옵션을 "**고급 설정**"에 추가했습니다.~~
...
- 수정: 광고 시청 후 YouTube에서 자막 동기화 문제를 수정했습니다.
- 수정: MacOS 15의 Safari에서 오른쪽 클릭 번역 메뉴의 표시 문제를 수정했습니다.
- 수정: 특정 웹사이트에서 **향상된 입력**의 Ctrl+Z 실행 취소 기능 문제를 수정했습니다.
...
- 추가됨: 이미지 번역 (베타 버전)
- 추가됨: 강제 마우스 지원 모드 활성화 (태블릿 장치에서 마우스 오버 기능이 사용 불가능할 때만 이 기능을 활성화) **설정** -> **고급 설정** -> **강제 마우스 지원 활성화**
...
- DeepL 번역 서비스가 이제 공식적으로 번체 중국어를 대상 언어로 지원합니다 (이전에는 DeepL로 번체 중국어로 번역할 때 추가적인 제3자 간체에서 번체로의 변환 과정이 필요했습니다).
- 리치 텍스트 번역 성능 최적화.

## 1.8.3

- Google Meet는 이제 실시간 회의에 대한 이중 언어 자막을 지원합니다: 이제 Google Meet 회의에서 이중 언어 자막 기능을 즐길 수 있습니다. 회의 링크를 열고, 몰입형 번역 패널에서 이중 언어 자막을 활성화한 후 페이지를 새로 고침하여 경험해보세요.
- 패널의 더 많은 옵션에서 "현재 웹페이지의 번역 문제 보고" 옵션과 "플로팅 볼 빠른 켜기/끄기" 옵션 추가.
...
- 이제 몰입형 번역의 플로팅 볼에서 웹페이지 번역 문제를 빠르게 보고할 수 있습니다. 웹페이지를 번역한 후 문제가 있는 경우, 플로팅 볼 오른쪽의 [피드백] 버튼을 클릭하여 문제 설명을 작성하면, 가능한 한 빨리 처리하겠습니다.
...
- Claude 번역은 이제 티베트어를 지원하며 Temperature 매개변수 구성을 허용합니다
- AI 전문가 세부 페이지는 전문가가 사용하는 프롬프트를 표시합니다
- 단축키 설정은 이제 모든 번역 서비스에 대해 고유한 단축키 할당을 허용합니다
...
- DeepL, Microsoft Translate, Google Translate, OpenAI, Claude, Gemini 및 기타 번역 서비스는 원본 텍스트 형식을 유지하는 번역을 지원합니다 (예: 링크, 굵게 등)
- 텍스트를 선택한 후, 마우스 오른쪽 버튼 메뉴가 [텍스트 번역]으로 변경되며, 클릭하면 몰입형 번역 텍스트 번역 페이지로 자동 이동합니다
...
- AI 전문가 사용자 정의 지원, 입구는 [설정]->[AI 전문가] 페이지 하단에 있습니다.
- TED 사이트에서 자막 로딩 최적화
...
- YouTube 이중 언어 자막은 이제 지능형 문장 분할을 지원합니다 (베타) ([설정] - [비디오 자막]에서 YouTube 자막의 몰입형 번역을 수동으로 활성화할 때만, 원본 비디오 자막이 자동 생성된 영어 자막일 때)
...
- AI 전문가가 "Intelligent Selection"에 사용될 때, 다양한 AI 전문가를 다른 웹사이트에 맞게 사용자 정의할 수 있습니다. 이는 [Settings] -> [AI Experts] -> [Enter any expert]에서 설정할 수 있습니다.
- "Translation Only" 모드에서 YouTube 자막이 표시되지 않는 문제를 수정했습니다.
- Mubi에서 이중 언어 자막이 작동하지 않는 문제를 수정했습니다.
- Adobe Acrobat 플러그인으로 열리는 PDF와 호환됩니다.
...
- 중국 내에서의 최근 Microsoft 번역 서비스 요청이 불안정; 오류 발생 시 시스템이 현재 사용 가능한 번역 서비스를 자동으로 감지하여 사용자가 빠르게 전환할 수 있도록 함.
- 네트워크 오류 메시지 최적화
- 텍스트 색상 구성 시 RGBA 미리보기를 지원하지 않는 문제 수정 [#1435](https://github.com/immersive-translate/immersive-translate/issues/1435)
- Safari 플러그인 버전 업그레이드 시 항상 설치 성공 페이지를 표시하는 문제 수정
...
- **특정 단락 번역에 대한 모바일 지원:** 모바일 버전은 이제 지정된 단락 번역을 지원하며, 왼쪽으로 스와이프, 오른쪽으로 스와이프, 더블 탭, 트리플 탭, 다중 손가락 터치 제스처를 포함한 다양한 단축 작업을 추가. 기본적으로 활성화되어 있지 않으며, 사용자가 [마우스 호버] 설정 페이지에서 트리거 제스처를 선택해야 함.
- **Gemini 기본 버전 업데이트:** 기본 버전이 `v1beta`로 업데이트.
- **고전 중국어 번역 수정:** Microsoft 및 OpenAI의 고전 중국어 번역 기능 수정.
- **일본어 번역 최적화:** OpenAI의 일본어 번역을 더욱 최적화하여 정확성과 유창성 향상.
...
- **팝업 패널 UI 업그레이드:** 새로운 디자인은 사용성 및 이해도를 향상시키기 위한 것. 이 업데이트에는 다음이 포함됨:

  - **메인 메뉴의 새로운 기능:**
    - **이중 언어/번역 전용 모드 전환:** 이제 메인 메뉴에서 번역 버튼 왼쪽에 위치한 "이중 언어 번역 모드"와 "번역 전용 모드" 간 전환 가능.
...
- **통합 문서 번역 항목:** 이제 PDF, ePub, 자막 파일을 통합 업로드 항목을 통해 번역 가능. 팝업 패널에서 [PDF/ePub] 버튼을 클릭하면 [More]를 선택할 필요 없음.
...
- 최적화: Epub 및 PDF 번역 과정에서 일부 콘텐츠가 번역되지 않을 경우, 패널에서 전체 번역 과정을 다시 시작하지 않고 다른 번역 서비스로 전환 가능 (이전 로직은 즉시 새로운 번역 서비스를 사용하여 전체 책을 다시 번역하는 것이었음). 즉, 번역 중간에 다른 번역 서비스로 전환하고 [모든 실패한 단락 다시 시도]를 클릭하면 시스템이 새로운 서비스로 번역을 계속 진행.
...
- **단축키 설정**: 새로운 최상위 메뉴 "단축키" 및 다음과 같은 사용자 정의 가능한 단축키 기능 추가:
  ...
  - 현재 입력 상자의 내용을 번역하기 위해 키 조합 지정, 이전의 스페이스 바를 세 번 빠르게 누르는 방법 보완.
  - 페이지에서 "마우스 호버 시 직접 번역"을 일시적으로 활성화하기 위해 키 조합 지정. 다시 누르면 이 기능 취소.
  - 6개의 번역 서비스(DeepL, OpenAI, Google, Microsoft, Gemini, Tencent Interactive Translation 등)에 대한 전용 단축키 추가로 번역 서비스 간 임시 전환 용이.

- **플러그인 설정 페이지 UI 업데이트**:

  - "고급 설정"에 특정 단어(예: "LLM")를 번역에서 제외하도록 지정할 수 있는 새로운 옵션 추가.
  - "고급 설정"에 단락 번역에 필요한 최소 문자 수를 구성할 수 있는 새로운 옵션 추가. 기본값은 4자이지만, 더 높게 설정 가능(예: 20자), 따라서 더 긴 단락만 번역.
...
- **신규 사용자 가이드**: 신규 사용자가 비대상 언어로 페이지를 처음 열 때, 플로팅 볼 도움말 가이드 버블 표시.
...
- **Feature Removal**: 이중 언어 정보 향상 기능(구글 검색 페이지에서 영어 검색 결과를 동시에 표시)을 제거했습니다.
...
- 플러그인 및 공식 웹사이트에 11개의 새로운 인터페이스 언어가 추가되어 이제 지원되는 인터페이스 언어는 14개로, 중국어 간체, 중국어 번체, 영어, 일본어, 한국어, 러시아어, 스페인어, 포르투갈어, 힌디어, 이탈리아어, 독일어, 프랑스어, 아랍어, 페르시아어가 포함됩니다.
...
- 유형: // Hello World를 웹 페이지의 입력 상자에 입력한 다음, 스페이스 바를 세 번 클릭하여 문단을 영어로 번역합니다.
...
- UI: 로딩 색상 더 투명하게
- 수정: eBook 언어 감지
- 기능: eBook 빌더 번역 진행 추가, 성공 후 아름다운 콘페티
...
- 버전: 이제부터 매월 마이너 버전 번호를 변경합니다. 예를 들어, 3월에는 버전이 0.3.0에서 시작하고, 4월에는 0.4.0에서 시작합니다. 이는 확장이 의미론을 따르는 것이 의미가 없기 때문이며, 시간의 법칙에 따라 버전 번호를 표준화하는 것이 개발을 계속 업데이트하는 동기부여가 되고, 사용자가 문제를 더 쉽게 찾을 수 있도록 하기 위함입니다.
...
- 기능: 일부 사이트가 본문을 메인 프레임으로 사용하지 않기 때문에 사용자 지정 메인 프레임 지원
