...
- 추가: OpenRouter 번역 서비스
- 수정: 웹페이지 번역 중 일부 문단이 번역되지 않는 문제
- 수정: Netflix 자막 병합 문제
- 호환: Mac "360 Extreme Browser Pro" YouTube 자막 표시 문제
- 개선: 플로팅 볼 스타일 업데이트
...
- 추가: Deepseek, Gemini, Claude가 컨텍스트 전환 지원
...
- 개선: Twitter의 경우, 다중 라인 콘텐츠 번역이 기본적으로 줄 바꿈되지 않도록 설정. 콘텐츠가 10줄 또는 1000자를 초과할 때만 줄 바꿈. 줄 바꿈은 설정 **고급 설정** -> **긴 단락에 대한 자동 줄 바꿈 활성화**를 통해 활성화 가능
...
- 개선: 이중 언어 모드에서 리치 텍스트 번역이 기본적으로 꺼져 번역 품질 개선
- ~~개선: "**사이드바 및 네비게이션 바 번역 활성화**"를 "**고급 설정**"에서 사용자 정의할 수 있는 옵션 추가~~
- 개선: "**호버 - 즉시 이 문단 번역**" 모드에서 이미지는 더 이상 번역되지 않음

## 1.11.4 (2024-11-16)

- 수정됨: 버전 1.11.2의 "Twitter Translation Improvement"로 인해 발생한 수식 번역 문제.
...
- 추가됨: 실시간 회의 **Subtitles Translation**이 이제 "float ball"을 통해 활성화 가능, Zoom, Google Meet, Microsoft Teams에서 사용 가능.
- 수정됨: 광고 시청 후 YouTube에서 자막 동기화 문제.
- 수정됨: MacOS 15의 Safari에서 우클릭 번역 메뉴 표시 문제.
- 수정됨: 특정 웹사이트에서 **Enhanced input**의 Ctrl+Z 실행 취소 기능 문제.
...
- 추가됨: 이미지 번역 (Beta 버전)
- 추가됨: 마우스 지원 강제 활성화 모드 (태블릿 장치에서 마우스 오버 기능이 불가능할 때만 이 기능 활성화) **Settings** -> **Advanced Settings** -> **Forece Enable Mouse Support**
...
- Baidu, Gmail 및 기타 입력 필드에 대한 Enhanced input 지원
...
- 기본적으로, 이중 언어 비교 또는 번역 전용 표시를 위해 패널의 아이콘을 클릭하여 전환한 모드를 시스템이 기억함. 일시적으로 전환하려면 패널에서 "More" -> "Switch to translation-only display"를 클릭.
- 기본적으로, 간체 중국어를 번체 중국어로 번역하거나 그 반대로 번역할 때 이중 언어 비교 모드가 아닌 번역 전용 모드를 사용함.
- 몇 가지 버그 수정.
...
- 단락 내 코드 블록 최적화로 원본 텍스트의 보다 정확한 복원.
- 고급 설정에서 "번역 불가 단어" 처리 개선.

## 1.8.2

- 이제 입력 상자에서 우클릭으로 텍스트 번역 가능: 웹페이지의 입력 상자에서 텍스트를 선택하고 우클릭하여 번역을 선택하면 몰입형 번역이 자동으로 선택한 텍스트를 입력 상자의 대상 언어로 번역하여 입력 상자에서 모국어 텍스트를 다른 언어로 빠르게 번역할 수 있음.
- 이제 몰입형 번역의 플로팅 볼에서 웹페이지 번역 문제를 빠르게 보고할 수 있음. 웹페이지를 번역한 후 문제가 있으면 플로팅 볼 오른쪽의 [피드백] 버튼을 클릭하여 문제 설명을 작성하면 최대한 빨리 처리할 것임.
...
- AI 전문가 사용자 정의 지원, [Settings]->[AI Expert] 페이지 하단에 입구.
- TED 사이트에서 자막 로딩 최적화
...
- YouTube 이중 언어 자막이 이제 지능형 문장 분할을 지원 (Beta) (설정 - 비디오 자막에서 YouTube 자막의 몰입형 번역을 수동으로 활성화할 때만, 원본 비디오 자막이 자동 생성된 영어 자막일 때)
...
- 사용자 정의 프롬프트가 너무 많이 사용될 때 비효과적인 문제 수정.
...
- 현재 페이지에만 적용되는 번역 버튼만 표시하는 패널 스위치 수정
...
- 최근 중국 내에서의 Microsoft 번역 서비스 요청이 불안정; 오류 발생 시 시스템이 현재 사용 가능한 번역 서비스를 자동으로 감지하여 사용자가 빠르게 전환할 수 있도록 함.
...
- 수정: Safari에서 밑줄과 같은 번역 스타일이 효과적이지 않은 문제.
...
- 모바일 기기에서 플로팅 볼 패널에 [지정된 단락 번역]에 대한 단축키 구성 추가
...
- **특정 단락 번역에 대한 모바일 지원:** 모바일 버전은 이제 지정된 단락 번역을 지원하며, 왼쪽으로 스와이프, 오른쪽으로 스와이프, 더블 탭, 트리플 탭, 다중 손가락 터치 제스처 등 다양한 단축 작업을 추가했습니다. 기본적으로 활성화되어 있지 않으며, 사용자가 [마우스 호버] 설정 페이지에서 트리거 제스처를 선택해야 합니다.
...
- 최적화: Epub 및 PDF 번역 과정에서 일부 콘텐츠가 번역되지 않을 경우, 패널에서 전체 번역 과정을 다시 시작하지 않고 다른 번역 서비스로 전환할 수 있습니다 (이전 로직은 새로운 번역 서비스를 즉시 사용하여 전체 책을 다시 번역하는 것이었습니다). 즉, 번역 중간에 다른 번역 서비스로 전환하고 [모든 실패한 단락 다시 시도]를 클릭하면 시스템이 새로운 서비스로 번역을 계속 진행합니다.
...
- **플러그인 설정 페이지 UI 업데이트**:
  ...
  - "고급 설정"에 특정 단어(예: "LLM")를 번역에서 제외하도록 사용자가 지정할 수 있는 새로운 옵션 추가.
  - "고급 설정"에 단락을 번역하기 위해 필요한 최소 문자 수를 구성할 수 있는 새로운 옵션 추가. 기본값은 4자이지만, 더 높게 설정할 수 있습니다 (예: 20자), 따라서 더 긴 단락만 번역됩니다.
...
- **openAI 구성 업데이트**: 이제 초당 구성 수를 소수점으로 설정할 수 있습니다. 예를 들어 0.5는 2초에 1번 요청을 의미합니다.
...
- [호버볼 지원이 왼쪽에 고정되도록 구성 #1168](https://github.com/immersive-translate/immersive-translate/issues/1168)
- [학습 모드 표시 문제 수정 #1180](https://github.com/immersive-translate/immersive-translate/issues/1180)
- [일시적으로 웹 번역을 켜도 항상 번역이 취소되지 않는 문제 #1172](https://github.com/immersive-translate/immersive-translate/issues/1172)
- PDF 파일 초기화 문제 최적화.
...
- PDF 번역 도구가 온라인 버전으로 마이그레이션됨: https://app.immersivetranslate.com/pdf/ , Grease Monkey와 Safari가 PDF 번역을 사용할 수 있도록 하며, 문제를 해결하기 위해 버전을 발행할 필요 없이 더 나은 반복 가능
- POPUP UI 최적화, 패널이 더 아름다워짐!

## 0.9.1

- Epub eBook 생성 시 파일명 문제 수정
...
- 마우스 오버 번역 서비스를 개별적으로 설정 가능
- 입력 상자 번역 서비스를 개별적으로 설정 가능
- PDF 번역 최적화
- 일부 버그 수정
...
- Safari의 플러그인 다운로드 제한으로 인해 Safari의 PDF 내보내기 기능을 일시적으로 비활성화 (버그).
- 기타 문제 수정
...
- 브라우저 아이콘 상태 변경 추가, 웹 페이지가 번역되면 브라우저 아이콘의 오른쪽 하단에 ✅가 자동으로 추가됨
...
- 검색 향상 지원, 활성화되면 Google/Google 뉴스에서 중국어로 검색할 때 오른쪽 열에 해당 영어 키워드의 검색 결과가 자동으로 표시되며, 기본적으로 활성화됨
  - 이유: Google 검색에서 중국어 키워드와 영어 키워드의 검색 결과가 매우 다를 수 있으며, Immersive Translated Search Enhancement가 활성화되면 동일한 키워드를 영어로 자동 검색하여 오른쪽에 표시함. 이 기능이 필요하지 않으면 비활성화할 수 있음
  - API 제한으로 인해 Safari는 지원되지 않음

## 0.6.20

- 기본 설정 수정: 많은 사용자가 설치 후 번역 도구를 사용하지 않을 것이라는 피드백을 받았으며, 번역 도구에 대한 기대는 설치 후 영어 웹 페이지를 자동으로 번역하는 것이므로, 이 버전부터 중국 사용자에게 기본적으로 영어 페이지 번역 옵션이 켜져 있음 (사용자가 항상 번역할 언어를 이전에 설정한 경우, 해당 설정이 존중되며, 변경은 확장의 초기 설정만 수정함), 취소하려면 [팝업 패널 또는 설정 페이지](/docs/faq/#%E5%A6%82%E4%BD%95%E5%85%B3%E9%97%AD%E8%87%AA%E5%8A%A8%E7%BF%BB%E8%AF%91)에서 쉽게 취소할 수 있음
...
- PDF 번역 최적화, 번역 영역을 이제 편집/드래그/삭제 가능
...
- 일부 입력 상자 스타일 수정
- 내비게이션 영역인지 스마트하게 판단, 그렇다면 번역하지 않음
- XSS 인젝션 공격 가능성 수정
...
- 문제 수정: 번역 후 이중 언어 eBook에 이미지가 나타나지 않는 문제 해결
- 수정: 번체 중국어에서 openai 번역 문제

## 0.5.15

- 최적화: 번역을 트리거하는 단락의 최소 문자 수를 최소 4자로 수정하여 혼란을 줄이고, 사이트의 탐색 및 끝 영역 번역을 피하기 위해 다른 기능 사용
- 수정: Github 세부사항이 확장 후 번역되지 않는 문제

## 0.5.14

- 문제 수정: 일부 웹 페이지에서 이미지를 복사한 후 커지는 문제
- 수정: Medium 댓글 섹션 번역되지 않는 문제
- 문제 수정: 일부 페이지에서 이미지가 잘못 복사되는 문제
...
- 기능: iOS 앱에 확장 버튼 및 커뮤니티 링크 활성화 추가
...
- 수정: macOS 사파리 확장 단축키
- 수정: 확장에 대한 콘텐츠 보안 정책 팝업, 부분적으로 사용자 스크립트
