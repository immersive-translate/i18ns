...
이 변경 로그는 개발 진행에 따라 업데이트됩니다. 버전 뒤의 날짜는 코드 병합 날짜이며, 앱 스토어에서의 출시 날짜가 아닙니다(각 앱 스토어에 제출 후 검토 시간이 다르며, 일부는 검토에 최대 일주일이 걸릴 수 있습니다). 현재 우리는 두 가지 버전을 진행하고 있습니다.
...
- 개선: 원본 언어 인식 개선. 이전에는 여러 언어가 포함된 페이지가 번역되지 않을 수 있었으나, 이제는 올바르게 번역됨.
- 개선: Twitter의 경우, 다중 라인 콘텐츠 번역이 기본적으로 줄 바꿈되지 않도록 설정. 콘텐츠가 10줄 또는 1000자를 초과할 때만 줄 바꿈이 발생. 줄 바꿈은 설정 **고급 설정** -> **긴 문단에 대한 자동 줄 바꿈 활성화**를 통해 활성화 가능.
...
- 추가: 불완전한 ePub 파일을 다시 가져올 때 번역을 계속할 수 있도록 허용.
...
- 이중 언어 대비 또는 번역 전용 모드에서 언어 및 웹사이트에 대한 예외를 구성하는 기능 지원 (설정 페이지 -> 고급 설정에서 구성). 예를 들어, 기본 번역 모드가 이중 언어 대비인 경우, 번체 중국어가 이중 언어 대비를 사용하지 않도록 하려면 번체 중국어를 이중 언어 대비 예외 언어에 추가하여 번체 중국어가 번역 전용 모드를 사용하도록 할 수 있습니다. 마찬가지로, 기본 번역 모드가 번역 전용인 경우 특정 언어나 웹사이트가 이중 언어 대비 모드를 사용하도록 하려면 해당 언어나 웹사이트를 예외 언어에 추가할 수 있습니다.
...
- Claude 번역은 이제 티베트어를 지원하며 Temperature 매개변수 구성이 가능합니다.
...
- DeepL, Microsoft Translate, Google Translate, OpenAI, Claude, Gemini 등의 번역 서비스는 원본 텍스트 형식을 유지하는 번역을 지원합니다 (예: 링크, 굵게 등).
- 텍스트를 선택한 후 우클릭 메뉴가 [텍스트 번역]으로 변경되며, 클릭하면 몰입형 번역 텍스트 번역 페이지로 자동 이동합니다.
- 모든 사용자가 사용할 수 있는 새로운 대형 모델 무료 번역 서비스: SiliconCloud.
...
- 사용자 정의 AI 전문가 지원, [설정]->[AI 전문가] 페이지 하단에 입구가 있습니다.
- TED 사이트에서 자막 로딩 최적화
...
- YouTube 이중 언어 자막은 이제 지능형 문장 분할을 지원합니다 (베타) ([설정] - [비디오 자막]에서 YouTube 자막의 몰입형 번역을 수동으로 활성화할 때만, 원본 비디오 자막이 자동 생성된 영어 자막일 때만)
...
- AI 전문가가 "Intelligent Selection" 모드를 지원하여, 시스템이 현재 웹사이트에 따라 가장 적합한 AI 전문가를 자동으로 선택합니다 (예: The Verge 및 Hacker News에서는 기술 관련 AI 전문가가 자동으로 선택되며, Twitter에서는 Twitter 번역 향상이 자동으로 선택됩니다).
...
- 수정: Safari에서 밑줄과 같은 일부 번역 스타일이 효과적이지 않은 문제.
...
- **통합 문서 번역 항목:** 이제 PDF, ePub 및 자막 파일을 통합된 업로드 항목을 통해 번역할 수 있습니다. 팝업 패널에서 [PDF/ePub] 버튼을 클릭하기만 하면 되며, 더 이상 [더보기]를 선택할 필요가 없습니다.
...
- **플러그인 설정 페이지 UI 업데이트**:
  ...
  - "고급 설정"에서 단락을 번역하기 위해 필요한 최소 문자 수를 구성할 수 있는 새로운 옵션이 추가되었습니다. 기본값은 4자이지만, 더 높게 설정할 수 있습니다 (예: 20자), 따라서 더 긴 단락만 번역됩니다.
...
- **openAI 구성 업데이트**: 초당 구성 수를 소수점으로 설정할 수 있도록 지원합니다. 예를 들어, 0.5는 2초마다 1개의 요청을 의미합니다.
...
- [일시적으로 웹페이지 번역을 켜고 오랜 시간 후 패널에서 [항상 이 웹사이트 번역]을 클릭해도 항상 번역이 취소되지 않는 문제 #1172](https://github.com/immersive-translate/immersive-translate/issues/1172) 수정.
...
- [일시적으로 웹 번역을 켜고 오랜 시간 후 항상 번역 취소되지 않음 #1172](https://github.com/immersive-translate/immersive-translate/issues/1172)
- PDF 파일 초기화 문제 최적화.
...
- 플러그인 zip 크기를 크게 최적화하여 원래의 절반으로 줄여 다운로드 및 업데이트 속도를 높였습니다.
- 확장된 PDF 다운로드 문제 수정.
...
- Immersive Translate에 대해 번역된 이중 언어 페이지의 공개 링크를 생성할 수 있습니다.
  - [클릭](/docs/share/)하여 Immersive Translate 공유 아이콘을 한 번 클릭으로 생성하세요!
- 일부 플랫폼에서 마우스 지원 여부를 인식하지 못하는 문제 해결.
...
입력 상자 번역의 기본 언어는 이제 공백을 생략할 수 있음, 즉 //Hello World도 번역 가능
...
- 이중 언어 eBook을 내보낸 후 이미지가 없는 문제 수정
...
- 수정: Github 세부 정보가 확장 후 번역되지 않는 문제

## 0.5.14

- 일부 웹 페이지의 이미지를 복사한 후 커지는 문제 수정
...
- 수정: macOS 사파리 확장 단축키
- 수정: 확장에 대한 콘텐츠 보안 정책 팝업, 부분적으로 userscript에 대한 것
...
- 수정: sup, sub 태그 공백 감지
- 기능: 번역 서비스로 ChatGPT Plus 웹사이트 지원, 이는 베타 기능으로 개발자 설정에서 베타 기능을 활성화하여 액세스할 수 있습니다. 매우 느리며, chatgpt는 한 번에 하나의 요청만 보낼 수 있습니다. ChatGPT Plus 계정이 있는지 확인하세요. 무료 계정에는 더 많은 제한이 있으며, 계정에 위험이 있을 수 있으므로 주의해서 사용하세요.
...
- 기능: 로컬 .txt 파일 번역 지원
- 수정: [#273](https://github.com/immersive-translate/immersive-translate/issues/273) 컨텍스트 메뉴가 가끔 사용 불가
...
- 기능: 이제 번역 테마 배경색을 사용자 정의할 수 있습니다.
