...
- 추가: 현재 페이지를 탐색할 때 사전 번역할 화면 수를 설정하는 기능 추가.
...
- 개선: Microsoft, Google, Tencent가 이제 광동어를 지원.
- 개선: BBC가 이제 자막을 지원.
...
- 추가: 번역된 텍스트와 원본 텍스트가 동일한 문자 체계에 속할 때, 전문 스타일로 번역을 표시.
...
- 개선됨: 웹페이지에 포함된 iframe 내 입력 향상 및 마우스 호버 기능

## 1.10.2 (2024-10-11)

- 추가됨: 이미지 번역 (베타 버전)
- 추가됨: 마우스 지원 강제 활성화 모드 (태블릿 장치에서 마우스 호버 기능이 불가능할 때만 이 기능을 활성화) **설정** -> **고급 설정** -> **마우스 지원 강제 활성화**
...
- 기본적으로, 이중 언어 비교 또는 번역 전용 표시를 위해 패널의 아이콘을 클릭하여 전환한 모드를 시스템이 기억함. 일시적으로 전환하려면 패널에서 "더보기" -> "번역 전용 표시로 전환"을 클릭
...
- Google Meet는 이제 실시간 회의에 대한 이중 언어 자막을 지원: 이제 Google Meet 회의에서 이중 언어 자막 기능을 즐길 수 있음. 회의 링크를 열고, 몰입형 번역 패널에서 이중 언어 자막을 활성화한 다음 페이지를 새로 고침하여 경험할 수 있음.
...
- Microsoft Teams 웹 버전 비디오 회의에서 실시간 이중 언어 자막 지원 (Teams 회의 링크를 열고 몰입형 번역 패널에서 이중 언어 자막을 켠 다음 새로 고침).
...
- AI 전문가가 "지능형 선택" 모드를 지원하여, 시스템이 현재 웹사이트에 가장 적합한 AI 전문가를 자동으로 선택합니다 (예: The Verge 및 Hacker News에서는 기술 관련 AI 전문가가 자동으로 선택되며, Twitter에서는 Twitter 번역 향상이 자동으로 선택됩니다).
...
- `Ctrl+C`를 `Ctrl`로 오인하지 않도록 마우스 호버 감도 최적화
...
- **YouTube 이중 언어 자막**: YouTube 비디오 재생 창에 이중 언어 자막을 활성화하거나 숨기기 위한 빠른 액세스가 추가되었습니다 (이 기능은 끌 수 있습니다).
...
- [일시적으로 웹페이지 번역을 오랫동안 켜두면 패널에서 [항상 이 웹사이트 번역]을 클릭해도 항상 번역이 취소되지 않는 문제 #1172](https://github.com/immersive-translate/immersive-translate/issues/1172) 수정.
...
- 확장된 PDF 다운로드 문제 수정.
- [Arxiv](https://arxiv.org/abs/1910.06709) 논문 사이트의 오른쪽에 빠른 번역 PDF 포털 추가, 이는 깨끗한 HTML 페이지로 이동합니다(일부 논문만 지원되며, 원저자가 소스 코드를 제출해야 하므로 약 50%의 논문에서 이 포털이 표시됩니다).
...
- Youtube 자막이 이중 자막 플러그인과 충돌하는 문제 수정(Youtube 이중 플러그인이 감지되면 충돌을 피하기 위해 Youtube 자막 번역이 활성화되지 않음).
...
- 기본 설정 수정: 많은 사용자의 피드백에 따라 설치 후 번역 도구를 사용하지 않으며, 번역 도구에 대한 기대는 설치 후 영어 웹 페이지를 자동으로 번역하는 것이므로, 이번 버전부터 중국 사용자에게 기본적으로 영어 페이지 번역 옵션이 켜져 있음 (사용자가 항상 번역할 언어를 이전에 설정한 경우, 해당 설정이 존중되며, 변경은 확장의 초기 설정만 수정함), 취소하려면 [팝업 패널 또는 설정 페이지](/docs/faq/#%E5%A6%82%E4%BD%95%E5%85%B3%E9%97%AD%E8%87%AA%E5%8A%A8%E7%BF%BB%E8%AF%91)에서 쉽게 취소할 수 있음.
...
- OpenAI 번역이 무의미하게 마침표를 추가하는 문제 최적화, (원본 텍스트에 마침표가 감지되지 않으면, openai가 마침표를 반환하면 제거함)
...
- 유형: // Hello World를 웹 페이지의 입력 상자에 입력한 다음, 스페이스 바를 세 번 클릭하여 단락을 영어로 번역
...
- Fix: 확장 및 일부 userscript에 대한 콘텐츠 보안 정책 팝업.
...
- Fix: Chrome에서 ebook 리더가 이미지를 로드할 수 없음.
...
- Fix: 일부 사이트가 스타일에 `span`을 사용하므로 번역 대상 래퍼에 span 대신 `font` 사용
...
- 번역이 필요한 페이지 영역을 설정할 수 있도록 지원하여 더 많은 영역을 번역할 수 있도록 지원
