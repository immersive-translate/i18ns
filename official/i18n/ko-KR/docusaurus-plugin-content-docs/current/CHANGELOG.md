...
- 개선: Greasemonkey 및 Safari의 소수 언어가 있는 페이지에서 언어 인식 기능 향상
...
- 개선: Twitter의 경우, 다중 라인 콘텐츠 번역이 기본적으로 줄 바꿈되지 않도록 설정. 콘텐츠가 10줄 또는 1000자를 초과할 때만 줄 바꿈. 설정에서 **고급 설정** -> **긴 단락에 대한 자동 줄 바꿈 활성화**를 통해 줄 바꿈 가능
...
- 추가: 불완전한 ePub 파일을 재수입 시 번역을 계속할 수 있도록 허용
...
- Fixed: Facebook의 번역 전용 모드에서 "더 보기"를 클릭한 후 콘텐츠가 사라지는 문제.
- ~~Improved: Twitter에서 다중 언어 번역 표시 개선.~~
- Improved: 패널의 번역 서비스 드롭다운 목록 UI 업데이트.

## 1.11.1 (2024-11-05)

- Added: 실시간 회의 **Subtitles Translation**이 이제 "float ball"을 통해 활성화 가능, Zoom, Google Meet, Microsoft Teams에서 사용 가능.
- Fixed: 광고 시청 후 YouTube에서 자막 동기화 문제.
- Fixed: MacOS 15의 Safari에서 우클릭 번역 메뉴 표시 문제.
- Fixed: 특정 웹사이트에서 **Enhanced input**의 Ctrl+Z 실행 취소 기능 문제.

## 1.10.6 (2024-10-25)

- Fixed: **Enhanced input** 단축키가 작동하지 않는 문제
- Improved: 설치 패키지 크기 감소
- Improved: Netflix 자막 표시 솔루션

## 1.10.5 (2024-10-23)

- Added: 소스 언어와 대상 언어가 동일할 때 경고 표시
- Fixed: 리치 텍스트 공백 문자 번역 문제 [#2175](https://github.com/immersive-translate/immersive-translate/issues/2175)
- Improved: 웹페이지 내 임베디드 iframe에서 입력 향상 및 마우스 오버 기능

## 1.10.2 (2024-10-11)

- Added: 이미지 번역 (베타 버전)
- Added: 마우스 지원 강제 활성화 모드 (태블릿 장치에서 마우스 오버 기능이 불가능할 때만 이 기능 활성화) **Settings** -> **Advanced Settings** -> **Forece Enable Mouse Support**
- Added: 비디오 자막 번역 실패 시 오류 메시지 표시
- Fixed: 리치 텍스트 번역 문제 [#2163](https://github.com/immersive-translate/immersive-translate/issues/2163)
- Improved: PDF 번역 중 번역 버튼이 작동하지 않을 수 있는 문제 해결
- Improved: 번역된 수식의 렌더링 향상
- Improved: 언어 선택 목록

## 1.9.8 (2024-09-28)

- Added: 번역 서비스 "Zhipu BigModel"
- Removed: "SiliconCloud" 모델 qwen1.5-7B-chat (공식 중단으로 인한)
- Fixed: macOS 15의 Safari 플러그인과의 로그인 호환성 문제 해결
...
- 기본적으로, 이중 언어 비교 또는 번역 전용 표시를 위해 패널의 아이콘을 클릭하여 전환한 모드를 시스템이 기억합니다. 임시로 전환하려면 패널에서 "More" -> "Switch to translation-only display"를 클릭하세요.
- 기본적으로, 간체 중국어를 번체 중국어로 번역하거나 그 반대로 번역할 때는 이중 언어 비교 모드가 아닌 번역 전용 모드를 사용합니다.
- 몇 가지 버그 수정.

## 1.9.1 (2024-09-03)

- 이중 언어 대비 또는 번역 전용 모드에서 언어 및 웹사이트에 대한 예외를 구성할 수 있는 지원 (설정 페이지 -> 고급 설정에서 구성). 예를 들어, 기본 번역 모드가 이중 언어 대비인 경우, 번체 중국어가 이중 언어 대비를 사용하지 않도록 하려면 번체 중국어를 이중 언어 대비 예외 언어에 추가하여 번체 중국어가 번역 전용 모드를 사용하도록 할 수 있습니다. 마찬가지로, 기본 번역 모드가 번역 전용인 경우 특정 언어나 웹사이트가 이중 언어 대비 모드를 사용하도록 하려면 해당 언어나 웹사이트를 예외 언어에 추가할 수 있습니다.
- Tiktok 개인 메시지 인터페이스의 입력 상자가 잘못 번역되는 문제 해결
- Read Comic Online의 만화가 번역되지 않는 문제 해결
...
- Epub 파일은 이제 리치 텍스트 번역을 지원합니다 (즉, 각 단락의 원본 텍스트 형식, 예: 링크, 굵게 등을 유지).
- Microsoft Teams 웹 버전 비디오 회의에서 실시간 이중 언어 자막 지원 (Teams 회의 링크를 열고 몰입형 번역 패널에서 이중 언어 자막을 켠 후 새로 고침)
- iQIYI (iq.com) 영어 버전에 대한 이중 언어 자막 최적화
...
- YouTube의 자동 생성 영어 자막에 대한 AI 지능형 문장 분할 알고리즘 채택 [Pro Available]
...
- 사용자 정의 AI 전문가 지원, [Settings]->[AI Expert] 페이지 하단에 입구.
- TED 사이트에서 자막 로딩 최적화
...
- iOS 18에서 플로팅 볼 단축키가 번역되지 않는 문제 수정.
...
- AI 번역 서비스는 이제 모델 목록을 사용자 정의할 수 있으며, 예를 들어 [OpenAI]의 경우 시스템에는 가장 일반적으로 사용되는 모델 몇 개만 내장되어 있습니다. 모델 드롭다운 목록을 클릭하면 마지막 항목으로 [Set More Models]가 표시되며, 설정 후 자동으로 기억되어 사용자가 다양한 사용자 정의 모델을 테스트하는 데 편리합니다.
...
- 번역 서비스는 이제 별도의 탭 아래에 명확하게 정리되어, 사용 가능한 모든 번역 서비스를 포괄적으로 볼 수 있습니다. 또한, 사용자는 표시할 번역 서비스를 사용자 정의할 수 있는 유연성을 가지고 있습니다. 기본적으로 제한된 번역 서비스만 표시되지만, 사용자는 [More Services](https://dash.immersivetranslate.com/#services) 섹션에서 표시 환경 설정을 조정할 수 있습니다.
...
- YouTube 자막 단축 아이콘이 표시되지 않는 문제 수정.
- 확장 언어 인식 최적화.
...
- ePub 생성 시 자리 표시자가 제대로 교체되지 않는 문제 수정
...
- **특정 단락 번역에 대한 모바일 지원:** 모바일 버전은 이제 지정된 단락 번역을 지원하며, 왼쪽으로 스와이프, 오른쪽으로 스와이프, 더블 탭, 트리플 탭, 다중 손가락 터치 제스처를 포함한 다양한 단축 작업을 추가했습니다. 기본적으로 활성화되어 있지 않으며, 사용자가 [마우스 호버] 아래의 설정 페이지에서 트리거 제스처를 적극적으로 선택해야 합니다.
...
- **팝업 패널 UI 업그레이드:** 새로운 디자인은 사용성 및 이해도를 향상시키기 위한 것입니다. 이 업데이트에는 다음이 포함됩니다:
  ...
  - **메인 메뉴의 새로운 기능:**
    - **이중 언어/번역 전용 모드 전환:** 이제 메인 메뉴에서 "이중 언어 번역 모드"와 "번역 전용 모드" 간에 전환할 수 있으며, 번역 버튼 왼쪽에 위치.
    - **문서 번역 항목:** "PDF/ePub/자막 파일" 번역 항목이 메인 메뉴로 이동하여 빠르게 접근 가능.
    - **비디오 번역 설정:** "비디오 번역" 설정 항목도 메인 메뉴에 배치되어 빠르게 조정 가능.
    - **사용 설명서에 대한 새로운 항목:** 자세한 작업 가이드 및 도움말 문서 제공.

- **통합 문서 번역 항목:** 이제 PDF, ePub, 자막 파일을 통합 업로드 항목을 통해 번역할 수 있습니다. 팝업 패널에서 [PDF/ePub] 버튼을 클릭하면 되며, 더 이상 [더보기]를 선택할 필요가 없습니다.
...
- 최적화: Epub 및 PDF 번역 과정에서 일부 콘텐츠가 번역되지 않을 경우, 패널에서 전체 번역 과정을 다시 시작하지 않고 다른 번역 서비스로 전환할 수 있음 (이전 로직은 즉시 새로운 번역 서비스를 사용하여 전체 책을 다시 번역하는 것이었음). 즉, 번역 중간에 다른 번역 서비스로 전환하고 [모든 실패한 단락 다시 시도]를 클릭하면 시스템이 새로운 서비스로 번역을 계속 진행.
...
- **신규 사용자 가이드**: 신규 사용자가 비대상 언어로 페이지를 처음 열 때, 떠다니는 도움말 가이드 버블이 표시됩니다.

### 최적화 및 수정

- **UI 최적화**: 페이지 번역 오류 알림의 UI를 재설계하여 이해하기 쉽게 만들었습니다. 오류가 많을 경우, 팝업이 사용자에게 적극적으로 알립니다.

- **버그 수정**:

  - 페이지가 포커스를 잃을 때 마우스오버 번역이 실패하는 문제를 수정했습니다.
  - 입력 상자 강화 기능에서 3자 미만의 문자가 번역되지 않는 문제를 수정했습니다.
  - 이중 언어 Epub 제작 중 일부 디렉토리가 번역되지 않는 문제를 수정했습니다.

- **기능 제거**: 이중 언어 정보 강화 기능(구글 검색 페이지에서 영어 검색 결과를 동시에 표시)을 제거했습니다.

### 기타 업데이트

- **openAI 구성 업데이트**: 이제 초당 구성 수를 소수점으로 설정할 수 있습니다. 예를 들어 0.5는 2초마다 1개의 요청을 의미합니다.
...
- [웹페이지 번역을 일시적으로 켜고 오랜 시간 후 패널에서 [항상 이 웹사이트 번역]을 클릭해도 항상 번역이 취소되지 않는 문제 #1172](https://github.com/immersive-translate/immersive-translate/issues/1172) 수정.
...
- 검색 강화(구글 검색의 일부 페이지에서 이중 언어 검색 결과 표시) 기능이 이제 기본적으로 활성화되지 않으며, 다음 Release에서 제거될 예정입니다.
...
- [웹 번역을 일시적으로 켜고 오랜 시간 후 항상 번역이 취소되지 않는 문제 #1172](https://github.com/immersive-translate/immersive-translate/issues/1172)
- PDF 파일 초기화 문제 최적화.
...
- Safari의 시스템 스타일에 맞게 조정, Safari의 확장 아이콘이 투명한 윤곽선으로 변경됨
- 브라우저 아이콘 상태 변경 추가, 웹 페이지가 번역되면 브라우저 아이콘의 오른쪽 하단에 자동으로 ✅가 추가됨
- 새로운 사용자를 위해 자주 사용하는 영어 웹사이트의 번역 자동 활성화
...
- 일부 입력 상자 스타일 수정
- 내비게이션 영역인지 스마트하게 판단, 그렇다면 번역 수행 안 함
- 가능한 XSS 인젝션 공격 수정

## 0.6.8

- 확장 패널이 이제 지원되지 않는 페이지를 표시할 수 있음 (예: 권한이 없는 페이지 및 비-HTML 페이지)
- 입력 상자 향상은 번역 중 로딩 상태를 표시
- 번역의 기본 로딩 색상 업데이트
- 입력 상자가 접두사 없이 구성된 경우, `ja Hello`는 일본어로 번역되고 `English Hello`는 영어로 번역을 지원
...
- 최적화: 번역을 트리거하는 단락의 최소 문자 수를 혼란을 줄이기 위해 최소 4자로 수정하고, 사이트의 탐색 및 끝 영역 번역을 피하기 위해 다른 기능 사용.
...
- 수정: 트윗 부분 줄 바꿈 인식되지 않는 문제
- 수정: eBook 저작 스타일 작동하지 않는 문제

## 0.5.10

- 수정: 트윗 줄 바꿈 인식되지 않는 문제
...
- 수정: eBook 온라인 읽기 스타일 작동하지 않는 문제
...
- 번역만 표시 기능이 도착했습니다! [More] -> [Switch to show translations only] 클릭.
...
- 웹 코어 파싱 리팩토링, 이는 다음을 의미합니다:
  ...
  - 대규모 웹 페이지 번역이 몇 초 만에 완료
    - 예를 들어, https://pve.proxmox.com/pve-docs/pve-admin-guide.html, 이전에는 30초가 걸렸지만 이제는 몇 초 만에 완료됩니다.
...
- 수정: 컨텍스트 메뉴 비활성화 작동하지 않음 [#428](https://github.com/immersive-translate/immersive-translate/issues/428)
...
- 수정: Azure 번역 설정이 필요하지만 여전히 표시됨

## 0.2.66

- 수정: PDF 파일 번역 실패, zh-CN에서 zh-TW로 Deepl 지원을 위한 0.2.60의 버그
...
- 터치 디바이스에서 세 손가락으로 페이지 번역 토글 트리거 지원
...
- 번역 토글을 위한 기본 단축키가 `alt+A`로 변경되었습니다. 이는 가장 편리한 키입니다.

### 기타

- 즉시 번역 모드를 설정할 수 있어 웹 페이지를 최대한 빠르게 번역할 수 있습니다.
