...
Этот журнал изменений обновляется в соответствии с ходом разработки. Дата после версии — это дата слияния кода, а не дата Release в магазинах приложений (время проверки варьируется после отправки в каждый магазин приложений, некоторые могут занять до недели на проверку). В настоящее время мы продвигаем две версии.
...
- [пользовательский скрипт официального сайта](https://download.immersivetranslate.com/immersive-translate.user.js)
...
- Исправлено: Проблема, когда AI translation service неправильно выводила упрощенный китайский при переводе традиционного китайского.
...
- Оптимизировано: Улучшено качество AI translation service для основного контента.
...
- Оптимизировано: Gemini translation service поддерживает пользовательский системный запрос (System Prompt).

## 1.15.2 Release (2025-03-02)

- Добавлено: Gemini поддерживает португальский (Бразилия).
- Добавлено: Grok, Ollama, Groq, Azure-OpenAI translation service.
...
- Оптимизировано: [Бесплатный перевод изображений] поддерживает форматирование для языков справа налево.
...
- Улучшено: Фильтры грязного текста в deepl translation service.
- Исправлено: Неверный перевод боковой панели в расширенных настройках.
- Исправлено: Проблема отображения пользовательского deepseek перевода.
...
- Новое: Профессиональное членство поддерживает DeepSeek (v3) translation service.
...
- Исправлено: Проблема, когда **Mouse Hover Translation** не могла перевести форматированный текст.
...
- Исправлено: Проблема интеллектуального сопоставления AI экспертов в пользовательских службах перевода.
- Исправлено: Проблема синхронизации Google Drive в некоторых браузерах.
- Исправлено: Проблема с переводом комментариев в реальном времени на YouTube.
...
- Новое: Бесплатный перевод изображений теперь доступен (в настоящее время поддерживается только в версиях браузеров Chrome и Edge для ПК), доступен через контекстное меню.
...
- Новое: AI translation service теперь поддерживает норвежский.

## 1.13.6 Preview (2025-01-17)

- Улучшено: **AI Expert** может использоваться с **AI Context-Aware Translation**.
- Улучшено: **Image Translation** теперь совместима с weibo.com (поддерживается только в Chrome и Edge).
...
- Улучшено: Регулировка макета для языков справа налево в панелях и на страницах настроек.

## 1.12.6 (2024-12-26)

- Исправлено: Проблема, когда функция наведения мыши загружает неправильную службу перевода при определенных условиях.
- Исправлено: Проблема, когда временное включение двуязычных субтитров на YouTube не работает.
- Исправлено: После переключения служб перевода служба перевода в "**Enhanced Input Box**" не обновляется.
...
- Улучшено: Обновлен интерфейс выпадающего списка служб перевода на панели.

## 1.11.1 (2024-11-05)

- Добавлено: Перевод субтитров в реальном времени теперь поддерживает активацию через "плавающий шар", доступен на Zoom, Google Meet и Microsoft Teams.
...
- Исправлено: Проблема перевода пробелов в формате rich text [#2175](https://github.com/immersive-translate/immersive-translate/issues/2175)
...
- Добавлено: Режим принудительного включения поддержки мыши (включайте эту функцию только если функция наведения мыши недоступна на планшетных устройствах) **Настройки** -> **Расширенные настройки** -> **Принудительное включение поддержки мыши**
...
- По умолчанию система будет запоминать режим, переключенный нажатием на иконку в панели для двуязычного сравнения или только перевода. Для временного переключения нажмите "Еще" -> "Переключиться на режим только перевода" в панели.
...
- Google Meet теперь поддерживает двуязычные субтитры для живых встреч: теперь вы можете наслаждаться функцией двуязычных субтитров в встречах Google Meet. Просто откройте ссылку на встречу, активируйте двуязычные субтитры в панели иммерсивного перевода, а затем обновите страницу, чтобы испытать это.
...
- Теперь вы можете переводить текст в полях ввода с помощью правой кнопки мыши: выберите любой текст в поле ввода на веб-странице, щелкните правой кнопкой мыши, чтобы выбрать перевод, и иммерсивный перевод автоматически переведет выбранный текст на целевой язык поля ввода, что удобно для быстрого перевода текста на родном языке в полях ввода на другие языки.
- Теперь вы можете быстро сообщать о проблемах перевода веб-страниц в плавающем шаре иммерсивного перевода. После перевода веб-страницы, если есть какие-либо проблемы, вы можете нажать кнопку [Обратная связь] на правой стороне плавающего шара, заполнить описание проблемы, и мы решим ее как можно скорее.
...
- Поддержка двуязычных субтитров в реальном времени в видеовстречах веб-версии Microsoft Teams (откройте ссылку на встречу Teams, включите двуязычные субтитры в панели иммерсивного перевода, а затем обновите)
...
- Исправлены проблемы перевода с помощью скрипта Tampermonkey SiliconCloud
...
- После выбора текста, контекстное меню изменится на [Перевести текст], нажав на которое вы можете автоматически перейти на страницу текстового перевода иммерсивного перевода
...
- Поддержка интеграции иммерсивного [JS SDK](https://immersivetranslate.com/docs/js-sdk/) для сторонних веб-сайтов
- Оптимизировано отображение субтитров Hulu
- Поддержка перевода субтитров в веб-версии ZOOM

## 1.7.6

- Поддержка настройки AI экспертов, вход находится внизу страницы [Настройки]->[AI Эксперт].
...
- Двуязычные субтитры YouTube теперь поддерживают интеллектуальное разделение предложений (бета) (только при ручном включении иммерсивного перевода субтитров YouTube в [Настройки] - [Видео субтитры], и оригинальные субтитры видео являются автоматически сгенерированными английскими субтитрами)
- Добавлен сервис перевода Tencent [【Hunyuan Large Model】](https://immersivetranslate.com/docs/services/tencent-hunyuan/)

## 1.6.5

- Исправлены проблемы с компоновкой текста перевода комиксов для языков на латинском алфавите.
...
- Когда AI эксперты используются для "Интеллектуального выбора", разные AI эксперты могут быть настроены для разных веб-сайтов. Это можно настроить в [Настройки] -> [AI Эксперты] -> [Введите любого эксперта].
...
- Все пользователи могут [внести вклад онлайн](https://weblate.immersivetranslate.com/projects/immersive-translate/extension/) в многоязычный перевод интерфейса иммерсивного перевода.
...
- Двуязычные субтитры YouTube поддерживают настройки шрифта и веса шрифта
...
- Поддержка независимых настроек активации для двуязычных субтитров видео YouTube (включено по умолчанию для новых пользователей)
...
- Контроль стиля субтитров YouTube поддерживает стилизацию краев
...
- Сервис перевода Gemini скрыт по умолчанию, его можно включить в настройках разработчика для отображения бета-версии этого сервиса перевода
...
- Недавние запросы на перевод от Microsoft из Китая были нестабильны; при возникновении ошибок система автоматически обнаруживает доступную в данный момент службу перевода, позволяя пользователям быстро переключаться.
- Оптимизировано сообщение об ошибке для сетевых ошибок
- Исправлена проблема, из-за которой настройка цвета текста не поддерживала RGBA предварительный просмотр [#1435](https://github.com/immersive-translate/immersive-translate/issues/1435)
...
- Поддержка настройки определенных веб-сайтов/языков для отображения только переведенного текста.
...
- **Поддержка перевода определенных абзацев на мобильных устройствах:** Мобильная версия теперь поддерживает перевод указанных абзацев и добавила различные ярлыки, включая смахивание влево, смахивание вправо, двойное нажатие, тройное нажатие и жесты с несколькими пальцами. Эти функции не включены по умолчанию и требуют активного выбора пользователем жеста срабатывания на странице настроек в разделе [Наведение мыши].
- **Обновление версии по умолчанию для Gemini:** Версия по умолчанию теперь `v1beta`.
- **Исправление перевода классического китайского языка:** Исправлена функциональность перевода классического китайского языка от Microsoft и OpenAI.
- **Оптимизация японского перевода:** Дальнейшая оптимизация японского перевода от OpenAI для улучшения точности и плавности.
- **Иммерсивный опыт перевода:** Для лучшей адаптации к пользовательским привычкам мы переместили ярлык для двуязычных субтитров в полноэкранном режиме на платформе YouTube влево.
...
- Исправление: Проблема, из-за которой переключение служб перевода не вступало в силу, когда страница еще не была переведена.
...
- **Исправления ошибок**:
  ...
  - Исправлена проблема, из-за которой перевод при наведении мыши не срабатывал, когда страница теряла фокус.
  - Исправлена проблема, из-за которой менее 3 символов в функции улучшения полей ввода не переводились.
...
- **openAI Configuration Update**: Теперь поддерживается установка количества конфигураций в секунду в десятичных дробях, например, 0.5, что означает 1 запрос каждые 2 секунды.
...
- Исправлено: Проблема с сшиванием длинных абзацев в OpenAI в некоторых случаях. [#1276](https://github.com/immersive-translate/immersive-translate/issues/1276)
...
- Оптимизация скорости экспорта только перевода PDF, если вы выбираете экспорт только перевода, вы можете напрямую вызвать системный PDF-просмотр для экспорта, быстрее.
- Deeplx поддерживает несколько URL, просто разделите их с помощью .

## 0.9.2

- Инструмент перевода PDF перенесен на онлайн-версию: https://app.immersivetranslate.com/pdf/ , чтобы Grease Monkey и Safari могли использовать перевод PDF, и проблемы можно было бы лучше итеративно решать без необходимости выпуска версии для решения проблемы.
...
- Поддержка PDF регулировки межстрочного и межсловного интервала для повторного распознавания абзацев
...
- Исправление: проблема с переводом всех регионов Gmail
...
- Добавление изменения статуса значка браузера, когда веб-страница была переведена, в правом нижнем углу значка браузера автоматически добавляется ✅.
...
- Поддержка перевода результатов с отменой ctrl+z в режиме улучшенного ввода
...
- Оптимизация: минимальное количество символов в абзаце, которое вызывает перевод, было изменено на минимум 4 символа для уменьшения путаницы, при этом используя другие функции, чтобы избежать перевода навигационных и конечных областей сайта.
...
- Fix: состояние ввода переключения сервисов перевода в Options UI не изменилось.
...
- Feat: обновить темный значок на прозрачный.
...
- Feat: Поддержка перевода веб-страниц txt
...
- Функция: Добавить меню Включить/Отключить для всплывающего окна -> больше
...
- Функция: поддержка OpenAI CHATGPT 3.5 (поддерживает интерфейс OpenAI ChatGPT 3.5)
- Функция: Добавить новую тему Solid Border (новая тема, сплошная граница)
...
- Функция: Поддержка настройки количества текста для немедленного перевода для различных сервисов перевода.

## 0.2.74

- Функция: Поддержка Tencent (alpha)
- Исправление: перевод openai
- Исправление: проверка неизвестных тегов inline

## 0.2.73

- Функция: Поддержка серой темы перевода
...
- Функция: Поддержка Open AI в качестве сервиса перевода.
...
- Функция: Добавить `<all>` для всегда переводимых языков, теперь вы можете использовать его для перевода всех языков, кроме целевого языка, и никогда не переводимых языков
- Исправление: Разрешить настройку пользовательского API Google
...
- Производительность: последовательные запросы к конкурентным
- Улучшение для определения количества японских символов

## 0.2.62

- Функция: Добавить правило waitForSelectors, для исправления некоторых сайтов, таких как reddit
...
- Функция: Поддержка zh-CN на zh-TW для Deepl
- Функция: Функция Immersive Translate Deepl
- Функция: Поддержка настройки масштаба шрифта
...
- Функция: Поддержка настройки цвета текста перевода, цвета границы.
...
- Исправление проблемы загрузки на Google Drive (действительно) [#81](https://github.com/immersive-translate/immersive-translate/issues/81)

## 0.2.49

- Удаление горячих клавиш alt+d, alt+s, так как они могут конфликтовать с родными горячими клавишами.
- Исправление проблемы загрузки на Google Drive [#81](https://github.com/immersive-translate/immersive-translate/issues/81)
...
- Исправление игнорирования тегов rb, rt на японском языке.
...
- Исправление высоты линии выделения
...
- Исправить исключение элементов
...
- Изменить образец pdf файла

## 0.0.62

- Исправить формат pdf, отступы.
- Исправить изменение элемента на telegra.ph
...
- Исправить ошибку разделения предложений.

## 0.0.55

- Исправить отображение элемента с none.
- рефакторинг метки элемента

## 0.0.54

- Поддержка переноса строк для X символов.
...
- Исправить проверку inline элементов
- Добавить опцию журнала отладки для разработчиков

## 0.0.39

- Исправить службу перевода, содержащую mock2
...
- Лучше для темы с пунктиром

## 0.0.29

- Исправить допустимость абзаца.
- Добавить темы dotted, thinDashed
- Лучше для тем с пунктиром, выделением
...
- Исправить разрешение на подключение userscript volc alpha.
...
- Лучше для обнаружения китайского языка
