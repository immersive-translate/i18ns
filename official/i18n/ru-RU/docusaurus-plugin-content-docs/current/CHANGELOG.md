...
- Исправлено: игнорирование тегов rb, rt на японском языке.
...
- Поддержка перевода shadowroot. Например:
  ...
  Область разговора.
- также проверить правило синхронизации во всплывающем окне
...
- Исправить safari globalThis.GM не работает

## 0.2.13

- Поддержка перетаскивания всплывающего окна Userscript
- Поддержка трех пальцев на сенсорных устройствах для переключения перевода страниц
...
- использовать sendMessage вместо connect, потому что chrome отключит порт через
  5 минут
...
- Исправить размер шрифта интерфейса опций
...
- Исправить youtube переход на следующую страницу
...
- Горячая клавиша по умолчанию для переключения перевода изменена на `alt+A`, потому что
...
- Улучшено для обнаружения китайского языка
