...
- Добавлено: Управление AI глоссарием (поддерживаются только AI переводческие сервисы)
...
- Добавлено: OpenRouter переводческие сервисы
- Исправлено: Проблема, когда некоторые абзацы не переводились при переводе веб-страницы
- Исправлено: Проблема с объединением субтитров на Netflix
- Совместимость: Проблема отображения субтитров YouTube в Mac "360 Extreme Browser Pro"
...
- Новое: Поддержка Pro членства для сервиса перевода DeepSeek (v3).
...
- Новое: Доступен бесплатный перевод изображений (в настоящее время поддерживается только в версиях для ПК браузеров Chrome и Edge), доступный через контекстное меню.
...
- Добавлено: Когда переведенный текст и оригинальный текст принадлежат к одной и той же системе письма, перевод отображается с использованием специализированного стиля.
...
- Улучшено: Улучшены описания для традиционного китайского языка интерфейса.
- Улучшено: Регулировка макета для языков с письмом справа налево в панелях и на страницах настроек.

## 1.12.6 (2024-12-26)

- Исправлено: Проблема, когда функция наведения мыши загружала неправильный переводческий сервис при определенных условиях.
...
- Добавлено: Перевод субтитров в реальном времени теперь поддерживает активацию через "float ball", доступно в Zoom, Google Meet и Microsoft Teams.
...
- Добавлено: Отображение сообщения об ошибке при сбое перевода субтитров видео
...
- После выбора текста, меню правого клика изменится на [Перевести текст], нажав на которое вы автоматически перейдете на страницу перевода текста Immersive Translation
...
- Исправлены проблемы с макетом текста в переводах комиксов для языков на латинице.
...
- Исправлена проблема, когда изменения в общем экспертном запросе переопределяли указанный AI-экспертный запрос [#1692](https://github.com/immersive-translate/immersive-translate/issues/1692)
...
- Исправлена проблема, когда перевод не удавался при переключении пользовательских запросов OpenAI под тестовой кнопкой
- Исправлена проблема, когда значок ярлыка субтитров YouTube не отображался
- Оптимизировано распознавание языка расширения

## 1.4.6

- Исправлена проблема, когда пользовательские запросы были неэффективны
...
- Варианты панели "больше" теперь поддерживают [ввод текста для перевода](https://app.immersivetranslate.com/text/)

## 1.4.2

- Поддержка сервиса перевода Claude
- Оптимизированы многозапросные слова OpenAI, поддерживающие формат YAML, что улучшает гибкость и удобство использования конфигурации
...
- **Погружающий переводческий опыт:** Для лучшей адаптации к пользовательским привычкам мы переместили ярлык для двуязычных субтитров в полноэкранном режиме на платформе YouTube влево.
...
- **Оптимизация UI**: Переработан интерфейс для сообщений об ошибках перевода страниц, чтобы они были более понятными. При большом количестве ошибок всплывающее окно активно уведомляет пользователя.
...
- Исправлена проблема, когда эмодзи в конце поля ввода Twitter не переводятся
...
- Поддержка настройки кнопок быстрого перевода шарика наведения (поддержка как ПК, так и мобильных устройств)
...
- Исправлена проблема с не переводом текста при нажатии на "показать больше"
...
- Исправлена автоперевод при выделении в readwise
...
- Поддержка улучшения поиска, когда включено, при поиске в Google/Google News на китайском языке, в правой колонке автоматически будут отображаться результаты поиска по соответствующим английским ключевым словам, что включено по умолчанию.
  - Причина: Мы обнаружили, что в поиске Google результаты поиска по китайским ключевым словам и английским ключевым словам могут сильно отличаться, с включенным улучшением поиска Immersive Translated, мы автоматически ищем те же ключевые слова на английском языке для вас и отображаем их на правой стороне. Вы можете отключить его, если вам не нужна эта функция.
...
- Выравнивание выпадающего списка влево в Windows
...
- Оптимизация проблемы с добавлением точек в переводах OpenAI, (если в оригинальном тексте не обнаружена точка, если openai возвращает точку, то удалите ее)
...
- Исправлено: настройки горячих клавиш наведения мыши для userscript
...
- Исправлено: переключатель UI параметров переводческих сервисов не изменял состояние ввода.
...
- Исправлено: настройка перевода Azure, но все равно показывается (нужна настройка)
...
- Исправлено: синхронизация Google Drive на native firefox android.
...
- Улучшено для обнаружения японского количества
...
- Новая функция: Поддержка настройки масштаба размера шрифта
...
- Исправлено: ошибка синхронизации userscript с Google Drive.
...
- Исправлено: переключение маски userscript с помощью сенсорного события.
...
- Исправлено: перевод userscript niu
...
- Поддержка перевода shadowroot. Например:
  ...
  Область разговора.
- также проверьте правило синхронизации на всплывающем окне
...
- Исправлено: онлайн перевод pdf на firefox
...
- Исправить проблему с полным переводом текста
...
- Исправить, что некоторые теги span являются блочными элементами
