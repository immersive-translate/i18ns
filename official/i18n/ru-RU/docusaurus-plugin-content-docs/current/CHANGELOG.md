...
**Release версия** — это официальная стабильная версия, доступная в основных магазинах приложений, таких как
...
[Apple](https://apps.apple.com/app/id6447957425) и т.д.
...
- Добавлено: Поддержка переключения контекста для Deepseek, Gemini, Cluade.
- Исправлено: Обновление терминов не отправляет новый запрос на перевод.
- Улучшено: Интерфейсный язык добавляет венгерский.
...
- Новое: Поддержка сервиса перевода DeepSeek (v3) для Pro подписки.
...
- Исправлено: Решена проблема, когда часть контента пропускалась при многосегментном переводе в Gemini.
...
- Улучшено: Для Twitter переводы многострочного контента по умолчанию не переносятся. Перенос будет происходить только при превышении 10 строк или 1000 символов. Перенос можно включить через настройки **Advanced Settings** -> **Enable automatic line wrapping for long paragraphs**.
...
- Улучшено: Корректировка макета для языков с письмом справа налево в панелях и на страницах настроек.
...
- Добавлено: Принудительное включение режима поддержки мыши (включите эту функцию только если функция наведения мыши недоступна на планшетных устройствах) **Settings** -> **Advanced Settings** -> **Forece Enable Mouse Support**.
...
- По умолчанию система будет запоминать режим, переключенный нажатием на иконку в панели для двуязычного сравнения или отображения только перевода. Чтобы временно переключиться, нажмите "More" -> "Switch to translation-only display" в панели.
...
- Теперь вы можете быстро сообщать о проблемах перевода веб-страниц в плавающем шаре погружающего перевода. После перевода веб-страницы, если есть какие-либо проблемы, вы можете нажать кнопку [Feedback] на правой стороне плавающего шара, заполнить описание проблемы, и мы решим ее как можно скорее.
...
- Предоставлено больше статей arXiv с оптимизированной раскладкой двуязычного перевода
...
- Исправлены проблемы с переводом форматированного текста для таких сервисов перевода, как Google, DeepL (например, страницы, непосредственно отображающие `<button>` и т.д.)
...
- После выбора текста, контекстное меню изменится на [Translate the text], нажав на которое вы можете автоматически перейти на страницу текстового перевода погружающего перевода
...
- Двуязычные субтитры YouTube теперь поддерживают интеллектуальное разделение предложений (Бета) (только при ручном включении погружающего перевода субтитров YouTube в [Settings] - [Video Subtitles], и оригинальные субтитры видео автоматически сгенерированы на английском языке)
...
- Новая функция: перевод манги (Бета), на поддерживаемых сайтах манги, кнопка перевода манги появится под плавающим шаром быстрого перевода на веб-странице. Нажав на нее, вы активируете перевод манги. Эта функция доступна для Pro участников (500 страниц в месяц, можно приобрести дополнительные пакеты), в настоящее время поддерживаются следующие сайты:
...
- AI эксперты поддерживают режим "Интеллектуальный выбор", когда система автоматически выбирает наиболее подходящего AI эксперта в зависимости от текущего веб-сайта (например, для The Verge и Hacker News автоматически выбираются AI эксперты, связанные с технологиями, а для Twitter автоматически выбирается улучшение перевода Twitter).
...
- Страница настроек теперь позволяет регулировать [стили субтитров YouTube](https://dash.immersivetranslate.com/#subtitle).
...
- Платформа видео субтитров поддерживает [DeepLearning.ai](https://learn.deeplearning.ai)
...
- Исправлена проблема, когда некоторые традиционные китайские субтитры на YouTube не могли быть отображены правильно
...
- Исправлена проблема, когда placeholders в создании ePub не заменялись должным образом
...
- Недавние запросы на перевод Microsoft из Китая были нестабильными; при возникновении ошибок система автоматически обнаруживает доступную в данный момент службу перевода, позволяя пользователям быстро переключаться.
...
- Поддержка перевода видеосубтитров для [Kanopy](https://www.kanopy.com/), [RachelsEnglishAcademy](https://www.rachelsenglishacademy.com/), [Hulu](https://www.hulu.com/), [Three.js Journey](https://threejs-journey.com/).
...
- Поддержка перевода видеосубтитров на платформах [LinkedIn](https://www.linkedin.com/) и [Viu](https://www.viu.com/).
- Добавлен быстрый доступ к видеосубтитрам для дополнительных платформ.
- Поддержка настройки отображения только переведенного текста для определенных веб-сайтов/языков.
...
- **Погружающий переводческий опыт:** Чтобы лучше адаптироваться к пользовательским привычкам, мы переместили ярлык для двуязычных субтитров в полноэкранном режиме на платформе YouTube влево.
...
- **Оптимизация пользовательского интерфейса**: Переработан пользовательский интерфейс для подсказок об ошибках перевода страницы, чтобы сделать их более понятными. Когда возникает много ошибок, всплывающее окно активно уведомляет пользователя.
...
- **Обновление конфигурации openAI**: Теперь поддерживается установка количества конфигураций в секунду в десятичных дробях, например, 0.5, что означает 1 запрос каждые 2 секунды.
...
- Оптимизировано: При использовании наведения мыши, проблема, когда потеря фокуса на странице и затем повторное срабатывание становятся неэффективными.
...
- Исправлена проблема, когда мышь нависала для продолжения перевода без удержания клавиши быстрого доступа в некоторых случаях.
...
- Добавлены двуязычные субтитры для поддержки платформ [TED](https://www.ted.com), [Frontend Masters](https://frontendmasters.com/), [edx](https://www.edx.org/), [CodeWithChris](https://www.edx.org/), [Skillshare](https://www.ted.com) https://learn.codewithchris.com/enrollments), [Skillshare](https://www.skillshare.com/).
...
- Исправление для функции [постоянно отключить субтитры видео] не работает [#1175](https://github.com/immersive-translate/immersive-translate/issues/1175).
...
- Исправлено отсутствие реакции на нажатие hoverball в онлайн PDF.
...
- Исправлена проблема, когда нажатие на "показать больше" не переводилось.
...
- Исправление проблемы автоматической прокрутки в мобильном режиме онлайн-чтения epub
...
- Поддержка перевода субтитров для экспорта только переводов
...
- Исправление: на странице деталей Reddit возвращаются некоторые абзацы, которые не могут быть переведены
...
- Исправлено: проблема с всплывающим окном новой вкладки на macOS.
...
- Новая функция: Bing Translate на Microsoft Translate для более стабильного качества.
...
- Исправлено: проблема с расширением safari на iOS 15.0.
- Исправлено: горячие клавиши расширения safari на macOS
...
- Новая функция: поддержка ChatGPT Plus Website в качестве сервиса перевода, это бета-функция, поэтому вы можете получить к ней доступ, включив Beta Feature в настройках разработчика. Это очень медленно, потому что chatgpt может отправлять только один запрос за раз. Убедитесь, что у вас есть ChatGPT Plus Account, потому что на бесплатном аккаунте больше ограничений, и я не уверен, несет ли это риск для вашего аккаунта, будьте осторожны, чтобы использовать его.
...
- UI: по умолчанию сворачивать пользовательские опции openai.
- UI: Добавление статуса экспорта для экспорта epub.
- Исправлено: Gpt4 стандартный запрос

## 0.3.12

- Новая функция: Теперь мы можем настраивать цвет фона темы перевода Marker.
...
- Исправлено: переключатель состояния ввода сервисов перевода в UI Options.

## 0.3.8

- UI: цвет загрузки более прозрачный
- Исправлено: Обнаружение языка электронной книги.
- Новая функция: Добавление прогресса перевода для конструктора электронных книг и красивого конфетти после успеха.
- Новая функция: Добавление повторной попытки для всех неудавшихся абзацев для кнопки повторной попытки.
...
- Новая функция: добавление пользовательской конечной точки API для OpenAI
- Новая функция: добавление временных опций времени перевода сайта в `Advanced Settings`
...
- Новая функция: OpenAI поддерживает пользовательский apiUrl для настроек разработчика.

## 0.3.1

- Новая функция: обновление темного значка до прозрачности.
...
- Добавление опции максимальной длины текста для каждого запроса с Open AI
...
- Исправлено: ограничение на максимальное количество токенов OpenAI, изменение максимальных символов с 1500 до 1300.
...
- Новая функция: Добавление меню Включить/Отключить для всплывающего окна -> больше
...
- Новая функция: Добавление новой темы Solid Border (新增新主题，实线边框)

## 0.2.77

- Исправление: ошибка с множественными тегами кода [#178](https://github.com/immersive-translate/immersive-translate/issues/178)
...
- Исправление: API браузера для userscript stay.app
- Исправление: страница опций для stay.app

## 0.2.23

- Исправление: перевод страницы на нескольких языках
...
- Исправление
  ...
  проблема с совпадением URL файла
...
- Исправление соответствия regex
...
- Исправление отображения элемента с display none.
...
- Переписывание сообщений с фоном, использование connect вместо sendMessage
- Добавление поддержки мобильной версии Reddit
...
- Использование одного файла для userscript.
...
- Исправление проверки inline элементов
...
- Исправление перевода содержимого постов в мобильной версии Reddit.

## 0.0.36

- Исправление специальных символов в Википедии
...
- Исправление принудительного обновления поиска Google
...
- Исправление пустых мест в Википедии
...
- Улучшение обнаружения китайского языка
