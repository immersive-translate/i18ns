...
- Добавлено: Перевод **Subtitles Translation** в реальном времени теперь поддерживает активацию через "плавающий шар", доступен на Zoom, Google Meet и Microsoft Teams.
...
- Двуязычные субтитры YouTube теперь поддерживают интеллектуальное разбиение предложений (бета) (только при ручном включении immersive перевода субтитров YouTube в [Настройки] - [Видео субтитры], и оригинальные субтитры видео автоматически сгенерированы на английском языке)
...
- Оптимизация: Страница настроек, взаимодействие с пользовательским интерфейсом списка URL (чтобы избежать двусмысленности, флажки по умолчанию не отображаются).
...
- **Обновление пользовательского интерфейса всплывающей панели:** Новый дизайн направлен на улучшение удобства использования и понимания. Это обновление включает:
  ...
  - **Новые функции в главном меню:**
    - **Переключение режима двуязычного/только перевода:** Теперь вы можете переключаться между "Режимом двуязычного перевода" и "Режимом только перевода" непосредственно в главном меню, расположенном слева от кнопки перевода.
...
- **Оптимизация пользовательского интерфейса**: Переработан пользовательский интерфейс для подсказок об ошибках перевода страницы, чтобы сделать их более понятными. Когда ошибок много, всплывающее окно активно уведомляет пользователя.
...
- Исправлена проблема, когда наведение мыши продолжало переводить без удержания клавиши быстрого доступа в некоторых случаях.
...
- Deeplx поддерживает несколько URL, просто разделите их .
...
- Исправление проблемы входа в Google на Safari
...
- Arigato Translator поддерживает настройку полей и id терминологии
...
- Оптимизация перевода PDF, теперь область перевода можно редактировать/перетаскивать/удалять
  - Перетаскивание влево сверху, удаление вправо сверху, изменение размера снизу справа
...
Теперь язык по умолчанию для перевода в поле ввода может опустить пробел, т.е. //Hello World также может быть переведено.
...
- Fix: проблема с конкретными символами в конструкторе eBook, например, некоторые пути к главам `xxx' xxxx`.
- UI: свернуть пользовательские опции openai по умолчанию.
- UI: Добавить статус экспорта для экспорта epub.
- Fix: Gpt4 запрос по умолчанию
...
- Fix: лучше для разбиения длинного абзаца, 1.5 миллиардов, 25.5%, Mr. не будут считаться границей
...
- Исправление: ошибка нескольких тегов кода.[#178](https://github.com/immersive-translate/immersive-translate/issues/178)
...
- Исправление: проблема с проверкой сетевого сервиса перевода на firefox mobile.
...
- Исправление: советы по ярлыкам всплывающих окон
...
- Удаление ярлыков alt+d, alt+s, так как они могут конфликтовать с родными ярлыками.
...
- Исправление: userscript переключение маски с помощью сенсорного события.
- Исправление скорости (удаление sleep(300))

## 0.2.42

- Исправление: маска при наведении при повторном переключении маски.
- Добавление ярлыков маски для мобильных устройств
...
- Поддержка ярлыков для отображения маски перевода
- Поддержка включения бета-функции на панели разработчика
- Исправление ярлыков в мобильном расширении
...
- Исправление исключения метки фрейма
...
- Поддержка перевода в iframe, как в QQ mail, встроенный твит.
