...
- Добавлено: **AI Context-Aware Translation** может улучшить точность перевода профессионального контента. (Доступно только для профессиональных пользователей, поддерживается исключительно моделями OpenAI) **Options** -> **General** -> **Enable AI Context-Aware Translation**
...
- Исправлено: Проблема с неэффективностью горячих клавиш при включении или выключении функции "**Hover Translation**".
...
- Исправлено: Проблема с не срабатыванием горячих клавиш в **Enhanced input**.
...
- По умолчанию система будет запоминать режим, переключенный нажатием на значок в панели для двуязычного сравнения или только перевода. Для временного переключения нажмите "Еще" -> "Переключиться на только перевод" в панели.
...
- После регулировки позиции двуязычных субтитров на YouTube система автоматически запомнит новую позицию.
- Оптимизирована логика кэширования плагина, теперь автоматически очищая кэшированные данные, которым более 30 дней.
...
- Принят алгоритм интеллектуального разбиения предложений AI для автоматически сгенерированных английских субтитров на YouTube [Pro Available]
...
- В видео с двуязычными субтитрами, когда целевой язык — упрощенный китайский, а оригинальный текст — традиционный китайский, оригинальный текст будет автоматически преобразован в упрощенный китайский и наоборот.
...
- Двуязычные субтитры на YouTube поддерживают настройки шрифта и толщины шрифта
...
- Управление стилем субтитров на YouTube поддерживает стилизацию краев
...
- Оптимизированы многопромптовые слова OpenAI, поддерживающие формат YAML, что улучшает гибкость и удобство использования конфигурации
...
- Оптимизированы prompt слова Gemini
- Поддержка настройки отображения двуязычных/оригинальных и переводных субтитров
...
- Исправлена проблема, когда некоторые пользователи Pro не могли изменить настройки в браузере Chrome.
...
- Добавлены быстрые доступы к субтитрам видео для дополнительных платформ.
...
- Добавлены быстрые доступы для включения субтитров видео на Netflix, Ted, Bloomberg, Udemy, Coursera.
...
- **Оптимизация перевода на японский язык:** Дальнейшая оптимизация перевода на японский язык от OpenAI для улучшения точности и плавности.
- **Иммерсивный опыт перевода:** Чтобы лучше адаптироваться к привычкам пользователей, мы переместили ярлык для двуязычных субтитров в полноэкранном режиме на платформе YouTube влево.
...
- **Оптимизация пользовательского интерфейса**: Переработан пользовательский интерфейс для сообщений об ошибках перевода страниц, чтобы сделать их более понятными. Когда возникает много ошибок, всплывающее окно активно уведомляет пользователя.
...
- Исправлено: Проблема с сшиванием длинных абзацев OpenAI в некоторых случаях. [#1276](https://github.com/immersive-translate/immersive-translate/issues/1276)
...
- Теперь шарик наведения скрыт по умолчанию, когда видео на полном экране.
...
- Онлайн-страницы PDF без расширения .pdf теперь могут перейти непосредственно на страницу перевода PDF, нажав на шарик наведения на странице.
...
- Исправлено отсутствие реакции на нажатие шарика наведения онлайн pdf
...
- Поддержка наведения мыши, нажмите Ctrl еще раз, чтобы показать оригинальный текст
...
- Исправлен конфликт субтитров Youtube с плагином Dual subtitle (перевод субтитров Youtube Immersive Translate не включен, когда обнаружен плагин Youtube Dual, чтобы избежать конфликта)
...
- Для страниц статей теперь сначала будет переведен основной текст, а затем остальной контент боковой панели
...
- Инструмент перевода PDF перенесен в онлайн-версию: https://app.immersivetranslate.com/pdf/ , чтобы Grease Monkey и Safari могли использовать перевод PDF, и проблемы можно было бы лучше итеративно решать без необходимости выпускать версию для решения проблемы.
...
- PDF наконец поддерживает экспорт двуязычных версий! Вы можете нажать кнопку [Сохранить], чтобы экспортировать переведенный двуязычный PDF файл.
...
- Feature: Стиль перевода с вертикальной разделительной линией добавляет вертикальную линию для переводов в одну строку
...
- Оптимизация ограничения частоты запросов OpenAI
...
- Feat: Рефакторинг перевода PDF, теперь вы можете переводить большинство PDF с помощью Immersive Translate
...
- Feat: Теперь мы можем настраивать цвет фона темы перевода Marker.
...
- Fix: состояние ввода переключения сервисов перевода в UI Options не изменилось.
...
- Feat: добавить временные параметры времени перевода сайта в `Advanced Settings`
...
- Версия: С этого момента мы будем менять номер минорной версии раз в месяц, например, сейчас март, версия начнется с 0.3.0, в апреле номер версии начнется с 0.3.0, в апреле номер версии начнется с 0.4.0, в следующем апреле номер версии будет 1.4.0 и так далее. Это потому, что не имеет смысла для расширений следовать Это потому, что не имеет смысла для расширений следовать семантике, но стандартизация номеров версий в соответствии с законами времени является мотивацией для развития, чтобы продолжать обновляться, и для пользователей, чтобы легче находить проблемы.
...
- Feat: Разрешить ввод пароля в форме показывать пароль.

## 0.2.82

- Fix: Некоторые сайты используют `span` для стилей, поэтому мы используем `font` вместо span для обертки цели перевода
- Fix: ограничение максимального количества токенов OpenAI, изменение максимального количества символов с 1500 до 1300.
...
- Исправление: проблема с проверкой сетевого подключения сервиса перевода на мобильном Firefox.

## 0.2.71

- Исправление: разрешения userscript для Open AI
...
- Исправление: ошибка перевода PDF-файлов, ошибка из 0.2.60 для поддержки deepl с zh-CN на zh-TW
...
- Исправление: синхронизация Google Drive на Android в Firefox.
...
- Исправление: перевод caiyun не определен исходный язык

## 0.2.44

- Исправление переключения маски в userscript
- Исправление логики переключения маски

## 0.2.43

- Исправление переключения маски в userscript с помощью сенсорного события.
- Исправление скорости (удаление sleep(300))

## 0.2.42

- Исправление наведения на маску при повторном переключении маски.
- Добавление горячих клавиш для маски на мобильных устройствах
- Исправление проблемы с облачной синхронизацией в userscript
- Перемещение страницы расширенных опций в левое меню.
- Добавление логики повторной попытки для сервиса перевода

## 0.2.41

- Исправление перевода niu в userscript
...
- Поддержка вертикального режима письма, как в японском языке.
- Добавление 3 тем
...
- Исправление дополнения Firefox для Android
- Добавление расширенных настроек для перевода новой строки

## 0.2.25

- Поддержка перевода iframe, как в QQ mail, встроенных твитах.

## 0.2.24

- Добавление временного сайта перевода на некоторое время
- Исправление API браузера userscript stay.app
...
- Исправление умного японского языка Tencent
...
- исправить пробелы в некоторых статьях
...
- Исправить специальные символы в Википедии
  [#6](https://github.com/immersive-translate/next-immersive-translate/issues/6)
- Исправить размер иконки userscript.
...
- Исправить пустую страницу Википедии
...
- Поддержка установки режима немедленного перевода, чтобы веб-страница переводилась
  как можно быстрее.
- Поддержка установки области страницы, которую нужно перевести, чтобы можно было перевести
  больше областей.
...
- Исправить валидность абзаца.
- Добавить темы dotted, thinDashed
- Улучшение для тем с пунктиром, выделением
...
- Поддержка selectorMatches, теперь можно сопоставлять все manstondon.
