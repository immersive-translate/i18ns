...
- Добавлено: Браузер Android поддерживает бесплатный перевод изображений. 【Ожидается, необходимо дождаться Release браузера】
- Добавлено: Gemini поддерживает португальский (Бразилия).
- Добавлено: Grok, Ollama, Groq, Azure-OpenAI службы перевода.
...
- Исправлено: Проблема с переводом Google с экранированными символами.
...
- Новое: Служба AI перевода теперь поддерживает норвежский.
...
- Улучшено: Обновлен интерфейс выпадающего списка службы перевода на панели.
...
- Добавлено: Режим принудительного включения поддержки мыши (включите эту функцию только в случае, если функция наведения мыши недоступна на планшетных устройствах) **Настройки** -> **Расширенные настройки** -> **Принудительное включение поддержки мыши**
...
- Улучшено: Исправлены проблемы, из-за которых кнопка перевода могла не работать при переводе PDF
...
- Сервис перевода DeepL теперь официально поддерживает традиционный китайский как целевой язык (ранее перевод на традиционный китайский с помощью DeepL включал дополнительный процесс преобразования с упрощенного на традиционный китайский с помощью стороннего сервиса).
...
- Теперь вы можете быстро сообщать о проблемах с переводом веб-страниц в плавающем шаре погружающего перевода. После перевода веб-страницы, если возникают какие-либо проблемы, вы можете нажать кнопку [Обратная связь] на правой стороне плавающего шара, заполнить описание проблемы, и мы решим ее как можно скорее.
...
- Исправлена проблема, когда двуязычную горячую клавишу для видео на YouTube нельзя было отключить.
...
- После выбора текста, контекстное меню изменится на [Перевести текст], нажав на которое вы автоматически перейдете на страницу текстового перевода погружающего перевода
...
- В панели больше опций теперь поддерживается [ввод текста для перевода](https://app.immersivetranslate.com/text/)
...
- **Новое руководство для пользователей**: Когда новые пользователи впервые открывают страницу на нетаргетном языке, отображается пузырь помощи с плавающим шаром.
...
- Исправлена проблема с дрожанием при нажатии на панель действий hoverball на странице Firefox.
...
- [Поддержка hoverball, настроенная на фиксацию слева #1168](https://github.com/immersive-translate/immersive-translate/issues/1168)
...
- Исправлено, что онлайн pdf hoverball не реагирует на нажатие.
...
- Добавление изменения статуса значка браузера, когда веб-страница переведена, в нижнем правом углу значка браузера автоматически добавляется ✅.
...
- Оптимизировано для проблемы с ограничением частоты запросов OpenAI
...
- Fix: ограничение на максимальное количество токенов OpenAI, изменение максимального количества символов с 1500 до 1300.
...
- Исправление: проблема с сетью проверки сервиса перевода на firefox mobile.
...
- Исправление: синхронизация google drive на android для firefox.
...
- Исправление ошибки синхронизации userscript с google drive.
...
- Исправление: переключение маски userscript с сенсорным событием.
...
- Улучшение для всплывающего интерфейса больше
...
- Поддержка selectorMatches, чтобы теперь можно было сопоставить все manstondon.
