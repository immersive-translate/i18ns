...
- Добавлено: Динамический предварительный перевод контента, который нужно перевести при просмотре страниц.
...
- Новое: Доступен бесплатный перевод изображений (в настоящее время поддерживается только в версиях браузеров Chrome и Edge для ПК), доступный через контекстное меню.
...
- Добавлено: Режим Forece Enable Mouse Support (включайте эту функцию только если функция наведения мыши недоступна на планшетных устройствах) **Settings** -> **Advanced Settings** -> **Forece Enable Mouse Support**
- Добавлено: Отображение сообщения об ошибке при неудачном переводе субтитров видео
...
- Двуязычные субтитры YouTube теперь поддерживают интеллектуальное разбиение предложений (бета) (только при ручном включении immersive translation субтитров YouTube в [Settings] - [Video Subtitles], и если оригинальные субтитры видео автоматически сгенерированы на английском языке)
...
- Варианты панели теперь поддерживают [вход для текстового перевода](https://app.immersivetranslate.com/text/)
...
- Оптимизация: Страница настроек, взаимодействие с URL списком (чтобы избежать двусмысленности, флажки не отображаются по умолчанию).
...
- Добавлена поддержка новой настройки ярлыков, позволяющей быстро переключать использование встроенных машинных переводов для видео субтитров
...
- Исправлена проблема с ярлыком для двуязычных субтитров на YouTube, где текст на английском был слишком длинным.
...
- Для более старых версий скрипта Tampermonkey теперь добавлено уведомление об обновлении на странице настроек.
...
- Теперь hoverball скрыт по умолчанию, когда видео на полном экране.
- Исправлена проблема дрожащего нажатия на панель действий hoverball в Firefox.
...
- Исправлены дрожащие нажатия на hoverball страницы
...
- Исправлена проблема, из-за которой: временная настройка продолжительности перевода веб-сайта не вступала в силу.
...
- Рефакторинг ядра веб-парсинга, что означает:
  ...
  - Адаптация к большему количеству веб-сайтов

- Поддерживаются все переводы веб-сайтов ShadowRoot.
...
- Исправлена проблема белого экрана после перевода веб-сайтов с гидратацией, таких как Next.js.
...
- Ядро веб-парсинга было перестроено, что означает.
  ...
  - Лучшая совместимость с большим количеством веб-сайтов

- Теперь поддерживаются переводы для всех веб-сайтов с ShadowRoot.
...
- Исправлено: проблема с всплывающим окном новой вкладки macOS.
...
- Исправлено: настройки горячих клавиш наведения мыши userscript
...
- Функция: Bing Translate на Microsoft Translate для более стабильного качества.
- Исправлено: Чистая TXT веб-страница, как [эта](https://edoras.sdsu.edu/~vinge/misc/singularity.html)
- Производительность: Исключение некоторых дочерних рекламных страниц на веб-сайте, и производительность немного улучшилась

## 0.4.7

- Исправлено: отсутствует всплывающее окно Userscript Firefox.
...
- Исправлено: проблема с расширением safari iOS 15.0.
- Исправлено: горячие клавиши расширения safari macOS
...
- Функция: поддержка ChatGPT Plus Website в качестве службы перевода, это бета-функция, поэтому вы можете получить к ней доступ, включив бета-функцию в настройках разработчика. Это очень медленно, потому что chatgpt может отправлять только один запрос за раз. Убедитесь, что у вас есть учетная запись ChatGPT Plus, потому что на бесплатной учетной записи больше ограничений, и я не уверен, несет ли это риск для вашей учетной записи, будьте осторожны, чтобы убедиться, что у вас есть учетная запись ChatGPT Plus, потому что на бесплатной учетной записи больше ограничений, и я не уверен, несет ли это риск для вашей учетной записи, будьте осторожны, чтобы использовать ее.
...
- Исправлено: проблема с конкретными символами в конструкторе eBook, например, некоторые пути к главам `xxx' xxxx`.
- UI: свернуть пользовательские параметры openai по умолчанию.
- UI: Добавить статус экспорта для экспорта epub.
- Исправлено: Gpt4 запрос по умолчанию
...
- Исправлено: переключатель UI Options не изменяет состояние ввода служб перевода.
...
- Исправлено: экспорт eBook userscript
- Функция: добавить пользовательскую конечную точку API для OpenAI
- Функция: добавить временные параметры времени перевода веб-сайта в "Расширенные настройки"
...
- Функция: OpenAI поддерживает настраиваемый apiUrl для настроек разработчика.
...
- Исправлено: проблема с сетью перевода на мобильном Firefox.
...
- Исправлено: синхронизация Google Drive на Android Firefox.
...
- Удаление разрешений на вкладки для отклонения в Chrome Store
...
- Исправлено: настройка всплывающей страницы на новой вкладке
...
- Исправлено: дополнение для Android Firefox
...
- Добавить временный сайт перевода на некоторое время
...
- Исправление ошибки переноса строк.
...
- Форматирование мета-информации userscript
...
- Исправление перевода содержимого постов на мобильном reddit.
...
- Поддержка установки режима немедленного перевода, чтобы вы могли
  переводить веб-страницу как можно быстрее.
- Поддержка установки области страницы, которую нужно перевести, чтобы вы могли
  переводить больше областей.
