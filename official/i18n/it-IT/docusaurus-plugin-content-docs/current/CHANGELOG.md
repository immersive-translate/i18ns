...
La **Preview version** viene pubblicata più frequentemente e include alcune funzionalità sperimentali. Rispetto alla versione Release, può contenere più bug. Viene principalmente rilasciata su
...
- Risolto: Gli aggiornamenti dei termini non inviano nuove richieste di traduzione.
...
- Dopo aver selezionato il testo, il menu del tasto destro cambierà in [Traduci il testo], cliccando su di esso si può saltare automaticamente alla pagina di traduzione del testo di Immersive Translation
...
- Nuovo pulsante di feedback utente per la traduzione di manga (dopo aver tradotto un manga, clicca sul pulsante [Feedback] sul lato destro della pallina fluttuante per dare feedback sulla qualità della traduzione).
...
- Risolti i problemi di layout delle traduzioni dei fumetti per le lingue con script latino.
...
- Ottimizza la performance di traduzione per file di grandi dimensioni in PDF-Pro
...
- Risolto il problema di visualizzazione anomala del plugin mobile quando si aprivano pagine PDF.
...
- Abilita automaticamente la traduzione di siti web inglesi frequentemente utilizzati per i nuovi utenti
- Risolto il problema di traduzione con https://claude.ai/

## 0.7.6

- Supporto migliorato per i risultati di traduzione ctrl+z Annulla
- Supporto alla traduzione in modalità di lettura documenti di Flying Book
...
- Risolto il problema per cui gli eBook bilingue non avevano immagini dopo l'esportazione
...
- Ottimizzato: il numero minimo di caratteri in un paragrafo che attiva la traduzione è stato modificato a un minimo di 4 caratteri per ridurre la confusione, utilizzando altre funzionalità per evitare di tradurre le aree di navigazione e di fine del sito.
- Risolto: dettagli di Github non tradotti dopo l'espansione.

## 0.5.14

- Risolto il problema per cui le immagini su alcune pagine web diventavano più grandi dopo la copia
- Risolto: sezione commenti di medium non tradotta
- Risolto il problema per cui le immagini su alcune pagine venivano copiate in modo errato
...
- Risolto un problema con: alcuni tag di codice non riconosciuti correttamente.
...
- Ottimizzato il problema di limitazione della frequenza delle richieste OpenAI
...
- Risolto: popup userscript di Firefox mancante.
...
- Risolto: problema di estensione safari iOS 15.0.
- Risolto: scorciatoie dell'estensione safari macOS
...
- Funzionalità: Supporto OpenAI con chiavi multiple, usa `,` per separarle.
...
- Correzione: traduzione file PDF fallita, Bug dalla 0.2.60 per supportare deepl da zh-CN a zh-TW
...
- Correzione problema richiesta macaco
- Correzione segna altezza linea evidenziata
...
- Correzione modifica url client, auto mantiene lo stato di traduzione.
...
- aggiornamento versione per test ci
