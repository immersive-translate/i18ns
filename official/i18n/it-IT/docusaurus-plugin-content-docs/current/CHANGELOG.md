...
- Aggiunto: Supporto per impostare il numero di schermi da pre-tradurre durante la navigazione della pagina corrente.
...
- Risolto: Un problema con la traduzione al passaggio del mouse che non funzionava nella modalità "Prima traduzione, poi testo originale".
...
- Risolto: Problema in cui il contenuto scompare dopo aver cliccato su "vedi di più" nella modalità solo traduzione di Facebook.
...
- Aggiunto: Modalità Forza Abilita Supporto Mouse (Abilita questa funzione solo se la funzione di passaggio del mouse non è disponibile sui dispositivi tablet) **Impostazioni** -> **Impostazioni Avanzate** -> **Forza Abilita Supporto Mouse**
...
- L'opzione per il confronto bilingue/solo traduzione è stata spostata nelle impostazioni generali.
- Per impostazione predefinita, il sistema ricorderà la modalità attivata cliccando sull'icona nel pannello per il confronto bilingue o la visualizzazione solo traduzione. Per passare temporaneamente, clicca su "Altro" -> "Passa alla visualizzazione solo traduzione" nel pannello.
...
- Il servizio di traduzione DeepL ora supporta ufficialmente il cinese tradizionale come lingua di destinazione (in precedenza, tradurre in cinese tradizionale con DeepL comportava un processo di conversione aggiuntivo da cinese semplificato a cinese tradizionale).
...
- DeepL, Microsoft Translate, Google Translate, OpenAI, Claude, Gemini e altri servizi di traduzione supportano la traduzione per mantenere la formattazione del testo originale (ad esempio, link, grassetto, ecc.)
- Dopo aver selezionato il testo, il menu del clic destro cambierà in [Traduci il testo], cliccando su di esso si può automaticamente saltare alla pagina di traduzione del testo di Immersive Translation
...
- Nuova funzionalità: traduzione di manga (Beta), nei siti web di manga supportati, un pulsante di traduzione manga apparirà sotto la sfera fluttuante di traduzione rapida della pagina web. Cliccando su di esso si attiverà la traduzione del manga. Questa funzionalità è disponibile per i membri Pro (500 pagine al mese, pacchetti aggiuntivi possono essere acquistati), attualmente supporta i seguenti siti:
...
- Risolto il problema per cui la modifica della lingua di destinazione e altre impostazioni nel pannello popup si resettava cliccando sulla sfera fluttuante di traduzione.
...
- Risolto il problema delle dimensioni del font che non si ridimensionavano proporzionalmente quando YouTube era a schermo intero [#1681](https://github.com/immersive-translate/immersive-translate/issues/1681)
...
- Ottimizzata significativamente la velocità di traduzione dei sottotitoli di YouTube, e aggiunto supporto per il cambio tra ordine cinese e inglese, personalizzazione del colore e della dimensione del font, ecc.
...
- Il pannello switch per visualizzare solo il pulsante di traduzione è stato modificato per avere effetto solo sulla pagina corrente
...
- Modificata la descrizione della scorciatoia per il pannello mobile.
...
- Ottimizzata la sensibilità del rilevamento del passaggio del mouse su Ctrl per evitare di confondere `Ctrl+C` come `Ctrl`
...
- **Voce di traduzione documenti integrata:** Ora puoi tradurre file PDF, ePub e sottotitoli tramite un ingresso di caricamento unificato. Basta fare clic sul pulsante [PDF/ePub] nel pannello popup, senza bisogno di selezionare [Altro] più.
...
- **Impostazioni dei Tasti di Scelta Rapida**: Aggiunto un nuovo menu di primo livello "Scorciatoie" e le seguenti funzioni di tasti di scelta rapida personalizzabili:
  ...
  - Aggiunti tasti di scelta rapida dedicati per 6 servizi di traduzione (come DeepL, OpenAI, Google, Microsoft, Gemini, Tencent Interactive Translation) per facilitare il passaggio temporaneo tra i servizi di traduzione.

- **Aggiornamento dell'interfaccia utente della pagina delle impostazioni del plugin**:

  - In "Impostazioni avanzate", è stata aggiunta una nuova opzione per consentire agli utenti di specificare determinate parole (ad esempio, "LLM") da escludere dalla traduzione.
...
- **Aggiornamento della Configurazione openAI**: Ora supporta l'impostazione del numero di configurazioni al secondo in decimali, come 0.5, che significa 1 richiesta ogni 2 secondi.
...
- Riparato il problema in cui i sottotitoli video originali non venivano visualizzati quando "Il sito corrente è impostato per non tradurre mai".
...
- Risolto il problema di clic tremolante del pannello di azione hoverball della pagina di Firefox.
...
- Quando si attivano i sottotitoli bilingue nel pannello, la pagina verrà prima aggiornata automaticamente (per visualizzare i sottotitoli bilingue in modo più accurato), e alcuni siti richiedono ancora agli utenti di fare clic manualmente sul pulsante "CC" sul sito per attivare i sottotitoli.
...
- Risolto il problema di clic tremolante della hoverball della pagina
- Risolti i problemi di traduzione di Azure
...
- Supporto per l'impostazione dei pulsanti di traduzione rapida hoverball (supporto sia per PC che per mobile)
...
- Supporto per la traduzione dei sottotitoli di Youtube
...
- Ingrandire la scala dei piccoli font PDF su Chrome e Safari
...
- Risolvere alcuni stili di casella di input
...
- Ottimizzare il problema delle traduzioni OpenAI che aggiungono punti senza motivo, (se non viene rilevato alcun punto nel testo originale, se openai restituisce un punto, allora rimuoverlo)
...
- Fix: Evita Content Security Policy per [Mastondon](https://mastodon.social/)
...
- Feat: app iOS aggiunge pulsante di abilitazione dell'estensione e link alle comunità
- UI: Il modello del campo delle impostazioni OpenAI utilizza la selezione invece della casella di input del testo.

## 0.4.5

- Fix: problema di estensione safari iOS 15.0.
- Fix: scorciatoie dell'estensione safari macOS
...
- Feat: Aggiungi riprova per tutti i paragrafi falliti per il pulsante di riprova.
...
- Cambia: colore dell'icona di caricamento, da nero a blu, per adattarsi alla pagina web in modalità scura.
...
- Feat: Consenti al modulo di input password di mostrare la password.
...
- Feat: Supporta il lettore e il creatore di Ebook
...
- Fix: alto utilizzo di memoria per userscripts ed estensione (rimuovendo opencc zh-CN a zh-TW, invece con Google translate)
- Fix: Relingo [#159] (https://github.com/immersive-translate/immersive-translate/issues/159)
- Fix: configurazione traduzione Azure ma ancora mostra (necessaria configurazione)
...
- Fix: sincronizzazione google drive android nativa firefox.
...
- Migliore: ridurre dimensione file

## 0.2.60

- Feat: Supporto zh-CN a zh-TW per Deepl
- Feat: Caratteristica Deepl Immersive Translate
...
- Feat: supporto css iniettato su pannello sviluppatore
...
- Fix traduci l'intera pagina, il piè di pagina è ignorato
...
- Fix Google Auth Necessita Accesso su Chrome/Firefox
...
- Fix getpocket.com
- Fix piè di pagina per area corpo
...
- Fix traduzione volc per rilevamento automatico lingua
...
- Correggi l'aggiornamento forzato della ricerca di Google
...
- Supporta l'impostazione del primo x conteggio di testo da tradurre immediatamente.
