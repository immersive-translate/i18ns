...
- Aggiunto: Gestione della libreria di terminologia AI
...
- Risolto: Problema di miglioramento della casella di input di Midjourney che ripeteva il testo originale.
...
- Aggiunto: La **Traduzione Sottotitoli** in tempo reale ora supporta l'attivazione tramite "float ball", disponibile su Zoom, Google Meet e Microsoft Teams.
...
- Per impostazione predefinita, il sistema ricorderà la modalità attivata cliccando l'icona nel pannello per la visualizzazione solo traduzione o confronto bilingue. Per passare temporaneamente, clicca su "Altro" -> "Passa alla visualizzazione solo traduzione" nel pannello.
...
- Aggiunta l'opzione per "Segnalare problemi di traduzione della pagina corrente" e l'opzione per "Accendere/spegnere rapidamente il float ball" nelle opzioni aggiuntive del pannello.
...
- Nuovo pulsante di feedback utente per la traduzione di manga (dopo aver tradotto un manga, clicca il pulsante [Feedback] sul lato destro del float ball per dare feedback sulla qualità della traduzione).

## 1.7.7

- Adottato algoritmo di suddivisione intelligente delle frasi per sottotitoli inglesi auto-generati su YouTube [Pro Disponibile]
...
- Nuova funzionalità: la traduzione AI supporta [Doubao large model](https://www.volcengine.com/product/doubao)
...
- Risolto il problema per cui la dimensione del font non si scalava proporzionalmente quando YouTube è a schermo intero [#1681](https://github.com/immersive-translate/immersive-translate/issues/1681)
...
- Recentemente, le richieste del servizio di traduzione Microsoft dalla Cina sono state instabili; quando si verificano errori, il sistema rileva automaticamente il servizio di traduzione attualmente disponibile, consentendo agli utenti di passare rapidamente.
...
- Supporto per l'impostazione di pulsanti di traduzione rapida hoverball (supporto sia PC/mobile)
- Ottimizzazione del giudizio della lingua di Grease Monkey
- Risolto il problema di traduzione del file txt

## 0.10.7

- Supporto al passaggio del mouse per premere nuovamente Ctrl per mostrare il testo originale
...
- Aggiunta la [Disabilitazione Permanente della Funzione Sottotitoli Video], se ci sono altri problemi di conflitto e non si desidera abilitare la funzione di sottotitoli bilingue con Immersive Translate.
- Ottimizzazione delle interruzioni dei sottotitoli

## 0.10.3

- Risolto il problema di traduzione della Custom DeppL Auth Key
...
- Supporto per la traduzione dei risultati di Input Enhanced Support ctrl+z Annulla
- Supporto per la traduzione in modalità di lettura documenti di Flying Book
...
- Risolto il problema per cui le immagini su alcune pagine web: diventano più grandi dopo la copia
...
- Risolto: postMessage quando la pagina di inizializzazione rompe alcuni siti, ora lo faremo solo quando traduciamo davvero le pagine
...
- Risolto: il progresso del costruttore di ebook si ferma al 99%.
...
- Funzionalità: aggiungi il progresso della traduzione per il costruttore di ebook, e una bella pioggia di coriandoli dopo il successo.
- Funzionalità: aggiungi il pulsante di riprova per tutti i paragrafi falliti.
...
- Risolto: creatore di ebook per Kindle
...
- Correzione traduzione intera pagina, piè di pagina ignorato
...
- Correzione piè di pagina aside per area corpo
...
- Cambia evidenzia a segno
- Aggiungi tema traduzione evidenziato
...
- Supporto per il gestore delle scorciatoie di Grease Monkey.
...
- Risolto il problema delle due colonne nei PDF.
- Risolto il bug della versione del Chrome/Edge Store.
- Risolto
...
- Pubblicato su Edge
- risolto il problema del margine nei PDF.
...
- Risolto il problema dell'estensione su Firefox

## 0.0.48

- Risolto il problema dell'estensione su Chrome
...
- Risolto il problema dei caratteri speciali su Wikipedia
