...
- 修复：Safari 浏览器下 Cloudflare 人机验证失败的问题。
- 修复：iframe 内容翻译响应较慢时提前超时的问题，提升稳定性。
- 修复：长文本弹窗中“记住选项”设置不生效的问题。
- 修复：【输入框翻译】在 Firefox、Safari 浏览器下按下 `Option + 字母键` 时无法正常生效的问题。
- 修复：Firefox 浏览器在部分 iframe 页面中无法正常翻译的问题。

## 1.27.2 Release (2026-03-27)

- 优化：提升了免费 AI 翻译服务的表现与稳定性。
- 优化：增强了韩语和日语翻译场景中 AI 上下文理解能力，提升翻译质量。
- 优化：提升非 Chrome 浏览器下输入框自动语言识别的准确性与稳定性。
- 新增：谷歌翻译服务现已支持塔马齐格特语（Tamazight）。
- 修复：解决 iOS 全屏播放 YouTube 视频时字幕无法翻译的问题。
...
- 修复：修正部分浏览器下侧边栏无法正常显示的问题，提升兼容性和稳定性。
- 优化：iOS Safari 插件与 APP 的地区切换实现同步，体验更连贯。
- 优化：进一步提升在线 PDF 文档翻译在多场景下的兼容性。
- 优化：网页译文展示将更好地保留原网站的交互与视觉效果，保持一致性。

## 1.24.6 Release (2025-12-31)

- 修复：特定环境下侧边栏无法唤起的问题，提升了交互稳定性。
- 修复：钉钉、微信等三方平台账号无法正常登录的问题（因 iframe 通信干扰导致）。
- 优化：【AI Writing】新增极简模式。

## 1.24.4 Preview (2025-12-23)

- 优化：【输入框翻译】首次使用时的提示文案更新，提升用户理解。
- 优化：错误弹窗样式现已支持夜间模式，夜间使用更舒适。
- 优化：界面语言新增支持印尼语。

## 1.24.1 Preview (2025-12-08)

- 优化：优化了配置信息的同步机制，提升了同步效率和稳定性。
...
- 修复：【输入框翻译】修正了在 bilibili 站点无法正常翻译的问题。
- 修复：【输入框翻译】在部分安卓系统的浏览器（如：Edge）下无法触发的问题。
- 修复：【输入框翻译】提升对 Atlas 浏览器和指纹浏览器的兼容性。
- 修复：【双语字幕】在 TED 视频站点无法翻译的问题。
- 修复：【双语字幕】在模型翻译不够快时出现字幕无法及时显示的现象，提升翻译的稳定性。
- 优化：【双语字幕】提升 CNN 站点的兼容性。
- 优化：【AI 字幕】功能支持“重新生成”，可根据需要重新生成字幕内容。
- 优化：【长段落自动换行】开启后，长段落翻译时，自动添加分隔线区分段落，提升内容可读性。
...
- 修复：移动端插件面板切换翻译服务时状态不同步的问题
- 修复：免费图片翻译功能在某些情况下加载失败的问题
...
- 优化：术语功能新增域名匹配支持，提升术语应用的精准性与灵活性。
...
- 优化：Chrome 浏览器使用内置语言检测模型，提升语言识别效率。
- 优化：完善对 OpenAI Responses 接口的兼容性。
- 优化：漫画翻译支持更多站点。
